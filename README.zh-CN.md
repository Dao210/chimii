...
| 模块 | 已有代码 | 已执行测试 | 部署状态 | 实物验证 |
| --- | --- | --- | --- | --- |
| 工作区、聊天与执行基础 | 已实现 | 消息缓存及 PostgreSQL 清理、并发专项测试，详见验收记录 | 本次未重新核实 | 不适用 |
| 积木规划与编译 | 通用目标形状、澄清、单模型库存约束、已审阅模块、校验及 MPD 导出 | 本地编译器、接口检查及模型夹具驱动的浏览器流程 | 本次未重新核实 | 未进行；几何校验不代表实物测试 |
| 积木搭建进度 | 开始、继续、完成，版本冲突控制及摘要列表 | 本地接口及界面测试，详见验收记录 | 需要迁移 289；本次未部署 | 未进行 |
| 电路搭建 | 库存保存、步骤进度、参考电路、BOSON 模块连接及家庭实测记录 | 本地编译器、接口测试及收音机、BOSON 浏览器流程 | 本次未重新核实 | 参考设计仍标记 `not_tested`；家庭反馈作为独立证据 |
| 移动端作品入口 | 原生最近作品列表、摘要详情；完整搭建打开网页 | 移动端检查及限制详见验收记录 | 本次未发布移动端版本 | 未进行 |
| 多模态创意与家长成长门户 | 语音、草图规划和成长洞察仍属路线图 | 尚未建立验证结果 | 尚未建立部署证据 | 未进行 |

状态对应 2026-09-05 本地工作区，包含待提交或待发布内容。测试明细与限制见[验收记录](docs/plans/2026-09-05-reliability-acceptance.md)。本地测试不代表已部署或通过硬件验收。[《奇觅发明家 PRD》](docs/plans/%E5%A5%87%E8%A7%85%E5%8F%91%E6%98%8E%E5%AE%B6prd%2020260728.md)描述更完整的产品方向。

2026-09-08 的 [BrickGPT 对照实验](tasks/build-eval-first-batch.md)新增了离线 100 案例基线、固定版本的受力分析桥接及数据集候选导入。只有可选的 BrickGPT 原模型复现需要 Llama 基础权重；Chimii 继续使用 DeepSeek。多数受力对照遇到当前求解器许可的规模限制。

2026-09-09 的[算法集成](tasks/brickgpt-integration.md)已将连续接缝优先级和有限邻域重铺接入 Go 编译器的库存受限搜索。原有 100 案例基线保持一致，另设的 80 个库存案例从 68 个接受提高到 71 个。这是本地编译器验证，尚未部署，未完成真实模型或实物验收。
