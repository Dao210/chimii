...
2026-09-08 的 [BrickGPT 对照实验](tasks/build-eval-first-batch.md)新增了离线 100 案例基线、固定版本的受力分析桥接及数据集候选导入。确定性契约检查已通过；模型推理仍需要获授权的基础权重，多数受力对照遇到当前求解器许可的规模限制。工具尚未部署，未完成实物验证。
