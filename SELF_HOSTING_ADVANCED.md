...
This document covers advanced configuration for self-hosted Chimii deployments. For the quick start guide, see [SELF_HOSTING.md](SELF_HOSTING.md).
...
|----------|-------------|---------|
| `DATABASE_URL` | PostgreSQL connection string | `postgres://chimii:chimii@localhost:5432/chimii?sslmode=disable` |
...
Chimii supports two email backends. `SMTP_HOST` takes priority when set; otherwise `RESEND_API_KEY` is used. With neither configured, verification codes are printed to the server log — copy them from there to log in.
...
| `RESEND_FROM_EMAIL` | Sender email address (default: `noreply@chimii.ai`) |
...
> **Note:** If neither Resend nor SMTP is configured, generated verification codes are printed to backend logs — copy them from there to log in. A fixed local testing code (e.g. `888888`) is **opt-in only**: set `CHIMII_DEV_VERIFICATION_CODE=888888` and keep `APP_ENV` non-production. Native production services must set `APP_ENV=production`. **Never enable a fixed code on a publicly reachable instance.**
...
Changes take effect after restarting the backend service. The web UI reads `GOOGLE_CLIENT_ID` from `/api/config` at runtime, so no web rebuild is needed.
...
Changes take effect after restarting the backend service. The web UI reads `ALLOW_SIGNUP` and `DISABLE_WORKSPACE_CREATION` from `/api/config` at runtime, so no web rebuild is needed.
...
| `ATTACHMENT_DOWNLOAD_MODE` | Attachment download behavior: `auto` (default), `cloudfront`, `presign`, or `proxy`. Use `proxy` for buckets reachable only through a private network, such as `http://rustfs.internal:9000` |
...
If the frontend and backend are served from different hostnames, `COOKIE_DOMAIN` is **required**, not optional: the browser must be able to read the `chimii_csrf` cookie from the page's own origin to send the `X-CSRF-Token` header, and without it every write request fails with `403 {"error":"CSRF validation failed"}`. Scope it to the narrowest parent domain that covers both hosts — it also shares the `chimii_auth` session cookie with every host under that domain. See [Reverse Proxy](#reverse-proxy) for the full split-domain configuration and its trust requirements.
...
| Variable | Default | Description |
|----------|---------|-------------|
| `CHIMII_SERVER_URL` | `ws://localhost:8080/ws` | WebSocket URL for daemon → server connection |
| `CHIMII_APP_URL` | `http://localhost:3000` | Frontend URL for CLI login flow |
| `CHIMII_DAEMON_POLL_INTERVAL` | `3s` | How often the daemon polls for tasks |
| `CHIMII_DAEMON_HEARTBEAT_INTERVAL` | `15s` | Heartbeat frequency |
...
| `CHIMII_COPILOT_PATH` | Custom path to the `copilot` (GitHub Copilot CLI) binary |
...
| `CHIMII_GROK_PATH` | Custom path to the `grok` binary |
| `CHIMII_GROK_MODEL` | Override the Grok model used (e.g. `grok-4.5`) |

## Database Setup

Chimii requires PostgreSQL 17 with the pgvector extension.
...
Set `DATABASE_URL` in `/etc/chimii/backend.env` to the PostgreSQL connection string.

### Running Migrations Manually

Run migrations before activating each new backend release:
...
The Usage and Runtime dashboards read from `task_usage_hourly`, a derived table populated by `rollup_task_usage_hourly()`. As of MUL-2957 the backend runs this rollup **in-process** on every replica via a DB-backed scheduler (`sys_cron_executions`); a fresh self-host install needs no operator action beyond PostgreSQL 17 with pgvector.
...
DATABASE_URL="postgres://..." \
  /opt/chimii/current-backend/backfill_task_usage_hourly \
  --sleep-between-slices=2s

# Kubernetes
kubectl -n chimii exec deploy/chimii-backend -- \
...
If you are upgrading from a binary that pre-dates MUL-2957 (or the auto-hook fails for an environmental reason), recovery is the manual path: run `backfill_task_usage_hourly` against the database, then re-run `migrate up`. **Fresh installs are exempt** — the guard short-circuits when `task_usage` is empty, and the in-process scheduler picks up new buckets from the first tick.

## Manual Setup

If you prefer to build and run services manually:

**Prerequisites:** Go 1.26+, Node.js 22, pnpm 10.28+, PostgreSQL 17 with pgvector.

```bash
# Start PostgreSQL and ensure pgvector is available
...
**Single-domain layout** — frontend and backend served on the same hostname:

```
chimii.example.com {
    # WebSocket route — must come before the catch-all
    @chimii_ws path /ws /ws/*
    handle @chimii_ws {
...
REMOTE_API_URL=http://127.0.0.1:8080 # target the Next.js rewrites proxy /api, /auth and /uploads to
```
...
By default, Chimii works on `localhost`. If you access it from another machine on the LAN (e.g. `http://192.168.1.100:3000`), you need to tell the backend to accept that origin:
...
```bash
sudo systemctl restart chimii-backend chimii-web
```
...
2. **Bake a WebSocket URL into the web bundle.** If you are not running a reverse proxy, rebuild the web bundle with `NEXT_PUBLIC_WS_URL` pointing straight at the backend (port 8080 must be reachable from the browser):

   ```bash
   # In .env
   NEXT_PUBLIC_WS_URL=ws://<lan-ip>:8080/ws

   # Rebuild the target-compatible standalone bundle
   NEXT_PUBLIC_WS_URL=ws://<lan-ip>:8080/ws \
     STANDALONE=true pnpm --filter @chimii/web build
   ```

   `NEXT_PUBLIC_WS_URL` is a build-time variable, so changing only the systemd runtime environment has no effect. Deploy the rebuilt standalone bundle and restart `chimii-web`.

**Also required: allowlist the browser origin.** The two options above fix the WebSocket *upgrade proxying*, but a second, independent setting gates the connection: the backend validates the WebSocket `Origin` header against an allowlist that defaults to `localhost` only. When you open Chimii from any other origin — a LAN IP **or a public domain behind a reverse proxy** — set `CORS_ALLOWED_ORIGINS` (or `FRONTEND_ORIGIN`) on the backend to that exact origin and restart, exactly as shown under [LAN / Non-localhost Access](#lan--non-localhost-access) above. Otherwise the upgrade is refused with `403`: the backend logs `websocket: request origin not allowed by Upgrader.CheckOrigin` and the browser console loops `disconnected, reconnecting in 3s`, while HTTP requests (and manual page refreshes) keep working because they are same-origin to the page. The single value covers both HTTP CORS and the WebSocket origin check.

> **Note:** If you need to hard-code a different public API or WebSocket endpoint for any other reason, rebuild and redeploy the target-compatible standalone web bundle.
...
For native Linux or Kubernetes deployments, prefer a private scrape path: bind
the metrics listener to an internal interface and protect it with private
networking, allowlists, NetworkPolicy, or proxy authentication. Do not expose
the metrics listener on a public interface.

## Upgrading

Build target-compatible artifacts, upload them to new immutable release
directories, stop the backend, run the new release's `migrate up`, switch the
backend and web symlinks, and restart both systemd units. See
[Self-Hosting Guide → Database Setup and Migrations](SELF_HOSTING.md#database-setup-and-migrations).
