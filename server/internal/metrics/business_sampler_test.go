...
func TestBusinessSamplerCollectorEmitsExpectedMetrics(t *testing.T) {
	...
	wantSubstrings := []string{
		`chimii_active_users{window="5m"} 7`,
		`chimii_active_workspaces{window="5m"} 3`,
		`chimii_agent_task_queued{source="chat"} 5`,
		`chimii_agent_task_queued{source="issue"} 2`,
		// Zero series for sources that didn't appear in the snapshot.
		`chimii_agent_task_queued{source="autopilot"} 0`,
		`chimii_agent_task_queued{source="other"} 0`,
		`chimii_agent_task_running{runtime_mode="cloud",source="chat"} 3`,
		`chimii_agent_task_running{runtime_mode="local",source="issue"} 1`,
		`chimii_agent_task_stuck_total{source="issue"} 1`,
		`chimii_runtime_online{provider="claude",runtime_mode="local"} 4`,
		`chimii_runtime_online{provider="kiro",runtime_mode="cloud"} 2`,
		`chimii_runtime_heartbeat_age_seconds_count{runtime_mode="local"} 3`,
		`chimii_runtime_heartbeat_age_seconds_sum{runtime_mode="local"} 45`,
		`chimii_workspace_total 250`,
	}
	...
	for _, removed := range []string{
		`chimii_active_users{window="1h"}`,
		`chimii_active_users{window="24h"}`,
		`chimii_active_workspaces{window="1h"}`,
		`chimii_active_workspaces{window="24h"}`,
	} {
...
func TestBusinessSamplerSelfIntrospectionHistogramIsExposed(t *testing.T) {
	...
	NewHandler(registry).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/metrics", nil))
	body := rec.Body.String()
	if !strings.Contains(body, `chimii_business_sampler_query_seconds_count{name="active_users"} 1`) {
...
func TestBusinessSamplerCollectorBoundedCardinality(t *testing.T) {
	...
	if got := testutil.CollectAndCount(c, "chimii_agent_task_queued"); got != len(knownSourceLabels()) {
	...
	if got := testutil.CollectAndCount(c, "chimii_agent_task_running"); got != expectedRunning {
		t.Fatalf("agent_task_running series = %d, want %d", got, expectedRunning)
	}
	if got := testutil.CollectAndCount(c, "chimii_runtime_online"); got != 1 {
...
func TestBusinessSamplerCollectorDisabledWithoutOptions(t *testing.T) {
	...
	for _, forbidden := range []string{
		"chimii_active_users",
		"chimii_agent_task_queued",
		"chimii_runtime_online",
		"chimii_business_sampler_query_seconds",
	} {
