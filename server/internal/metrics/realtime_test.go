...
func TestRealtimeCollectorExposesCounters(t *testing.T) {
	...
	for _, want := range []string{
		"chimii_realtime_active_connections 3",
		"chimii_realtime_messages_sent_total 11",
		"chimii_realtime_redis_connected 1",
		`chimii_realtime_redis_mirror_errors_total{target="primary"} 2`,
		`chimii_realtime_redis_mirror_errors_total{target="secondary"} 5`,
	} {
