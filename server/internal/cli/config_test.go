...
func TestCLIConfig_BackwardCompat_OldFileLoadsWithNilBackends(t *testing.T) {
	...
	cfgDir := filepath.Join(tmp, ".chimii")
	if err := os.MkdirAll(cfgDir, 0o755); err != nil {
		t.Fatal(err)
	}
	historical := `{
  "server_url": "https://api.chimii.ai",
  "app_url": "https://chimii.ai",
  "workspace_id": "ws-123",
  "token": "mul_abcdef"
}`
	...
	if cfg.ServerURL != "https://api.chimii.ai" {
...
func TestCLIConfig_BackwardCompat_NilBackendsOmittedFromJSON(t *testing.T) {
	...
	data, err := os.ReadFile(filepath.Join(tmp, ".chimii", "config.json"))
	if err != nil {
		t.Fatal(err)
	}
	if string(data) == "" {
...
func TestCLIConfig_ProfileCommandOverrides_OmittedWhenEmpty(t *testing.T) {
	...
	cfg := CLIConfig{ServerURL: "https://api.chimii.ai", Token: "mul_xyz"}
	if err := SaveCLIConfig(cfg); err != nil {
		t.Fatal(err)
	}
