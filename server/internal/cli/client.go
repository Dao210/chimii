...
// Set by the chimii binary at init() so the package doesn't depend on the
...
// APIClient is a REST client for the Chimii server API.
...
// reading a capped slice of the body. Every Chimii API helper funnels its
...
// It can be overridden with the CHIMII_HTTP_TIMEOUT environment variable
...
// httpTimeout returns the HTTP client timeout, honoring CHIMII_HTTP_TIMEOUT.
...
func httpTimeout() time.Duration {
	v := strings.TrimSpace(os.Getenv("CHIMII_HTTP_TIMEOUT"))
	if v == "" {
...
// which honors CHIMII_HTTP_TIMEOUT) plus a small grace margin, so a
...
// hardcoded duration so the deadline always respects CHIMII_HTTP_TIMEOUT.
