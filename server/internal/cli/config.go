...
const defaultCLIConfigPath = ".chimii/config.json"
...
type CLIConfig struct {
	...
	// CHIMII_DAEMON_DEVICE_NAME env, this field, os.Hostname().
	...
	// CHIMII_AGENT_RUNTIME_NAME env, this field, the built-in
	// DefaultRuntimeName.
	...
	// CHIMII_DAEMON_MAX_CONCURRENT_TASKS env, this field, default.
	...
	// CHIMII_DAEMON_POLL_INTERVAL env, this field, DefaultPollInterval.
	...
	// flag, CHIMII_DAEMON_HEARTBEAT_INTERVAL env, this field,
	...
	// explicit 0), CHIMII_AGENT_TIMEOUT env, this field,
	// DefaultAgentTimeout.
	...
	// CHIMII_CODEX_SEMANTIC_INACTIVITY_TIMEOUT env, this field,
	...
	// CHIMII_CODEX_HANDSHAKE_TIMEOUT env, this field,
	...
	// already resolves to enabled on Chimii Cloud. Absent / false means
	// "let env/default decide". Resolution precedence:
	// --no-auto-update flag, CHIMII_DAEMON_AUTO_UPDATE=false env, this
	...
	// --auto-update-interval flag, CHIMII_DAEMON_AUTO_UPDATE_INTERVAL
	...
	// `chimii runtime profile set-path`; the daemon prefers it over the
...
//
//	BinaryPath: CHIMII_OPENCLAW_PATH (env)  > backends.openclaw.binary_path > PATH lookup
...
// CHIMII_OPENCLAW_STATE_DIR. Rationale: OpenClaw already honors its own env
...
// OpenClaw's own env, CHIMII_OPENCLAW_STATE_DIR can be layered on top
...
// An empty profile returns the default path (~/.chimii/config.json).
// A named profile returns ~/.chimii/profiles/<name>/config.json.
func CLIConfigPathForProfile(profile string) (string, error) {
	...
	return filepath.Join(home, ".chimii", "profiles", profile, "config.json"), nil
}

// ProfileDir returns the base directory for a profile's state files (pid, log).
// An empty profile returns ~/.chimii/. A named profile returns ~/.chimii/profiles/<name>/.
func ProfileDir(profile string) (string, error) {
	...
	if profile == "" {
		return filepath.Join(home, ".chimii"), nil
	}
	return filepath.Join(home, ".chimii", "profiles", profile), nil
}
