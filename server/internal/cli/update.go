...
func releaseAssetCandidates(targetVersion, goos, goarch string) []string {
	...
	// `chimii_{os}_{arch}` name for releases that still ship it.
	return []string{
		fmt.Sprintf("chimii-cli-%s-%s-%s.%s", version, goos, goarch, ext),
		fmt.Sprintf("chimii_%s_%s.%s", goos, goarch, ext),
	}
}
...
func fetchReleaseByTag(tag string) (*GitHubRelease, error) {
	...
	req, err := http.NewRequest(http.MethodGet, "https://api.github.com/repos/chimii-ai/chimii/releases/tags/"+tag, nil)
	if err != nil {
		return nil, err
	}
...
// FetchLatestRelease fetches the latest release tag from the chimii GitHub repo.
func FetchLatestRelease() (*GitHubRelease, error) {
	client := &http.Client{Timeout: 10 * time.Second}
	req, err := http.NewRequest(http.MethodGet, "https://api.github.com/repos/chimii-ai/chimii/releases/latest", nil)
	if err != nil {
		return nil, err
	}
	req.Header.Set("Accept", "application/vnd.github+json")
...
// IsBrewInstall checks whether the running chimii binary was installed via Homebrew.
...
// UpdateViaBrew runs `brew upgrade chimii-ai/tap/chimii`.
// Returns the combined output and any error.
func UpdateViaBrew() (string, error) {
	cmd := exec.Command("brew", "upgrade", "chimii-ai/tap/chimii")
...
func UpdateViaDownloadWithTimeout(targetVersion string, downloadTimeout time.Duration) (string, error) {
	...
	tmpFile, err := os.CreateTemp(dir, "chimii-update-*")
	if err != nil {
