...
func TestBuiltinSkillsConformToTemplate(t *testing.T) {
	...
	for _, skill := range skills {
		t.Run(skill.Name, func(t *testing.T) {
			// The chimii- prefix keeps the on-disk slug from colliding with a
			// user-authored workspace skill.
			if !strings.HasPrefix(skill.Name, "chimii-") {
				t.Errorf("skill name %q must carry the chimii- prefix", skill.Name)
			}
...
// must declare user-invocable:false and fence itself to the chimii CLI. New
// contract skills should copy this shape.
func TestMentioningSkillFollowsContractFrontmatter(t *testing.T) {
	skill, ok := findSkill(t, "chimii-mentioning")
	if !ok {
		return
	}
	...
	if got := strings.TrimSpace(fm["allowed-tools"]); got != "Bash(chimii *)" {
		t.Errorf("allowed-tools = %q, want Bash(chimii *) (fence the skill to the CLI it teaches)", got)
	}
}
...
func TestWorkingOnIssuesSkillCoversIssueLoopContracts(t *testing.T) {
	skill, ok := findSkill(t, "chimii-working-on-issues")
	if !ok {
		return
	}
	...
	if got := strings.TrimSpace(fm["allowed-tools"]); !strings.Contains(got, "Bash(chimii *)") {
		t.Errorf("allowed-tools = %q, want access to the Chimii CLI", got)
	}
	...
	mustContain := []string{
		...
		"open or update a PR before posting the final Chimii issue comment",
	...
	mustNotContain := []string{
		...
		"chimii issue get <issue-id> --output json",
		"chimii issue metadata list <issue-id> --output json",
		"chimii issue comment list <issue-id> --thread <trigger-comment-id>",
		"chimii issue comment add <issue-id> --parent <trigger-comment-id>",
	}
...
func TestSkillImportingSkillCoversWorkspaceImportContracts(t *testing.T) {
	skill, ok := findSkill(t, "chimii-skill-importing")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustNotContain := []string{
		"chimii agent skills set <agent-id> --skill-ids <skill-id>",
...
func TestCreatingAgentsSkillCoversAgentCreationContracts(t *testing.T) {
	skill, ok := findSkill(t, "chimii-creating-agents")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustContain := []string{
		...
		"chimii agent create --name <name> --runtime-id <runtime-id>",
		...
		"chimii agent skills add <agent-id> --skill-ids <skill-id> --output json",
		"chimii agent skills list <agent-id> --output json",
		"chimii agent get <agent-id> --output json",
		"255",
...
func TestSquadsSkillCoversLeaderRoutingContract(t *testing.T) {
	skill, ok := findSkill(t, "chimii-squads")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
...
func TestAutopilotsSkillCoversDispatchAndSideEffects(t *testing.T) {
	skill, ok := findSkill(t, "chimii-autopilots")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustContain := []string{
		...
		"create_issue",
		"run_only",
		"chimii autopilot trigger-add <autopilot-id> --kind schedule",
		"chimii autopilot trigger <autopilot-id> --output json",
		"Do not run `trigger`",
		"webhook tokens",
		"{{date}}",
...
func TestRuntimesAndReposSkillCoversClaimAndCheckoutChain(t *testing.T) {
	skill, ok := findSkill(t, "chimii-runtimes-and-repos")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustContain := []string{
		...
		"chimii runtime list --output json",
...
func TestProjectsAndResourcesSkillCoversDurableContext(t *testing.T) {
	skill, ok := findSkill(t, "chimii-projects-and-resources")
	if !ok {
		return
	}
	fm, body, _ := splitFrontmatter(skill.Content)
	...
	mustContain := []string{
		...
		"chimii project resource list <project-id> --output json",
		"chimii project resource add <project-id> --type github_repo --url <github-url> --output json",
		"chimii project resource add <project-id> --type github_repo --url <github-url> --ref <branch-or-sha> --output json",
		"chimii project resource add <project-id> --type local_directory",
