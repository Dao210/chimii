...
## `chimii issue pull-requests` — read PR links from Chimii

| Behavior | File:line | Drifted from |
|---|---|---|
| CLI command `pull-requests <id>` (alias `prs`) | `server/cmd/chimii/cmd_issue.go:105` | `:104` |
| `runIssuePullRequests` handler | `server/cmd/chimii/cmd_issue.go:507` | new citation |
| Calls `GET /api/issues/<id>/pull-requests` | `server/cmd/chimii/cmd_issue.go:522` | `:522` (unchanged) |
...
This is what `chimii issue pull-requests` later reads back.
...
| `--stage` on `issue create` / `issue update` | `server/cmd/chimii/cmd_issue.go:328,350` |
| `chimii issue children <id>` (sub-issues grouped by stage) | `server/cmd/chimii/cmd_issue.go:114,678`; route `GET /api/issues/{id}/children` → `ListChildIssues` |
...
comment explicitly asks for `chimii issue status <parent-id> in_review` —
...
| Behavior | File:line |
|---|---|
| `chimii issue metadata set <issue-id> --key --value [--type]` | `server/cmd/chimii/cmd_issue_metadata.go:80,109-111` |
| `chimii issue metadata delete <issue-id> --key` | `server/cmd/chimii/cmd_issue_metadata.go:93,113` |
...
| Behavior | File:line |
|---|---|
| `chimii property list/get/create/update/archive/unarchive` | `server/cmd/chimii/cmd_property.go` |
| `chimii issue property list/set/unset` (name→id translation) | `server/cmd/chimii/cmd_property.go` (`encodeIssuePropertyValue`) |
...
```bash
cd server
grep -n 'pull-requests <id>'                 cmd/chimii/cmd_issue.go
