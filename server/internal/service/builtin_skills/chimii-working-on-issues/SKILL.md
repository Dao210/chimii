---
name: chimii-working-on-issues
description: "Use when working on a Chimii issue after the runtime has provided the trigger context — to apply the product contracts the runtime brief does not encode: how PR linking differs from close intent, how to read a linked PR's real state via the pull-requests CLI, which metadata keys are high-signal, what status changes trigger on the server, and how sub-issue create status (todo vs backlog) controls whether assigned agents start immediately."
user-invocable: false
allowed-tools: Bash(chimii *), Bash(git *), Bash(gh *)
---

# Working on Chimii issues
...
For building mention links, load `chimii-mentioning` instead — not this skill.
...
This is the link that `chimii issue pull-requests` reads back — but see the
...
**excluded from `chimii issue pull-requests`** (and the issue's right-side PR
...
is to open or update a PR before posting the final Chimii issue comment, unless
...
When a step depends on PR state, query Chimii's link table — do not infer it
from branch names, GitHub search, memory, or `pr_url` metadata (which can be
stale).

```bash
chimii issue pull-requests <issue-id> --output json
```
...
```bash
chimii issue metadata set <issue-id> --key pr_url --value <url>
chimii issue metadata delete <issue-id> --key <stale-key>
```
...
- Read what exists before writing: `chimii property list` shows the catalog;
  `chimii issue property list <issue-id>` shows values set on the issue.
- Set values by property name and option name — the CLI translates to ids:

```bash
chimii issue property set <issue-id> --name Environment --value staging
chimii issue property set <issue-id> --name Platforms --value "iOS,Android"
chimii issue property unset <issue-id> --name Environment
```
...
```bash
chimii issue create --title "..." --parent <issue-id> --assignee <agent> --status todo
```

Strictly serial children — park later steps, promote one at a time:

```bash
chimii issue create --title "Step 2: ..." --parent <issue-id> --assignee <agent> --status backlog
chimii issue status <child-id> todo   # promote when the previous step is truly done
```
...
chimii issue create --title "Research A" --parent <id> --assignee <agent> --stage 1 --status todo
chimii issue create --title "Research B" --parent <id> --assignee <agent> --stage 1 --status todo
chimii issue create --title "Build"      --parent <id> --assignee <agent> --stage 2 --status backlog
chimii issue create --title "Ship"       --parent <id> --assignee <agent> --stage 3 --status backlog
```
...
```bash
chimii issue children <parent-id>             # sub-issues grouped by stage
chimii issue status <stage-2-child-id> todo   # promote when its deps are met
```
...
chimii issue create --title "Step 2" --parent <issue-id> --assignee <agent> --status todo
chimii issue create --title "Step 3" --parent <issue-id> --assignee <agent> --status todo

# correct — stage them; Stage 1 runs, later stages park and are promoted as
# each stage's barrier closes
chimii issue create --title "Step 1" --parent <issue-id> --assignee <agent> --stage 1 --status todo
chimii issue create --title "Step 2" --parent <issue-id> --assignee <agent> --stage 2 --status backlog
chimii issue create --title "Step 3" --parent <issue-id> --assignee <agent> --stage 3 --status backlog
```
