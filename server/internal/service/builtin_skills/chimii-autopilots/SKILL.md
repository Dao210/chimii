---
name: chimii-autopilots
description: "Use when creating, updating, inspecting, triggering, or debugging Chimii autopilots. Covers the full chain: schedule/webhook/manual trigger, create_issue vs run_only execution, agent/squad leader admission, runs, created issues/tasks, webhook URL rotation, and side-effect boundaries."
...
```bash
chimii autopilot list --output json
chimii autopilot get <autopilot-id> --output json
chimii autopilot runs <autopilot-id> --output json
```
...
- `create_issue` creates a Chimii issue, making the run visible as issue state.
...
chimii autopilot list --output json
chimii autopilot get <autopilot-id> --output json
chimii autopilot create --title "<title>" --description "<task prompt>" --agent <agent-name-or-id> --mode create_issue|run_only --output json
chimii autopilot update <autopilot-id> --status active|paused --output json
chimii autopilot runs <autopilot-id> --output json
chimii autopilot trigger-add <autopilot-id> --kind schedule --cron "0 9 * * *" --timezone Asia/Shanghai --output json
chimii autopilot trigger-add <autopilot-id> --kind webhook --label "ci" --output json
chimii autopilot trigger <autopilot-id> --output json
chimii autopilot trigger-rotate-url <autopilot-id> <trigger-id> --yes --output json
```
...
1. `chimii autopilot get <id> --output json` — status, mode, assignee, triggers.
2. `chimii autopilot runs <id> --output json` — run status and failure reason.
3. If assigned to a squad, inspect the squad: `chimii squad get <squad-id> --output json`; execution goes to the leader.
4. Inspect the target agent/runtime: `chimii agent get <agent-id> --output json` and `chimii runtime list --output json`.
