...
This file records source evidence for `chimii-squads/SKILL.md`.
...
```text
server/cmd/chimii/cmd_squad.go
```
...
```bash
chimii squad list
chimii squad get <squad-id>
chimii squad create
chimii squad update <squad-id>
chimii squad delete <squad-id>
chimii squad activity <issue-id> <outcome>

chimii squad member list <squad-id>
chimii squad member add <squad-id>
chimii squad member remove <squad-id>
...
- agent member roster rows list assigned workspace skills via
  ...
  "no skills assigned"; builtin chimii-* skills are excluded and human
...
- parent status is not auto-advanced by the barrier: the system comment asks the
  ...
  `chimii issue status <parent-id> in_review`. That explicit ask is what lets a
