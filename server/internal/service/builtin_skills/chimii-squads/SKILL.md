---
name: chimii-squads
description: "Use when creating, inspecting, updating, assigning, mentioning, or debugging Chimii squads. Explains what squads are, squad/member fields, CLI commands, leader routing, issue assignment, comments, mentions, autopilot behavior, leader briefing, side effects, and product-gap handling."
...
chimii squad get <squad-id> --output json
chimii squad member list <squad-id> --output json
chimii issue comment list <issue-id> --recent 10 --output json
```
...
```bash
chimii squad --help
chimii squad member --help
chimii issue update --help
...
A Chimii squad is a workspace routing and coordination object.
...
```bash
chimii squad list --output json
chimii squad get <squad-id> --output json
chimii squad create --name <name> --leader <agent-name-or-id> --output json
chimii squad update <squad-id> --instructions "<leader coordination policy>" --output json
chimii squad delete <squad-id>
```
...
```bash
chimii squad member list <squad-id> --output json
chimii squad member add <squad-id> --member-id <id> --type agent|member --role <role> --output json
chimii squad member remove <squad-id> --member-id <id> --type agent|member
chimii squad member set-role <squad-id> --member-id <id> --member-type agent|member --role <role> --output json
```

Squad leader evaluation command:

```bash
chimii squad activity <issue-id> action|no_action|failed --reason "<why>" --output json
```
...
```bash
chimii issue get <issue-id> --output json
chimii issue update <issue-id> --help
chimii issue comment list <issue-id> --output json
chimii issue comment add <issue-id> --help
```
...
For squad leader tasks, Chimii appends a squad leader briefing to the leader
...
members carry no skills segment. Builtin `chimii-*` skills are not listed —
...
- recording squad activity with `chimii squad activity`;
...
- Getting the leader briefing does NOT imply status authority. A squad
  ...
  delegation rules yes, `chimii issue status` no.
