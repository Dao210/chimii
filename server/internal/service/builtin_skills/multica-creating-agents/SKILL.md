...
```bash
chimii agent get <agent-id> --output json      # full persisted agent record
chimii agent skills list <agent-id> --output json   # current skill bindings
chimii agent env get <agent-id> --output json  # plaintext env (owner/admin only, agents denied)
```
...
`POST /api/agents` (`chimii agent create`). At task claim time the daemon
...
`chimii agent copy <source-agent-id>` forks an existing agent's portable
...
```bash
chimii agent copy <source-agent-id> --name "My Agent (copy)"   # same runtime
chimii agent copy <source-agent-id> --runtime-id <target> --model <model>  # cross-runtime fork
```
...
```bash
chimii agent create --name <name> --runtime-id <runtime-id> --custom-env-stdin --output json
chimii agent create --name <name> --runtime-id <runtime-id> --custom-env-file <0600-json> --output json
```
...
- Reading plaintext values requires the dedicated `GET /api/agents/{id}/env`
  endpoint (`chimii agent env get`). It is gated to workspace **owner/admin**
...
- Writing values after creation does NOT go through `agent update`. The generic
  ...
  `PUT /api/agents/{id}/env` (`chimii agent env set`), which is owner/admin-only
...
```bash
chimii agent create --name <name> --runtime-id <runtime-id> --mcp-config-file <0600-json> --output json
chimii agent update <agent-id> --mcp-config-stdin --output json
chimii agent update <agent-id> --mcp-config 'null'   # clears the config
```
...
- `chimii agent create` — inserts a new agent row.
- `chimii agent copy` — inserts a new agent row (a fork of an existing agent);
  the source is left untouched.
- `chimii agent skills add` / `set` — mutate bindings (`set` is destructive:
  it drops bindings not in the new list).
- `chimii agent env set` — overwrites the full `custom_env` map and writes an
  audit row.
