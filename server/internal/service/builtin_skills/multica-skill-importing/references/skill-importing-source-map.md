...
Evidence layer for `chimii-skill-importing`. Every behavioral claim in `SKILL.md`
...
grep -n "func runSkillImport"           server/cmd/chimii/cmd_skill.go
...
## CLI: `chimii skill import --url` / `--file`

| Behavior | File:line |
|---|---|
| `skill import` command def | `server/cmd/chimii/cmd_skill.go:59-63` |
| `--url` flag | `server/cmd/chimii/cmd_skill.go:143` |
| `--file` flag (local `.skill` / `.zip`; mutually exclusive with `--url`) | `server/cmd/chimii/cmd_skill.go:144` |
| `--on-conflict` flag (default `fail`) | `server/cmd/chimii/cmd_skill.go:145` |
| `--output` flag (default `json`) | `server/cmd/chimii/cmd_skill.go:146` |
| `runSkillImport` | `server/cmd/chimii/cmd_skill.go:412` |
| Requires exactly one of `--url` / `--file` | `server/cmd/chimii/cmd_skill.go:420-427` |
| `--file` reads the archive and posts multipart via `ImportSkillFile` | `server/cmd/chimii/cmd_skill.go:436-447`, client method `server/internal/cli/client.go:535` |
| `POST /api/skills/import` (URL, JSON body) | `server/cmd/chimii/cmd_skill.go:455` |
| Structured HTTP error body handling | `server/cmd/chimii/cmd_skill.go:437-440`, `handleSkillImportError` at `:454` |
| Prints structured result (`json` or table) | `server/cmd/chimii/cmd_skill.go:443`, helper at `:497` |
...
| CLI normalizes legacy `{existing_skill}` body into `status:"conflict"` | `server/cmd/chimii/cmd_skill.go:454-482`, helper at `:484` |
...
| CLI `agent skills add` def ("without replacing existing assignments") | `server/cmd/chimii/cmd_agent.go:125-130` |
| `runAgentSkillsAdd` → `POST .../skills/add` | `server/cmd/chimii/cmd_agent.go:797`; POST `:818` |
| CLI `agent skills set` def ("replaces all current assignments") | `server/cmd/chimii/cmd_agent.go:118-123` |
| `runAgentSkillsSet` → `PUT .../skills` | `server/cmd/chimii/cmd_agent.go:772`; PUT `:790` |
| CLI `agent skills list` | `server/cmd/chimii/cmd_agent.go:740`; GET `:750` |
