---
name: chimii-skill-importing
description: "Use when a user provides a skill URL, slug, or clear intent to import/install a specific skill into the current Chimii workspace. Teaches the workspace import API/CLI path (POST /api/skills/import), the supported URL source families, --on-conflict fail|overwrite|rename|skip behavior and structured import results, additive agent binding vs replace-all, and the reserved SKILL.md supporting-file rule. Do not use it to decide which skill the user needs, and never treat an external local installer like npx skills add as the final Chimii install."
user-invocable: false
allowed-tools: Bash(chimii *)
---

# Importing skills into Chimii

Use this skill when the user already provided a skill URL, slug, or a clear intent
to import a specific skill into the current Chimii workspace.
...
A skill is installed for Chimii only when it exists in the current workspace's
...
```bash
chimii skill import --url <url> --output json              # hosted source
chimii skill import --file <path-to.skill> --output json   # local archive
```
...
environment, not the Chimii workspace DB, so Chimii cannot manage or bind it.

## Supported URL source families

`detectImportSource` accepts these hosts (and `www.` variants). Pass any of these
forms to `chimii skill import --url <url> --output json`:

```bash
chimii skill import --url clawhub.ai/owner/skill --output json
chimii skill import --url skills.sh/owner/repo/skill --output json
chimii skill import --url github.com/owner/repo --output json
chimii skill import --url github.com/owner/repo/tree/main/path/to/skill --output json
chimii skill import --url github.com/owner/repo/blob/main/path/to/SKILL.md --output json
```
...
`chimii skill import --file <path> --output json` imports a skill from a local
...
```bash
chimii skill import --url <url> --output json
```
...
```bash
chimii agent skills add <agent-id> --skill-ids <skill-id> --output json
chimii agent skills list <agent-id> --output json
```

After the final `chimii agent skills list <agent-id> --output json`, verify the
target skill id is present before claiming the skill is available to that agent.

## Additive add vs replace-all set

`chimii agent skills add` is additive: the server inserts the assignments without
clearing existing ones (`AddAgentSkills`).

`chimii agent skills set` is replace-all: the server clears every current
...
Default behavior is safe: `chimii skill import --url <url>` is equivalent to
...
chimii skill import --url https://skills.sh/acme/repo/review-helper --output json

# Replace the existing same-name skill, preserving its ID and agent bindings.
chimii skill import --url https://skills.sh/acme/repo/review-helper --on-conflict overwrite --output json

# Keep the existing skill and import a copy such as review-helper-2.
chimii skill import --url https://skills.sh/acme/repo/review-helper --on-conflict rename --output json

# Batch-friendly behavior: leave the existing skill alone and mark it skipped.
chimii skill import --url https://skills.sh/acme/repo/review-helper --on-conflict skip --output json
```
...
```bash
chimii skill get <skill-id> --output json
```
...
Incorrect (bypasses Chimii):

```bash
npx skills add https://skills.sh/owner/repo/skill
```

The skill may exist locally, but Chimii cannot manage it as a workspace skill.
...
```bash
chimii skill import --url https://skills.sh/owner/repo/skill --output json
```
