...
- `server/cmd/chimii/cmd_runtime.go` registers `runtime list`, `usage`, `activity`, `update`, and `delete`.
...
- `server/cmd/chimii/cmd_repo.go` registers `repo checkout <url> [--ref]`.
- `repo checkout` requires `CHIMII_DAEMON_PORT`, sends `workspace_id`, `workdir`, `ref`, `agent_name`, `task_id`, and the daemon-managed optional `checkout_mode` to local daemon `/repo/checkout`, then prints the checked-out path.
...
- `server/internal/daemon/daemon.go` injects `CHIMII_REPO_CHECKOUT_MODE=isolated` only for Linux Codex tasks. `server/internal/daemon/repocache/cache.go` implements that mode as a same-filesystem local clone with task-local Git metadata and the real repository as `origin`; other runtimes keep the linked-worktree path.
