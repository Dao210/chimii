...
This skill states WHAT a mention link does in the Chimii backend, traced to
...
- a person → `chimii workspace member list --output json` → use `user_id`
- an agent → `chimii agent list --output json` → use `id`
- a squad  → `chimii squad list --output json` → use `id`
...
- **A private agent you cannot access:** skipped — the mention path gates on
  ...
  one; the child-done wake is ungated — see the chimii-squads skill).
...
Correct:
  1. `chimii workspace member list --output json`  → Alice's `user_id` = 7f3a…
