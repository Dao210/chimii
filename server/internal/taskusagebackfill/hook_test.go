...
func TestHook_DirectV034Upgrade(t *testing.T) {
	...
	if adminURL == "" {
		adminURL = "postgres://chimii:chimii@localhost:5432/chimii?sslmode=disable"
	}
	...
	tmpDB := fmt.Sprintf("chimii_v034_upgrade_%d", time.Now().UnixNano())
...
func TestHook_FreshDatabaseStampsWatermarkOnly(t *testing.T) {
	...
	tmpDB := fmt.Sprintf("chimii_v034_fresh_%d", time.Now().UnixNano())
	if err := createDatabase(ctx, adminURL, tmpDB); err != nil {
