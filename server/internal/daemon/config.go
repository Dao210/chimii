...
import (
	...
	"github.com/chimii-ai/chimii/server/internal/cli"
)
...
const (
	...
	// hard ceiling for cost/resource control can set CHIMII_AGENT_TIMEOUT.
	...
	// Set CHIMII_AGENT_IDLE_WATCHDOG=0 to disable.
	...
	// that stuck-tool case (MUL-3064). Set CHIMII_AGENT_TOOL_WATCHDOG=0 to
...
// CHIMII_GC_ARTIFACT_PATTERNS to extend the list per deployment.
...
type Config struct {
	...
	DeviceName                     string
	RuntimeName                    string
	CLIVersion                     string                // chimii CLI version (e.g. "0.1.13")
	...
	WorkspacesRoot                 string                // base path for execution envs (default: ~/chimii_workspaces)
	...
	GCCodexSessionTTL              time.Duration         // reclaim a per-issue Codex session store (~/.codex/chimii-sessions/<agent>/<issue>) untouched for at least this long, so a done/abandoned issue's conversation history does not accumulate forever (default: 14d, set 0 to disable)
	AutoUpdateEnabled              bool                  // periodically check for a newer CLI release and self-update when idle (default: true on Chimii Cloud, false on self-host)
	...
	// written by `chimii runtime profile set-path`. appendProfileRuntimes
...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	rawServerURL := envOrDefault("CHIMII_SERVER_URL", DefaultServerURL)
	...
	//
	//   - CHIMII_OPENCLAW_PATH: read by probe() via envOrDefault for the
	...
	// users already get with CHIMII_OPENCLAW_PATH today. We achieve it with
	...
	// pinned to an explicit CHIMII_*_PATH) must not pay the cost of
	...
	probe := func(envVar, defaultCmd, modelEnv string) (AgentEntry, bool) {
		...
		// who pinned CHIMII_*_PATH to an absolute or relative path that
	...
	if e, ok := probe("CHIMII_CLAUDE_PATH", "claude", "CHIMII_CLAUDE_MODEL"); ok {
		agents["claude"] = e
	}
	if e, ok := probe("CHIMII_CODEX_PATH", "codex", "CHIMII_CODEX_MODEL"); ok {
		agents["codex"] = e
	}
	if e, ok := probe("CHIMII_OPENCODE_PATH", "opencode", "CHIMII_OPENCODE_MODEL"); ok {
		agents["opencode"] = e
	}
	if e, ok := probe("CHIMII_DEVECO_PATH", "deveco", "CHIMII_DEVECO_MODEL"); ok {
		agents["deveco"] = e
	}
	if e, ok := probe("CHIMII_OPENCLAW_PATH", "openclaw", "CHIMII_OPENCLAW_MODEL"); ok {
		agents["openclaw"] = e
	}
	if e, ok := probe("CHIMII_HERMES_PATH", "hermes", "CHIMII_HERMES_MODEL"); ok {
		agents["hermes"] = e
	}
	if e, ok := probe("CHIMII_PI_PATH", "pi", "CHIMII_PI_MODEL"); ok {
		agents["pi"] = e
	}
	if e, ok := probe("CHIMII_CURSOR_PATH", "cursor-agent", "CHIMII_CURSOR_MODEL"); ok {
		agents["cursor"] = e
	}
	if e, ok := probe("CHIMII_COPILOT_PATH", "copilot", "CHIMII_COPILOT_MODEL"); ok {
		agents["copilot"] = e
	}
	if e, ok := probe("CHIMII_KIMI_PATH", "kimi", "CHIMII_KIMI_MODEL"); ok {
		agents["kimi"] = e
	}
	if e, ok := probe("CHIMII_KIRO_PATH", "kiro-cli", "CHIMII_KIRO_MODEL"); ok {
		agents["kiro"] = e
	}
	if e, ok := probe("CHIMII_CODEBUDDY_PATH", "codebuddy", "CHIMII_CODEBUDDY_MODEL"); ok {
		agents["codebuddy"] = e
	}
	// agy 1.0.6 added a `--model` flag (MUL-3125), so Antigravity now takes a
	// model env like every other backend. CHIMII_ANTIGRAVITY_MODEL seeds the
	...
	if e, ok := probe("CHIMII_ANTIGRAVITY_PATH", "agy", "CHIMII_ANTIGRAVITY_MODEL"); ok {
		agents["antigravity"] = e
	}
	qoderPath := envOrDefault("CHIMII_QODER_PATH", "qodercli")
	if path, err := resolveAgentExecutablePath(qoderPath); err == nil {
		agents["qoder"] = AgentEntry{
			Path:    path,
			Command: qoderPath,
			Model:   strings.TrimSpace(os.Getenv("CHIMII_QODER_MODEL")),
		}
	}
	// ByteDance official TRAE CLI (the `traecli` binary from https://docs.trae.cn/cli),
	// driven over ACP via `traecli acp serve --yolo`. CHIMII_TRAECLI_MODEL seeds
	// the daemon-wide default model (a model id from the user's logged-in traecli
	// catalog).
	if e, ok := probe("CHIMII_TRAECLI_PATH", "traecli", "CHIMII_TRAECLI_MODEL"); ok {
		agents["traecli"] = e
	}
	// xAI Grok Build CLI (`grok`), driven over ACP via
	// `grok agent --always-approve stdio`. CHIMII_GROK_MODEL seeds the
	// daemon-wide default (e.g. grok-4.5).
	if e, ok := probe("CHIMII_GROK_PATH", "grok", "CHIMII_GROK_MODEL"); ok {
		agents["grok"] = e
	}
	...
	if e, ok := probe("CHIMII_QWEN_PATH", "qwen", "CHIMII_QWEN_MODEL"); ok {
		agents["qwen"] = e
	}
	...
	claudeArgs, err := shellArgsFromEnv("CHIMII_CLAUDE_ARGS")
	if err != nil {
		return Config{}, err
	}
	codexArgs, err := shellArgsFromEnv("CHIMII_CODEX_ARGS")
	if err != nil {
		return Config{}, err
	}
	codebuddyArgs, err := shellArgsFromEnv("CHIMII_CODEBUDDY_ARGS")
	if err != nil {
		return Config{}, err
	}
	qwenArgs, err := shellArgsFromEnv("CHIMII_QWEN_ARGS")
	if err != nil {
		return Config{}, err
	}
	...
	pollInterval, err := durationFromEnv("CHIMII_DAEMON_POLL_INTERVAL", DefaultPollInterval)
	if err != nil {
		return Config{}, err
	}
	...
	heartbeatInterval, err := durationFromEnv("CHIMII_DAEMON_HEARTBEAT_INTERVAL", DefaultHeartbeatInterval)
	if err != nil {
		return Config{}, err
	}
	...
	agentTimeout, err := durationFromEnv("CHIMII_AGENT_TIMEOUT", DefaultAgentTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	codexSemanticInactivityTimeout, err := durationFromEnv("CHIMII_CODEX_SEMANTIC_INACTIVITY_TIMEOUT", DefaultCodexSemanticInactivityTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	codexHandshakeTimeout, err := durationFromEnv("CHIMII_CODEX_HANDSHAKE_TIMEOUT", DefaultCodexHandshakeTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	// CHIMII_AGENT_IDLE_WATCHDOG=0 disables the per-task idle watchdog. We
	...
	agentIdleWatchdog, err := durationFromEnv("CHIMII_AGENT_IDLE_WATCHDOG", DefaultAgentIdleWatchdog)
	if err != nil {
		return Config{}, err
	}
	// CHIMII_OPENCODE_IDLE_WATCHDOG narrows the no-message window for
	// OpenCode's streamed model responses. Zero removes the provider-specific
	// override and falls back to CHIMII_AGENT_IDLE_WATCHDOG; positive values
	// cannot extend the global bound, and the global zero still disables the
	// whole mechanism.
	openCodeIdleWatchdog, err := durationFromEnv("CHIMII_OPENCODE_IDLE_WATCHDOG", DefaultOpenCodeIdleWatchdog)
	if err != nil {
		return Config{}, err
	}

	// CHIMII_AGENT_TOOL_WATCHDOG=0 disables the in-flight-tool backstop; any
	// positive duration overrides DefaultAgentToolWatchdog.
	agentToolWatchdog, err := durationFromEnv("CHIMII_AGENT_TOOL_WATCHDOG", DefaultAgentToolWatchdog)
	if err != nil {
		return Config{}, err
	}

	maxConcurrentTasks, err := intFromEnv("CHIMII_DAEMON_MAX_CONCURRENT_TASKS", DefaultMaxConcurrentTasks)
	if err != nil {
		return Config{}, err
	}
	...
	// Callers may still pin a specific id via CHIMII_DAEMON_ID or the
	// override field (e.g. for tests or embedded environments).
	daemonID := strings.TrimSpace(os.Getenv("CHIMII_DAEMON_ID"))
	...
	// — e.g. ~/.chimii/daemon.id (default) plus ~/.chimii/profiles/<x>/
	...
	// the user explicitly pins CHIMII_DAEMON_ID=<hostname>, or when the
	// canonical id was itself promoted from a pre-change profile file).
	legacyDaemonIDs = filterLegacyIDs(legacyDaemonIDs, daemonID)

	deviceName := envOrDefault("CHIMII_DAEMON_DEVICE_NAME", host)
	if overrides.DeviceName != "" {
		deviceName = overrides.DeviceName
	}

	runtimeName := envOrDefault("CHIMII_AGENT_RUNTIME_NAME", DefaultRuntimeName)
	if overrides.RuntimeName != "" {
		runtimeName = overrides.RuntimeName
	}

	// Workspaces root: override > env > default (~/chimii_workspaces or ~/chimii_workspaces_<profile>)
	...
	keepEnv := os.Getenv("CHIMII_KEEP_ENV_AFTER_TASK") == "true" || os.Getenv("CHIMII_KEEP_ENV_AFTER_TASK") == "1"

	// GC config: env > defaults
	gcEnabled := true
	if v := os.Getenv("CHIMII_GC_ENABLED"); v == "false" || v == "0" {
		gcEnabled = false
	}
	gcInterval, err := durationFromEnv("CHIMII_GC_INTERVAL", DefaultGCInterval)
	if err != nil {
		return Config{}, err
	}
	gcTTL, err := durationFromEnv("CHIMII_GC_TTL", DefaultGCTTL)
	if err != nil {
		return Config{}, err
	}
	gcOrphanTTL, err := durationFromEnv("CHIMII_GC_ORPHAN_TTL", DefaultGCOrphanTTL)
	if err != nil {
		return Config{}, err
	}
	gcArtifactTTL, err := durationFromEnv("CHIMII_GC_ARTIFACT_TTL", DefaultGCArtifactTTL)
	if err != nil {
		return Config{}, err
	}
	gcCodexSessionTTL, err := durationFromEnv("CHIMII_GC_CODEX_SESSION_TTL", DefaultGCCodexSessionTTL)
	if err != nil {
		return Config{}, err
	}
	gcArtifactPatterns := patternsFromEnv("CHIMII_GC_ARTIFACT_PATTERNS", DefaultGCArtifactPatterns)

	// Auto-update config: default -> env override -> CLI override.
	//
	// Default is opt-in on Chimii Cloud (api.chimii.ai) and opt-out for
	...
	// Operators on either side can flip the default with CHIMII_DAEMON_AUTO_UPDATE.
	autoUpdateEnabled := isOfficialCloudServer(serverBaseURL)
	if v := strings.TrimSpace(os.Getenv("CHIMII_DAEMON_AUTO_UPDATE")); v != "" {
	...
	autoUpdateInterval, err := durationFromEnv("CHIMII_DAEMON_AUTO_UPDATE_INTERVAL", DefaultAutoUpdateCheckInterval)
	if err != nil {
		return Config{}, err
	}
...
// officialCloudHost is the hostname of Chimii's hosted cloud. It's the only
// origin we treat as "official" for the auto-update default — staging,
// preview, and any future *.chimii.ai subdomains are deliberately excluded
// so they inherit the safer self-host default until explicitly opted in.
const officialCloudHost = "api.chimii.ai"

// isOfficialCloudServer reports whether the resolved server base URL points
// at Chimii's hosted cloud. Used to pick the auto-update default: cloud
...
func NormalizeServerBaseURL(raw string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("invalid CHIMII_SERVER_URL: %w", err)
	}
	switch u.Scheme {
	case "ws":
		u.Scheme = "http"
	...
	case "http", "https":
	default:
		return "", fmt.Errorf("CHIMII_SERVER_URL must use ws, wss, http, or https")
	}
	if u.Path == "/ws" {
		u.Path = ""
	}
...
// CHIMII_WORKSPACES_ROOT env > default ($HOME/chimii_workspaces, or
// $HOME/chimii_workspaces_<profile> for a named profile). Read-only callers
// (e.g. `chimii daemon disk-usage`) use this directly so they pick the same
// directory the running daemon would have picked.
func ResolveWorkspacesRoot(profile, override string) (string, error) {
	root := strings.TrimSpace(os.Getenv("CHIMII_WORKSPACES_ROOT"))
	if override != "" {
		root = override
	}
	if root == "" {
		...
		if err != nil {
			return "", fmt.Errorf("resolve home directory: %w (set CHIMII_WORKSPACES_ROOT to override)", err)
		}
		if profile != "" {
			root = filepath.Join(home, "chimii_workspaces_"+profile)
		} else {
			root = filepath.Join(home, "chimii_workspaces")
		}
	}
...
func ArtifactPatternsFromEnv() []string {
	return patternsFromEnv("CHIMII_GC_ARTIFACT_PATTERNS", DefaultGCArtifactPatterns)
}
...
// When ~/.chimii/hooks shadows a real agent binary, skip that hooks directory:
...
func resolveAgentExecutablePath(cmd string) (string, error) {
	...
	if isInChimiiHooksDir(resolved) {
		if unshadowed, err := lookPathExcludingChimiiHooks(cmd); err == nil {
...
// probe() order in LoadConfig: exec.LookPath (with the ~/.chimii/hooks
...
func reresolveAgentCommand(cmd string) (string, bool) {
	...
	// this — an operator-pinned CHIMII_*_PATH that no longer exists should
...
func lookPathExcludingChimiiHooks(cmd string) (string, error) {
	for _, dir := range filepath.SplitList(os.Getenv("PATH")) {
		if dir == "" {
			dir = "."
		}
		if isChimiiHooksDir(dir) {
			continue
		}
...
func isInChimiiHooksDir(path string) bool {
	if path == "" {
		return false
	}
	return isChimiiHooksDir(filepath.Dir(path))
}

func isChimiiHooksDir(dir string) bool {
	home, err := os.UserHomeDir()
	if err != nil || home == "" {
		return false
	}
	return samePathDir(dir, filepath.Join(home, ".chimii", "hooks"))
}
...
// before any CHIMII_*_PATH override is applied. Kept in sync with the
...
//     keep the script readable. Custom CHIMII_*_PATH values never reach this
...
//  6. if the resolved path lives in ~/.chimii/hooks, searches the same
...
func buildLoginShellResolveScript(names []string) string {
	...
	b.WriteString("  hc=\"\"\n")
	b.WriteString("  if [ -n \"${HOME:-}\" ]; then hd=\"$HOME/.chimii/hooks\"; hc=$(cd \"$hd\" 2>/dev/null && pwd -P) || hc=\"\"; fi\n")
...
//
//   - The two vars touched (CHIMII_OPENCLAW_PATH, OPENCLAW_STATE_DIR) are
...
func applyOpenclawOverride(oc *cli.OpenClawOverride) {
	...
	if oc.BinaryPath != "" {
		if _, set := os.LookupEnv("CHIMII_OPENCLAW_PATH"); !set {
			_ = os.Setenv("CHIMII_OPENCLAW_PATH", oc.BinaryPath)
		}
	}
	if oc.StateDir != "" {
