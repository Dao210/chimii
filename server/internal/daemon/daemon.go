...
import (
	...
	"github.com/chimii-ai/chimii/server/internal/daemon/repocache"
	"github.com/chimii-ai/chimii/server/internal/selfexec"
	"github.com/chimii-ai/chimii/server/pkg/agent"
	"github.com/chimii-ai/chimii/server/pkg/skillbundle"
...
const (
	...
	repoCheckoutModeEnv      = "CHIMII_REPO_CHECKOUT_MODE"
...
//     (preserving the ~/.chimii/hooks exclusion and the login-shell fallback).
...
func (d *Daemon) Run(ctx context.Context) error {
	...
	// fault can strip every CHIMII_* env var from an agent subprocess; the
...
func (d *Daemon) resolveAuth() error {
	...
	if cfg.Token == "" {
		...
		if d.cfg.Profile != "" {
			loginHint = fmt.Sprintf("'chimii login --profile %s'", d.cfg.Profile)
		}
...
func (d *Daemon) appendProfileRuntimes(ctx context.Context, workspaceID string, runtimes *[]map[string]string, failedProfiles *[]map[string]string) string {
	...
	for _, profile := range resp.RuntimeProfiles {
		...
		// path override (MUL-3284, `chimii runtime profile set-path`) wins
...
// `chimii repo checkout` accepts project-only URLs without an extra round
...
// other API call is what surfaces a user-actionable "run chimii login"
...
func (d *Daemon) handleUpdate(ctx context.Context, runtimeID string, update *PendingUpdate) {
	...
	if d.cfg.LaunchedBy == "desktop" {
		...
		d.reportUpdateResult(ctx, runtimeID, update.ID, map[string]any{
			"status": "failed",
			"error":  "CLI is managed by Chimii Desktop — update the Desktop app to upgrade the CLI",
		})
		return
	}
...
// For brew installs, it keeps the symlink path (e.g. /opt/homebrew/bin/chimii)
...
func (d *Daemon) triggerRestart() {
	...
	// must use the stable <brew-prefix>/bin/chimii symlink instead.
	if isBrewInstall() {
		if brewPrefix := getBrewPrefix(); brewPrefix != "" {
			newBin = filepath.Join(brewPrefix, "bin", "chimii")
		} else if prefix := matchKnownBrewPrefix(newBin); prefix != "" {
			newBin = filepath.Join(prefix, "bin", "chimii")
		} else {
...
// release. Used by pollLoop to expose CHIMII_TASK_SLOT to spawned tasks.
...
func (d *Daemon) runTask(ctx context.Context, task Task, provider string, slot int, taskLog *slog.Logger) (taskResult TaskResult, returnErr error) {
	...
	// here would make CHIMII_WORKSPACE_ID empty in the agent env, and the
	...
	// `chimii repo checkout` would reject project-only URLs that aren't also
	...
	// /chimii_workspaces/{ws}/{short-id}/workdir hit FileNotFoundError in
	...
	// up stale Chimii instructions (issue id, trigger comment id, reply
	...
	if env.LocalDirectory {
		defer func() {
			...
			// Excise the sidecar tree (.agent_context/, .chimii/,
	...
	// can call the Chimii API and the local daemon (e.g. `chimii repo checkout`).
	// CHIMII_TASK_SLOT is allocated from the daemon-wide concurrency pool, not
	// per-agent. When one daemon hosts multiple agents, slots index shared
	// daemon-level resources such as GPUs.
	// CHIMII_TOKEN is bound to (agent, task) by the server. Never fall back
	...
	agentEnv := map[string]string{
		"CHIMII_TOKEN":        agentToken,
		"CHIMII_SERVER_URL":   d.cfg.ServerBaseURL,
		"CHIMII_DAEMON_PORT":  fmt.Sprintf("%d", d.cfg.HealthPort),
		"CHIMII_WORKSPACE_ID": task.WorkspaceID,
		"CHIMII_AGENT_NAME":   agentName,
		"CHIMII_AGENT_ID":     task.AgentID,
		"CHIMII_TASK_ID":      task.ID,
		"CHIMII_TASK_SLOT":    strconv.Itoa(slot),
		"TMPDIR":              taskTempDir,
		"TMP":                 taskTempDir,
		"TEMP":                taskTempDir,
	}
	...
	if task.AutopilotRunID != "" {
		agentEnv["CHIMII_AUTOPILOT_RUN_ID"] = task.AutopilotRunID
	}
	if task.AutopilotID != "" {
		agentEnv["CHIMII_AUTOPILOT_ID"] = task.AutopilotID
	}
	// Quick-create marker — when set, the chimii CLI's `issue create`
	...
	if task.QuickCreatePrompt != "" {
		agentEnv["CHIMII_QUICK_CREATE_TASK_ID"] = task.ID
		if len(task.QuickCreateAttachmentIDs) > 0 {
			if raw, err := json.Marshal(task.QuickCreateAttachmentIDs); err == nil {
				agentEnv["CHIMII_QUICK_CREATE_ATTACHMENT_IDS"] = string(raw)
			} else {
				taskLog.Warn("quick-create attachment ids: marshal failed; skipping env injection", "error", err)
			}
		}
	}
	// Ensure the chimii CLI is on PATH inside the agent's environment.
	// Some runtimes (e.g. Codex) run in an isolated sandbox that may not
	// inherit the daemon's PATH. Prepend the directory of the running
	// chimii binary so that `chimii` commands in the agent always resolve.
	...
	// then the daemon-wide CHIMII_<PROVIDER>_MODEL env var. If
	...
	// `chimii issue status` / `chimii issue comment add`, leaving issues
	// stuck in `todo`.
	//
...
func (d *Daemon) executeAndDrain(ctx context.Context, backend agent.Backend, prompt string, opts agent.ExecOptions, taskLog *slog.Logger, taskID, codexHome string, msgSeq *atomic.Int32) (agent.Result, int32, error) {
	...
	// zero remains authoritative so CHIMII_AGENT_IDLE_WATCHDOG=0 still disables
...
func ensureTaskTempDir(envRoot string, workspaceID string, taskID string) (string, error) {
	...
	dir, err := os.MkdirTemp(socketSafeTempBaseDir(), "chimii-task-")
	if err != nil {
		return "", err
	}
...
func isBlockedEnvKey(key string) bool {
	...
	if strings.HasPrefix(upper, "CHIMII_") {
		return true
	}
	switch upper {
