...
func TestTryRenewToken_SurfacesReloginWarningOn401(t *testing.T) {
	...
	if !strings.Contains(out, "chimii login") {
		t.Fatalf("401 warning must tell the user to run 'chimii login', got: %s", out)
	}
}
...
// and the operator would never see a "run chimii login" hint.
func TestPreflightAuth_RenewsBeforeWorkspaceSyncOnExpiredToken(t *testing.T) {
	...
	if !strings.Contains(out, "chimii login") {
		t.Fatalf("expected the actionable 'run chimii login' hint in the WARN, got: %s", out)
	}
}
