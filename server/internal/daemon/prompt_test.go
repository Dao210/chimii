...
func TestBuildQuickCreatePromptRules(t *testing.T) {
	...
	mustContain := []string{
		...
		// auto-subscribe path fires (chimii issue create has no --subscriber flag)
		"CC exception",
		...
		// fixed prefix.
		"chimii issue create --output json",
		"JSON response",
		"identifier",
...
func TestBuildChatPromptChannelAwareness(t *testing.T) {
	t.Run("slack-backed prompt teaches both read commands", func(t *testing.T) {
		...
		for _, want := range []string{"Slack", "NOT in Chimii", "chimii chat history", "chimii chat thread", "Do NOT narrate"} {
			if !strings.Contains(out, want) {
	...
	t.Run("top-level mention starts with history", func(t *testing.T) {
		...
		if !strings.Contains(out, "top level: start with `chimii chat history`") {
	...
	t.Run("in-thread mention starts with thread", func(t *testing.T) {
		...
		if !strings.Contains(out, "inside a thread: start with `chimii chat thread`") {
	...
	t.Run("web-only session has no channel block", func(t *testing.T) {
		...
		if strings.Contains(out, "chimii chat history") {
...
//     Any IM reply leaves Chimii, where the upload has nothing to bind to.
...
func TestBuildChatPromptTwoLayerChannelPolicy(t *testing.T) {
	...
	// `chimii attachment upload` on purpose — to state that it does not apply
	...
	const uploadGuidance = "run `chimii attachment upload <local-path>`"
	const historyGuidance = "chimii chat history"
...
func TestBuildChatPromptFeishuIgnoresChatInThread(t *testing.T) {
	...
	for _, unwanted := range []string{"chimii chat thread", "chimii chat history"} {
...
func TestBuildPromptDefaultMentionsRecent(t *testing.T) {
	...
	for _, s := range []string{
		"chimii issue comment list issue-default-1 --recent 10 --output json",
		"Next thread cursor:",
		"--since",
	} {
		if !strings.Contains(out, s) {
	...
	if strings.Contains(out, "chimii issue comment list issue-default-1 --output json") {
...
func TestBuildPromptNewCommentsHint(t *testing.T) {
	...
	if !strings.Contains(out, "chimii issue comment list "+issueID+" --thread thread-root-1 --since "+since+" --output json") {
	...
	if !strings.Contains(out, "chimii issue comment list "+issueID+" --since "+since+" --output json") {
...
func TestBuildPromptColdStartThreadRead(t *testing.T) {
	...
	if !strings.Contains(out, "chimii issue comment list "+issueID+" --thread thread-root-1 --tail 30 --output json") {
		t.Errorf("cold start must point at the triggering thread read, got:\n%s", out)
	}
	if !strings.Contains(out, "chimii issue comment list "+issueID+" --recent 10 --output json") {
