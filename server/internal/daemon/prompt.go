...
import (
	...
	"github.com/chimii-ai/chimii/server/internal/daemon/execenv"
)
...
func buildPromptBody(task Task, provider string) string {
	...
	var b strings.Builder
	b.WriteString("You are running as a local coding agent for a Chimii workspace.\n\n")
	...
	fmt.Fprintf(&b, "Start by running `chimii issue get %s --output json` to understand your task, then complete it.\n", task.IssueID)
	fmt.Fprintf(&b, "For comment history, follow the rule in your runtime workflow file (assignment-triggered tasks treat the read as mandatory). Start with `chimii issue comment list %s --recent 10 --output json` to read the 10 most recently active threads, then page older threads via the stderr `Next thread cursor: ...` line and the matching `--before` / `--before-id` until you have enough history. Resolved threads come back folded — `--full` to expand. `--since <RFC3339>` is still available for incremental polling and may combine with `--recent`.\n", task.IssueID)
	return b.String()
}

// buildQuickCreatePrompt constructs a prompt for quick-create tasks. The
// user typed a single natural-language sentence in the create-issue modal;
// the agent's job is to translate it into one `chimii issue create` CLI
...
// call `chimii issue get` or attempt to comment — there's nothing to read
// or reply to.
func buildQuickCreatePrompt(task Task) string {
	var b strings.Builder
	b.WriteString("You are running as a quick-create assistant for a Chimii workspace.\n\n")
	b.WriteString("A user captured the following input via the quick-create modal. There is NO existing issue. Your job is to create a well-formed issue from this input with a single `chimii issue create` command.\n\n")
	...
	b.WriteString("    - When the user names someone (\"assign to X\" / \"@X\"), call `chimii workspace member list --output json`, `chimii agent list --output json`, and `chimii squad list --output json` and find the matching entity by display name. Squads are first-class assignees too — a squad name (e.g. \"Super Human\") routes work to the squad leader, who then delegates. On a clean unambiguous match, prefer `--assignee-id <uuid>` using the `user_id` (member) or `id` (agent or squad) from that JSON — UUID matching is exact and robust to name collisions in workspaces with overlapping names. `--assignee <name>` (fuzzy) is acceptable as a fallback when names are unambiguous. On no match or ambiguous match, do NOT pass either flag — instead append a final line to the description: `Unrecognized assignee: X`.\n")
	...
	b.WriteString("- Run exactly one `chimii issue create --output json` invocation. Do not retry for any reason — even on non-zero exit. The issue may already exist; another attempt would create a duplicate.\n")
	...
	b.WriteString("- Do NOT call `chimii issue get` or `chimii issue comment add` — there is no issue to query or comment on.\n")
...
func buildCommentPrompt(task Task, provider string) string {
	...
	if task.TriggerCommentContent != "" {
		...
		if len(task.CoalescedComments) > 0 {
			...
			fmt.Fprintf(&b, "\nIf you need the surrounding discussion for any of them, fetch its thread with `chimii issue comment list %s --thread <thread-id> --tail 30 --output json` using the thread id shown above.\n\n", task.IssueID)
		} else if len(task.CoalescedCommentIDs) > 0 {
			fmt.Fprintf(&b, "This run also covers %d earlier comment(s) posted before it started — you must read and address them too, not just the one above: %s. These may be in DIFFERENT threads, so do not assume they share the triggering thread; fetch each by pulling the issue-wide discussion with `chimii issue comment list %s --recent 30 --output json` (expand with `--full` if a thread is folded) and locate the ids above.\n\n",
		...
		if task.Agent != nil && strings.Contains(task.Agent.Instructions, "## Squad Operating Protocol") {
			fmt.Fprintf(&b, "⚠️ **Squad leader no_action rule:** If you decide no action is needed, call `chimii squad activity %s no_action --reason \"...\"` and EXIT. DO NOT post any comment — not even one that says \"no action needed\" or \"exiting silently\". The squad activity call records your decision; a comment is redundant noise.\n\n", task.IssueID)
		}
	}
	fmt.Fprintf(&b, "Start by running `chimii issue get %s --output json` to understand your task, then decide how to proceed.\n\n", task.IssueID)
	...
	if hint := execenv.BuildNewCommentsHint(task.IssueID, task.TriggerCommentID, task.TriggerThreadID, task.NewCommentsSince, task.NewCommentCount); hint != "" {
		...
		fmt.Fprintf(&b, "Read the discussion: `chimii issue comment list %s --recent 10 --output json` (resolved threads come back folded — `--full` to expand).\n\n", task.IssueID)
	}
...
func buildChatPrompt(task Task) string {
	...
	if task.ChatIntro {
		var b strings.Builder
		b.WriteString("You are running as a chat assistant for a Chimii workspace.\n")
	...
	// like "what did you just talk about" sends it to read Chimii instead of the
	// channel conversation. A web-only chat session gets no such block — its
	// history is the Chimii chat_session the agent already resumes.
	//
	// The history half is Slack-only, and that is a real server constraint, not a
	// simplification: `chimii chat history` / `chimii chat thread` are served by
	...
	if task.ChatChannelType != "" {
		...
		fmt.Fprintf(&b, "You are operating inside a %s conversation — not the Chimii web app. This conversation and its history live in %s, NOT in Chimii; never look in Chimii issues or comments for it.\n", platform, platform)
		if task.ChatChannelType == execenv.ChannelTypeSlack {
			...
			b.WriteString("- `chimii chat history --output json` — the channel overview: recent top-level messages, each thread tagged with a `thread_id` and `reply_count`. It does NOT expand thread contents.\n")
			b.WriteString("- `chimii chat thread [<thread_id>] --output json` — read one thread's messages; omit the id to read the thread you are in, or pass a `thread_id` from the overview to read a specific thread.\n")
			if task.ChatInThread {
				b.WriteString("You were @mentioned inside a thread: start with `chimii chat thread` to read it; if you need the wider channel, run `chimii chat history` and open a specific thread with `chimii chat thread <thread_id>`.\n")
			} else {
				b.WriteString("You were @mentioned at the channel top level: start with `chimii chat history` to see the channel, then read a specific thread's contents with `chimii chat thread <thread_id>`.\n")
			}
			...
			fmt.Fprintf(&b, "Work from the context already provided to you below — Chimii has no history reader for %s, so there is no command that can fetch more of this conversation. If you genuinely need earlier context that is not here, ask the user for it rather than guessing.\n", platform)
		}
	...
	// have expired. `chimii attachment download <id>` re-signs at click
	...
	if len(task.ChatMessageAttachments) > 0 {
		...
		b.WriteString("Use `chimii attachment download <id>` to fetch each file locally before referring to it.\n")
		b.WriteString("When creating an issue that should preserve one of these attachments, pass `--attachment-id <id>` to `chimii issue create` in addition to keeping the attachment markdown inline.\n")
	}
	...
	// that platform, not the Chimii chat UI, so this binding does not apply.
	...
	if task.ChatChannelType == "" {
		b.WriteString("\nTo include a file or image you produced in your reply, run `chimii attachment upload <local-path>`. The file binds to your reply automatically and appears as an attachment card below it even if you paste nothing. The command also returns a `markdown` snippet you may paste on its own line to place the item where you want it (files render as a card, images inline).\n")
	} else {
		fmt.Fprintf(&b, "\nThis reply is delivered to %s as text. You cannot attach a file to it: `chimii attachment upload` binds to a Chimii chat reply, which this is not. If you produce a file, describe it in words — never write its local path as a link, and never upload it and then write as though it arrived.\n", channelDisplayName(task.ChatChannelType))
	}
	return b.String()
}
...
func buildAutopilotPrompt(task Task) string {
	...
	if task.AutopilotID != "" {
		fmt.Fprintf(&b, "Start by running `chimii autopilot get %s --output json` if you need the full autopilot configuration, then complete the instructions above.\n", task.AutopilotID)
	} else {
		b.WriteString("Complete the instructions above.\n")
	}
	b.WriteString("Do not run `chimii issue get`; this run does not have an issue ID.\n")
	return b.String()
}
