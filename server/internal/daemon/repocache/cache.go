...
type Cache struct {
	root   string // base directory for all caches (e.g. ~/chimii_workspaces/.repos)
	logger *slog.Logger
...
type WorktreeParams struct {
	...
	// listed as a writable root (chimii-ai/chimii#2925).
...
const (
	isolatedCheckoutConfigKey   = "chimii.checkout-mode"
	isolatedCheckoutConfigValue = "isolated"
	isolatedCacheRemoteName     = "chimii-cache"
)
...
func (c *Cache) createOrUpdateIsolatedCheckout(barePath, repoURL, checkoutPath, branchName, baseRef string) (string, error) {
	...
	if _, err := os.Stat(checkoutPath); err == nil {
		return "", fmt.Errorf("checkout path already exists and is not a Chimii isolated checkout: %s", checkoutPath)
...
// deleteStaleAgentBranches prunes branches left by earlier Chimii tasks while
...
// chimiiHookMarker is a sentinel comment embedded in every prepare-commit-msg
...
const chimiiHookMarker = "# chimii:prepare-commit-msg:co-authored-by"
...
// treats a hook as Chimii-owned if its content contains ANY of these
// substrings. The list deliberately includes the legacy comment that the
// daemon used before chimiiHookMarker existed, so disabling the toggle on
...
var daemonInstalledHookSignatures = []string{
	chimiiHookMarker,
	"# Installed by the Chimii daemon.",
}

// prepareCommitMsgHook is the prepare-commit-msg hook script that appends a
// Co-authored-by trailer for the Chimii Agent to every commit message.
const prepareCommitMsgHook = `#!/bin/sh
# chimii:prepare-commit-msg:co-authored-by
# Chimii: add Co-authored-by trailer for the Chimii Agent.
# Installed by the Chimii daemon. Do not edit — it will be overwritten.
...
TRAILER="Co-authored-by: chimii-agent <github@chimii.ai>"
...
// a Co-authored-by trailer for the Chimii Agent. The hook is installed in the
...
// installed by the Chimii daemon (current or any previously released
