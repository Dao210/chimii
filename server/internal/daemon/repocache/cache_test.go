...
// for the Chimii Agent to every commit made in the worktree.
func TestCreateWorktreeInstallsCoAuthoredByHook(t *testing.T) {
	...
	expectedTrailer := "Co-authored-by: chimii-agent <github@chimii.ai>"
...
// path: a bare cache that already carries the Chimii prepare-commit-msg hook
...
func TestCreateWorktreeRemovesCoAuthoredByHookWhenDisabled(t *testing.T) {
	...
	commitMsg := string(out)
	if strings.Contains(commitMsg, "Co-authored-by: chimii-agent") {
...
// prepare-commit-msg hook that does NOT include the chimiiHookMarker
// sentinel — only the older `# Installed by the Chimii daemon.` comment.
...
func TestCreateWorktreeRemovesLegacyCoAuthoredByHook(t *testing.T) {
	...
	// previous daemon release (no chimiiHookMarker line). Keeping a
...
`
	...
	if commitMsg := string(out); strings.Contains(commitMsg, "Co-authored-by: chimii-agent") {
...
// without the Chimii marker (e.g. one a user added manually) must be left
