...
// same machine shares one UUID stored at `~/.chimii/daemon.id`. Profile
...
// LegacyDaemonUUIDs scans `~/.chimii/profiles/*/daemon.id` and returns every
...
// explicitly pins CHIMII_DAEMON_ID to the hostname itself, there's nothing
