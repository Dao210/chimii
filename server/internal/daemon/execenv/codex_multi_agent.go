...
// subagents. The Chimii daemon currently models only the parent Codex
...
// lifecycle state with drain/cancel primitives, or the Chimii runtime
...
//
// Users who explicitly want Codex native subagents inside a Chimii task
// (and accept the lifecycle risk) can keep the feature enabled by setting
// `CHIMII_CODEX_MULTI_AGENT=1` in the daemon environment.
//
// Layout note
//
...
// ChimiiCodexMultiAgentEnv is the env var users can set to keep Codex
...
const ChimiiCodexMultiAgentEnv = "CHIMII_CODEX_MULTI_AGENT"

// chimiiMultiAgentBeginMarker / chimiiMultiAgentEndMarker delimit the
// multi-agent-specific managed block. Kept separate from the sandbox
// block so each can evolve and migrate independently.
const (
	chimiiMultiAgentBeginMarker = "# BEGIN chimii-managed multi-agent (do not edit; regenerated by daemon)"
	chimiiMultiAgentEndMarker   = "# END chimii-managed multi-agent"
)
...
var multiAgentBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(chimiiMultiAgentBeginMarker) +
		`.*?^` + regexp.QuoteMeta(chimiiMultiAgentEndMarker) + `\n*`)
...
func codexMultiAgentEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(ChimiiCodexMultiAgentEnv))
...
// renderChimiiMultiAgentBlock returns the daemon-managed multi-agent
// block. The body uses `multi_agent = false` when injected inside a
// `[features]` table, and `features.multi_agent = false` otherwise.
func renderChimiiMultiAgentBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(chimiiMultiAgentBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(chimiiMultiAgentEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectManagedBlockIntoFeaturesTable(content string) string {
	block := renderChimiiMultiAgentBlock(true)
...
//
// When CHIMII_CODEX_MULTI_AGENT is set to a truthy value, the function is
...
func ensureCodexMultiAgentConfig(configPath string, logger *slog.Logger) error {
	if codexMultiAgentEnabled() {
		if logger != nil {
			logger.Info("codex multi-agent: leaving Codex native multi-agent untouched per CHIMII_CODEX_MULTI_AGENT",
	...
	if hasRootFeaturesTable(existing) {
		...
		block := renderChimiiMultiAgentBlock(false)
		if existing == "" {
			updated = block
		} else {
			updated = block + "\n" + existing
