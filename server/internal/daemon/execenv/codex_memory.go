...
// — users cannot audit or edit the contents from any Chimii UI.
//
// This conflicts with the Chimii daemon's context model. Chimii
...
//     unrelated host-project context into Chimii tasks. The reproduction
//     in github.com/chimii-ai/chimii#3130 saw Raw Memories from
//     `D:\Project\MoHaYu\WowChat` (a host-local project) injected into a
//     brand-new Chimii issue's first Codex turn.
//
...
//
// Users who explicitly want Codex native memory inside a Chimii task
// (and accept the leak risk) can keep the feature enabled by setting
// `CHIMII_CODEX_MEMORY=1` in the daemon environment. The long-term
// answer for durable agent context is a Chimii-owned, user-visible,
...
// ChimiiCodexMemoryEnv is the env var users can set to keep Codex
...
const ChimiiCodexMemoryEnv = "CHIMII_CODEX_MEMORY"

const (
	chimiiMemoryFeatureBeginMarker = "# BEGIN chimii-managed memory-feature (do not edit; regenerated by daemon)"
	chimiiMemoryFeatureEndMarker   = "# END chimii-managed memory-feature"
	chimiiMemoryConfigBeginMarker  = "# BEGIN chimii-managed memory-config (do not edit; regenerated by daemon)"
	chimiiMemoryConfigEndMarker    = "# END chimii-managed memory-config"
)
...
var memoryFeatureBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(chimiiMemoryFeatureBeginMarker) +
		`.*?^` + regexp.QuoteMeta(chimiiMemoryFeatureEndMarker) + `\n*`)

var memoryConfigBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(chimiiMemoryConfigBeginMarker) +
		`.*?^` + regexp.QuoteMeta(chimiiMemoryConfigEndMarker) + `\n*`)
...
func codexMemoryEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(ChimiiCodexMemoryEnv))
...
// renderChimiiMemoryFeatureBlock returns the daemon-managed memory-feature
// block. The body uses `memories = false` when injected inside a
// `[features]` table, and `features.memories = false` otherwise.
func renderChimiiMemoryFeatureBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(chimiiMemoryFeatureBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(chimiiMemoryFeatureEndMarker)
	b.WriteString("\n")
	return b.String()
}

// renderChimiiMemoryConfigBlock returns the daemon-managed memory-config
// block. The body uses bare keys when injected inside a `[memories]` table
// and dotted-key form otherwise.
func renderChimiiMemoryConfigBlock(inMemoriesTable bool) string {
	var b strings.Builder
	b.WriteString(chimiiMemoryConfigBeginMarker)
	b.WriteString("\n")
	if inMemoriesTable {
	...
	b.WriteString(chimiiMemoryConfigEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectMemoryFeatureBlockIntoFeaturesTable(content string) string {
	return injectAfterHeader(content, rootFeaturesTableHeaderRe, renderChimiiMemoryFeatureBlock(true))
}
...
func injectMemoryConfigBlockIntoMemoriesTable(content string) string {
	return injectAfterHeader(content, rootMemoriesTableHeaderRe, renderChimiiMemoryConfigBlock(true))
}
...
//
// When CHIMII_CODEX_MEMORY is set to a truthy value, the function is a
...
func ensureCodexMemoryConfig(configPath string, logger *slog.Logger) error {
	if codexMemoryEnabled() {
		if logger != nil {
			logger.Info("codex memory: leaving Codex native memory untouched per CHIMII_CODEX_MEMORY",
	...
	if !hasFeatures {
		prepend.WriteString(renderChimiiMemoryFeatureBlock(false))
	}
	if !hasMemories {
		prepend.WriteString(renderChimiiMemoryConfigBlock(false))
	}
