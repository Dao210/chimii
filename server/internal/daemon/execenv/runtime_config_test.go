	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, want := range []string{
				...
				"`--status backlog` = **wait**",
				"`chimii issue status <child-id> todo`",
				"all `--status todo`",
...
func TestBriefHasNoParentNotificationGuidance(t *testing.T) {
	...
	for _, ctx := range cases {
		...
		for _, banned := range []string{
			...
			"chimii issue comment add <parent-id>",
			"with NO `--parent`",
			...
			"`chimii issue status <this-issue-id> in_review`",
...
// dedicated protocol step that wrote `chimii issue status <this-issue-id> in_review`;
...
func TestCommentTriggeredProtocolDoesNotForceInReview(t *testing.T) {
	...
	if strings.Contains(out, "`chimii issue status <this-issue-id> in_review`") {
...
func TestColdCommentsHintPointsAtTriggeringThread(t *testing.T) {
	...
	if !strings.Contains(hint, "chimii issue comment list "+issueID+" --thread thread-root-1 --tail 30 --output json") {
...
func TestIssueWorkflowHonorsAgentIdentity(t *testing.T) {
	...
	for _, want := range []string{
		...
		"Before step 4, run `chimii issue status " + issueID + " in_progress` unless your Agent Identity forbids issue status changes; if it does, skip it.",
		...
		"When done, run `chimii issue status " + issueID + " in_review` unless your Agent Identity forbids issue status changes; if it does, skip it.",
		"If blocked, run `chimii issue status " + issueID + " blocked` unless your Agent Identity forbids issue status changes.",
	} {
		if !strings.Contains(out, want) {
	...
	for _, banned := range []string{
		"4. Run `chimii issue status " + issueID + " in_progress`\n",
		"5. Follow your Skills and Agent Identity to complete the task (write code, investigate, etc.)",
		"8. When done, run `chimii issue status " + issueID + " in_review`\n",
	} {
		if strings.Contains(out, banned) {
...
func TestSquadLeaderIssueWorkflowKeepsParentInProgress(t *testing.T) {
	...
	for _, want := range []string{
		...
		"do NOT run `chimii issue status " + issueID + " in_review` or `done` on this turn",
	...
	if strings.Contains(out, "When done, run `chimii issue status "+issueID+" in_review`") {
...
func TestChatOutputDoesNotRequireIssueComment(t *testing.T) {
	...
	for _, banned := range []string{
		...
		"do not call `chimii issue comment add`",
...
func TestWriteRuntimeConfigFileCreatesMissingFile(t *testing.T) {
	...
	const brief = "# Chimii Agent Runtime\n\nbrief body line"
...
func TestWriteRuntimeConfigFileReplacesExistingBlock(t *testing.T) {
	...
	const newBrief = "## New Chimii brief\n\nfresh body"
...
func TestWriteRuntimeConfigFileIgnoresStrayEndMarkerBeforeBegin(t *testing.T) {
	...
	// real block authored by an earlier Chimii run below.
	const userDoc = "# Repo CLAUDE.md\n\nExample of what Chimii writes:\n" +
		runtimeMarkerEnd + "\n\n# Real config below\n"
	original := userDoc +
		runtimeMarkerBegin + "\nFIRST BRIEF\n" + runtimeMarkerEnd + "\n"
	if err := os.WriteFile(path, []byte(original), 0o644); err != nil {
...
// `codex` run started by the user after a Chimii task must see the same
// file the user wrote.
