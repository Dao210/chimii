...
// `no such host` errors when calling out (for example, `chimii issue get`
// hitting the Chimii API). See upstream issue openai/codex#10390.
//
...
// actually reach the Chimii API. On Linux (and on macOS once the upstream
...
//     rejects non-safe mutation commands "by policy" — e.g. `chimii issue
...
//     danger-full-access so the Chimii CLI can reach the API.
...
// chimiiManagedBeginMarker / chimiiManagedEndMarker delimit the block the
...
const (
	chimiiManagedBeginMarker = "# BEGIN chimii-managed (do not edit; regenerated by daemon)"
	chimiiManagedEndMarker   = "# END chimii-managed"
)

// renderChimiiManagedBlock produces the managed block for the given policy.
//
...
//     other table (e.g. `[permissions.chimii]`), a bare `sandbox_mode = ...`
...
// at the top of the file (see upsertChimiiManagedBlock), avoids both traps.
func renderChimiiManagedBlock(policy codexSandboxPolicy) string {
	var b strings.Builder
	b.WriteString(chimiiManagedBeginMarker)
	b.WriteString("\n")
	...
	b.WriteString(chimiiManagedEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
var managedBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(chimiiManagedBeginMarker) +
		`.*?^` + regexp.QuoteMeta(chimiiManagedEndMarker) + `\n*`)

// upsertChimiiManagedBlock returns the config content with the chimii-managed
...
// `[permissions.chimii]` or `[profiles.foo]`. Combined with the dotted-key
// form used by renderChimiiManagedBlock, this means the managed block neither
// leaks into nor inherits from any surrounding table scope.
func upsertChimiiManagedBlock(content string, policy codexSandboxPolicy) string {
	// Drop any previously written managed block (wherever it sits).
	content = managedBlockRe.ReplaceAllString(content, "")
	block := renderChimiiManagedBlock(policy)
...
// ensureCodexSandboxConfig writes the chimii-managed sandbox block into the
...
func ensureCodexSandboxConfig(configPath string, policy codexSandboxPolicy, detectedVersion string, logger *slog.Logger) error {
	...
	updated := upsertChimiiManagedBlock(existing, policy)
