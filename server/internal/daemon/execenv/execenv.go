...
// checked out on demand by the agent via `chimii repo checkout`.
package execenv
...
type PrepareParams struct {
	WorkspacesRoot string // base path for all envs (e.g., ~/chimii_workspaces)
...
type TaskContextForEnv struct {
	...
	// policy only: any non-empty value means the reply leaves Chimii for an
	// external channel, so `chimii attachment upload` cannot deliver a file and
...
// on demand via `chimii repo checkout <url>`.
func Prepare(params PrepareParams, logger *slog.Logger) (*Environment, error) {
	...
	// that lose all CHIMII_* env vars AND escape above the workdir. Non-fatal:
...
func Reuse(params ReuseParams, logger *slog.Logger) *Environment {
	...
	// a collision-free sibling (issue-review, issue-review-chimii,
	// issue-review-chimii-2, …), accumulating a fresh duplicate on every
	...
	//      refresh back to issue-review-chimii.
...
const ManagedEnvProvenanceManagedBy = "chimii-daemon-managed-env"
