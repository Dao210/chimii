...
// exercises the same stdin/stdout protocol as the real chimii helper.
			...
			ProjectResources: []ProjectResourceForEnv{
				{
					...
					ResourceRef:  json.RawMessage(`{"url":"https://github.com/chimii-ai/chimii"}`),
				},
			},
		},
	}, logger)
...
func TestPreparationHelperRoundTripsProjectResources(t *testing.T) {
	...
	data, err := os.ReadFile(filepath.Join(env.WorkDir, ".chimii", "project", "resources.json"))
	if err != nil {
	...
	if resource.ID != "resource-helper-project-resource" ||
		...
		ref.URL != "https://github.com/chimii-ai/chimii" ||
		resource.Label != "Chimii" {
