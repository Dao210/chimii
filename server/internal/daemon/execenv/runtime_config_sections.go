...
import (
	...
	"github.com/chimii-ai/chimii/server/internal/runtimeapps"
)
...
func writeHeader(b *strings.Builder) {
	b.WriteString("# Chimii Agent Runtime\n\n")
	b.WriteString("You are a coding agent in the Chimii platform. Use the `chimii` CLI to interact with the platform.\n\n")
}
...
func writeBackgroundTaskSafetySlim(b *strings.Builder) {
	...
	b.WriteString("Chimii marks the task terminal the moment your top-level turn exits — any process, tool call, or subagent owned by this run that is still active is orphaned, its result lost, and the final comment you meant to post after it never sends. There is no background-completion wakeup here.\n\n")
	...
	b.WriteString("- If a tool response says to wait for a future notification/reminder, or that it is running in the background so you can keep working, do not rely on that in Chimii-managed runs — block on the appropriate wait / output / collect operation before exiting.\n")
...
func BuildTaskInitiatorBlock(initiatorType, initiatorName, initiatorEmail string) string {
	...
	b.WriteString("Attribute this request to that person and apply any per-person privacy or access rules your instructions define — in a workspace many people can reach, the initiator (not the runtime owner) is who you are answering. Your Chimii credentials stay scoped to the runtime owner, so this attribution does not widen what you can read or write — do not assume the initiator can see everything you can.\n\n")
	return b.String()
}
...
// preserved: each `chimii issue …` command name, all three `comment add`
...
// create/update tasks" intro phrase, and `chimii issue comment add
// --help`.
//
...
func writeAvailableCommands(b *strings.Builder) {
	...
	b.WriteString("Prefer `--output json` for structured data. The default brief lists only the core agent loop and common issue create/update tasks; for everything else run `chimii --help` or `chimii <command> --help`.\n\n")
	b.WriteString("### Core\n")
	b.WriteString("- `chimii issue get <id> --output json` — full issue.\n")
	b.WriteString("- `chimii issue comment list <issue-id> [--thread <comment-id> [--tail N] | --recent N] [--before <ts> --before-id <uuid>] [--since <RFC3339>] [--full] --output json` — thread-aware comment reads. Resolved threads come back folded by default on complete-thread reads (default list, `--recent`, `--thread` without `--tail`); pass `--full` to expand. Page older replies / threads with `--before`/`--before-id` (stderr labels: `Next reply cursor`, `Next thread cursor`); `--help` for full semantics.\n")
	...
	b.WriteString("- `chimii issue update <id> [--title X] [--description-file <path>] [--priority X] [--status X] [--assignee X] [--parent <issue-id>] [--stage N] [--project <project-id>] [--due-date <RFC3339>]` — update fields; pass `--parent \"\"` to clear parent.\n")
	b.WriteString("- `chimii issue status <id> <status>` — flip status (todo / in_progress / in_review / done / blocked / backlog / cancelled).\n")
	b.WriteString("- `chimii issue children <id> [--output json]` — list a parent's sub-issues grouped by stage.\n")
	b.WriteString("- `chimii issue comment add <issue-id> [--content \"...\" | --content-file <path> | --content-stdin] [--parent <comment-id>] [--attachment <path>]` — post a comment. Agent-authored bodies MUST use `--content-file`. `chimii issue comment add --help` for full flags.\n")
	b.WriteString("- `chimii issue metadata list <issue-id> [--output json]` — list KV metadata.\n")
	b.WriteString("- `chimii issue metadata set <issue-id> --key <k> --value <v> [--type string|number|bool]` — pin or overwrite a key.\n")
	b.WriteString("- `chimii issue metadata delete <issue-id> --key <k>` — remove a key.\n")
	b.WriteString("- `chimii repo checkout <url> [--ref <branch-or-sha>]` — repository checkout on a dedicated branch.\n\n")
	b.WriteString("### Squad maintenance\n")
	b.WriteString("- `chimii squad member set-role <squad-id> --member-id <id> --member-type <agent|member> --role <role> [--output json]` — change role in place (use this instead of remove+add).\n\n")
}
...
// every CLI other than `chimii issue create`, so listing more would just
// tempt the model to bend the guardrail.
func writeAvailableCommandsQuickCreate(b *strings.Builder) {
	b.WriteString("## Available Commands\n\n")
	b.WriteString("**Use `--output json` for structured data.** For anything beyond `issue create`, run `chimii --help` or `chimii <command> --help`.\n\n")
	b.WriteString("### Core\n")
...
func writeCommentFormatting(b *strings.Builder) {
	...
	if runtimeGOOS == "windows" {
		b.WriteString("On Windows, **always write the comment body to a UTF-8 file with your file-write tool first, then post it with `--content-file <path>`** — do NOT pipe via `--content-stdin` (PowerShell 5.1's `$OutputEncoding` defaults to ASCIIEncoding when piping to a native command, silently dropping non-ASCII characters as `?` before they reach `chimii.exe`). Never use inline `--content` for agent-authored comments. Write that file inside your working directory (`./reply.md`), never `/tmp` or shared paths — the CLI rejects a `--content-file` path outside the workdir so another run's stale file can't leak in (MUL-4252). Keep the same `--parent` value from the trigger comment when replying. Delete the temp file (`Remove-Item ./reply.md`) after posting; do not rely on `\\n` escapes.\n\n")
		return
	}
...
func writeRepositories(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("Available in this workspace — `chimii repo checkout <url> [--ref <branch-or-sha>]` to fetch (creates a repository checkout on a dedicated branch).\n\n")
...
func writeProjectContext(b *strings.Builder, ctx TaskContextForEnv) {
	...
	if len(ctx.ProjectResources) > 0 {
		b.WriteString("Project resources (also written to `.chimii/project/resources.json`):\n\n")
		for _, r := range ctx.ProjectResources {
			fmt.Fprintf(b, "- %s\n", formatProjectResource(r))
		}
		b.WriteString("\nResources are pointers — open them only when relevant to the task. ")
		b.WriteString("For `github_repo` resources, use `chimii repo checkout <url>` to fetch the code. Add `--ref <branch-or-sha>` when a task or handoff names an exact revision.\n\n")
	} else {
...
func writeIssueMetadata(b *strings.Builder) {
	...
	b.WriteString("- **Write on exit.** Pin only if BOTH: (a) materially important to this issue, AND (b) a future run is likely to re-read it. Otherwise leave the bag alone. Stale keys: overwrite with the new value or `chimii issue metadata delete`.\n")
...
func writeWorkflowChat(b *strings.Builder) {
	...
	b.WriteString("- You have full access to the `chimii` CLI to look up issues, workspace info, members, agents, etc.\n")
	b.WriteString("- If asked about issues, use `chimii issue list --output json` or `chimii issue get <id> --output json`\n")
	b.WriteString("- If asked about the workspace, use `chimii workspace get --output json`\n")
	b.WriteString("- If asked to perform actions (create issues, update status, etc.), use the appropriate CLI commands\n")
	b.WriteString("- If the task requires code changes, use `chimii repo checkout <url>` to get the code first. Use `--ref <branch-or-sha>` when you need an exact revision\n")
...
func writeWorkflowQuickCreate(b *strings.Builder) {
	b.WriteString("**This task was triggered by quick-create.** There is NO existing Chimii issue. Follow the field and output rules in the user message you just received; ignore the default assignment-task workflow.\n\n")
	b.WriteString("Hard guardrails (apply even if the user message is missing):\n")
	b.WriteString("- Run exactly one `chimii issue create` invocation, then exit.\n")
	b.WriteString("- Do NOT call `chimii issue get`, `chimii issue status`, or `chimii issue comment add` for this task — there is no issue to query, transition, or comment on. The platform writes the user's success/failure inbox notification automatically based on whether `chimii issue create` succeeded.\n")
...
func writeWorkflowAutopilot(b *strings.Builder, ctx TaskContextForEnv) {
	b.WriteString("**This task was triggered by an Autopilot in run-only mode.** There is no assigned Chimii issue for this run.\n\n")
	...
	if ctx.AutopilotID != "" {
		fmt.Fprintf(b, "- Run `chimii autopilot get %s --output json` if you need the full autopilot configuration\n", ctx.AutopilotID)
	}
	b.WriteString("- Complete the autopilot instructions directly\n")
	b.WriteString("- Do not run `chimii issue get`, `chimii issue comment add`, or `chimii issue status` for this run unless the autopilot instructions explicitly tell you to create or update an issue\n\n")
}
...
func writeWorkflowIssue(b *strings.Builder, ctx TaskContextForEnv) {
	...
	if ctx.IsSquadLeader {
		fmt.Fprintf(b, "5. **Post your final results as a comment** (unless your outcome is `no_action` — in that case, calling `chimii squad activity %s no_action --reason \"...\"` alone is sufficient; you MUST exit without posting any comment. DO NOT post a comment announcing no_action or saying you are exiting silently): post it with `chimii issue comment add %s` using the platform-correct non-inline mode from ## Comment Formatting (never inline `--content`). Your results are only visible to the user if posted via this CLI call; text in your terminal or run logs is NOT delivered.\n", ctx.IssueID, ctx.IssueID)
	} else {
		fmt.Fprintf(b, "5. **Post your final results as a comment — this step is mandatory**: post it with `chimii issue comment add %s` using the platform-correct non-inline mode from ## Comment Formatting (never inline `--content`). Your results are only visible to the user if posted via this CLI call; text in your terminal or run logs is NOT delivered. In Reply mode this step is conditional on the reply rule below.\n", ctx.IssueID)
	}
	b.WriteString("6. Before exiting: only if this run produced a fact that clears the high bar (important AND likely to be re-read by future runs on this same issue, e.g. a new PR URL or deploy URL), or you noticed a metadata key from entry that is now stale, pin or clear it via `chimii issue metadata set`/`delete`. Most runs write nothing here — that is the expected outcome, not a gap. When in doubt, do not write. See the `## Issue Metadata` section above for the full bar.\n\n")

	b.WriteString("**Ownership mode only — you own the issue status this run**\n\n")
	fmt.Fprintf(b, "- Before step 4, run `chimii issue status %s in_progress` unless your Agent Identity forbids issue status changes; if it does, skip it.\n", ctx.IssueID)
	if ctx.IsSquadLeader {
		fmt.Fprintf(b, "- After this initial dispatch, leave the parent issue `in_progress` — do NOT run `chimii issue status %s in_review` or `done` on this turn. Dispatching members is not completion. You will be re-triggered when members post updates or a stage closes; only then, if the overall goal is met, move the parent to `in_review`.\n", ctx.IssueID)
	} else {
		fmt.Fprintf(b, "- When done, run `chimii issue status %s in_review` unless your Agent Identity forbids issue status changes; if it does, skip it.\n", ctx.IssueID)
	}
	fmt.Fprintf(b, "- If blocked, run `chimii issue status %s blocked` unless your Agent Identity forbids issue status changes. Post a comment explaining the blocker unless your Agent Identity forbids issue comments.\n\n", ctx.IssueID)
...
func writeSubIssueCreation(b *strings.Builder) {
	...
	b.WriteString("**Choosing `--status` when creating sub-issues.** `--status todo` = **start now** (default — agent assignees fire immediately). `--status backlog` = **wait**, then promote later with `chimii issue status <child-id> todo`. Parallel children: all `--status todo`. Strict serial 1→2→3: only Step 1 `todo`, Steps 2/3 `--status backlog` from the start.\n\n")
	b.WriteString("**Ordering with stages.** For phased plans, group children with `--stage <N>` (N ≥ 1) instead of hand-promoting the backlog chain — stage members run together, and the parent wakes once per stage. Use `--stage k --status backlog` for later stages, then `chimii issue children <id>` to inspect groupings before promoting. Reach for stages whenever a plan has more than one step or a step must wait for a group.\n\n")
}
...
func writeAttachments(b *strings.Builder) {
	...
	b.WriteString("When a task includes attachment IDs and you need the files, inspect `chimii attachment --help` and use the authenticated CLI path. Do not open Chimii resource URLs directly.\n")
...
// writeAlwaysUseCLI emits the "must go through the chimii CLI" guardrail
// (compressed).
func writeAlwaysUseCLI(b *strings.Builder) {
	b.WriteString("## Important: Always Use the `chimii` CLI\n\n")
	b.WriteString("Access Chimii platform resources (issues, comments, attachments, files) only through the `chimii` CLI — never `curl` / `wget`. For any operation the CLI doesn't cover, post a comment mentioning the workspace owner rather than working around it.\n\n")
}
...
func writeOutput(b *strings.Builder, kind taskKind, ctx TaskContextForEnv) {
	...
	case kindQuickCreate:
		b.WriteString("This is a quick-create task. There is NO existing issue to comment on. Your final stdout is captured automatically and the platform writes the user's success/failure inbox notification based on whether `chimii issue create` succeeded.\n\n")
		b.WriteString("- Do NOT call `chimii issue comment add` — the issue you just created has no conversation context for this run.\n")
		b.WriteString("- Print exactly one final line: `Created <identifier-or-id>: <title>` after a successful `chimii issue create`. Use the created issue's `identifier` from JSON output when available; otherwise use its `id`. Do not assume any workspace issue prefix such as `MUL-`; workspaces can use custom prefixes.\n")
		...
		b.WriteString("**Delivering files here:** your stdout is text-only. A file that belongs to the new issue goes on the `chimii issue create` call itself via `--attachment <path>`; never put its path in the description or in your stdout line.\n")
	case kindChat:
		...
		// non-empty channel type means the reply leaves Chimii for an external
		...
		if ctx.ChatChannelType != "" {
			fmt.Fprintf(b, "**Delivering files here:** this %s conversation is text-only — Chimii cannot push a file you produced back into it. `chimii attachment upload` does NOT apply: it binds to a Chimii chat reply, which this is not. Say in words what you produced and where it can be obtained; never upload and then write as though the file arrived, and never link its local path.\n", ChannelDisplayName(ctx.ChatChannelType))
		} else {
			b.WriteString("**Delivering files here:** run `chimii attachment upload <local-path>` — it binds the file to your reply and it renders as an attachment card. That command is the ONLY way a file reaches the user; a path written into your reply text is not.\n")
		}
	default:
		if ctx.IsSquadLeader {
			b.WriteString("⚠️ **Final results MUST be delivered via `chimii issue comment add`** — unless your outcome is `no_action`. When you evaluate a trigger and decide no action is needed, calling `chimii squad activity <issue-id> no_action --reason \"...\"` alone is sufficient; you MUST exit without posting any comment. DO NOT post a comment that announces no_action, acknowledges another agent, or says you are exiting silently — such comments are noise. For all other outcomes (`action`, `failed`), a comment is still mandatory.\n\n")
		} else {
			b.WriteString("⚠️ **Final results MUST be delivered via `chimii issue comment add`.** The user does NOT see your terminal output, assistant chat text, or run logs — only comments on the issue. A task that finishes without a result comment is invisible to the user, even if the work itself was correct.\n\n")
		}
		...
		b.WriteString("**Delivering files here:** pass `--attachment <path>` to `chimii issue comment add` (repeatable). The file uploads and renders on the comment; that is the only way a screenshot or artifact reaches the reader.\n")
	}
	b.WriteString("\n")
