...
// daemon-spawned Codex process can have CHIMII_TOKEN while `chimii issue ...`
...
const (
	...
	chimiiShellEnvBeginMarker = "# BEGIN chimii-managed shell-environment (do not edit; regenerated by daemon)"
	chimiiShellEnvEndMarker   = "# END chimii-managed shell-environment"
)

var shellEnvBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(chimiiShellEnvBeginMarker) +
		`.*?^` + regexp.QuoteMeta(chimiiShellEnvEndMarker) + `\n*`)
...
// custom_env. Inherited CHIMII_* variables are always dropped because they
// belong to the daemon process, not necessarily to this task. Explicit
// CHIMII_* values are safe to include because daemon.go blocklists that
...
func renderChimiiShellEnvBlock(includeOnly []string) (string, error) {
	...
	b.WriteString(chimiiShellEnvBeginMarker)
	b.WriteByte('\n')
	b.WriteString("[")
	...
	b.WriteByte('\n')
	b.WriteString(chimiiShellEnvEndMarker)
	b.WriteByte('\n')
	return b.String(), nil
}
...
func EnsureCodexShellEnvPolicyConfig(configPath string, includeOnly []string, logger *slog.Logger) error {
	...
	block, err := renderChimiiShellEnvBlock(includeOnly)
	if err != nil {
		return err
	}
