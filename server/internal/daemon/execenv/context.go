...
import (
	...
	skillpkg "github.com/chimii-ai/chimii/server/internal/skill"
	"gopkg.in/yaml.v3"
)
...
// sandbox strips all CHIMII_* env vars before invoking `chimii`.
const TaskContextMarkerRelPath = ".chimii/daemon_task_context.json"
...
const TaskContextMarkerManagedBy = "chimii-daemon-task"
...
// {workspacesRoot}/.chimii/daemon_task_context.json.
//
// The per-workdir marker only protects `chimii` invocations whose cwd is
// inside the workdir, because the CLI discovers markers by walking *up* from
// cwd. A sandboxed subprocess that lost every CHIMII_* env var and escaped
...
func writeTaskContextMarker(workDir string, ctx TaskContextForEnv, manifest *sidecarManifest) error {
	...
	if err := recordMkdirAll(dir, 0o755, manifest); err != nil {
		return fmt.Errorf("create .chimii dir: %w", err)
	}
...
// writeProjectResources writes .chimii/project/resources.json into the
...
//
// manifest, when non-nil, is populated with the .chimii/project chain
...
func writeProjectResources(workDir string, ctx TaskContextForEnv, manifest *sidecarManifest) error {
	...
	dir := filepath.Join(workDir, ".chimii", "project")
	if err := recordMkdirAll(dir, 0o755, manifest); err != nil {
		return err
	}
	...
	if err := recordWriteFile(filepath.Join(dir, "resources.json"), data, 0o644, manifest); err != nil {
		// .chimii/project/resources.json is Chimii-owned and a
...
//
// When a Chimii skill's natural slug collides with a user-installed
// skill at the same path, we allocate a collision-free sibling slug
// (e.g. `issue-review-chimii`) and write there instead. Provider-native
...
// skill entirely (which would silently drop a Chimii skill the agent
// expects to see).
...
func renderIssueContext(provider string, ctx TaskContextForEnv) string {
	...
	fmt.Fprintf(&b, "Run `chimii issue get %s --output json` to fetch the full issue details.\n\n", ctx.IssueID)
...
func renderAutopilotContext(ctx TaskContextForEnv) string {
	...
	b.WriteString("## Quick Start\n\n")
	b.WriteString("This is a run-only autopilot task with no assigned issue. Do not run `chimii issue get` unless the autopilot instructions explicitly ask you to create or update an issue.\n\n")
	if ctx.AutopilotID != "" {
