...
func TestCodexShellEnvAllowlistUsesExactTaskAndSafeInheritedNames(t *testing.T) {
	...
	inherited := []string{
		...
		"CHIMII_SERVER_URL=https://wrong.example",
	}
...
set = { FOO = "bar", CHIMII_TOKEN = "stale" }
...
`,
			...
			removed: []string{"legal quoted table", `FOO = "bar"`, `CHIMII_TOKEN = "stale"`},
		},
		{
...
`,
	...
	includeOnly := []string{"CUSTOM_FLAG", "CHIMII_SERVER_URL", "CHIMII_TOKEN", "PATH", "SystemRoot"}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if strings.Contains(s, `"CHIMII_*"`) {
				t.Fatalf("managed policy must not use a CHIMII_* wildcard:\n%s", s)
			}
...
func TestEnsureCodexShellEnvPolicyConfigRejectsInvalidInputWithoutWriting(t *testing.T) {
	...
	if err := EnsureCodexShellEnvPolicyConfig(configPath, []string{"CHIMII_TOKEN", "PATH"}, testLogger()); err == nil {
...
func TestEnsureCodexShellEnvPolicyConfigIsIdempotent(t *testing.T) {
	t.Parallel()
	configPath := filepath.Join(t.TempDir(), "config.toml")
	includeOnly := []string{"CHIMII_SERVER_URL", "CHIMII_TOKEN", "PATH"}
	...
	s := string(data)
	assertValidToml(t, s)
	if n := strings.Count(s, chimiiShellEnvBeginMarker); n != 1 {
