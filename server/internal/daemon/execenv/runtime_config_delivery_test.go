...
func TestBriefSurfaceDeliveryPolicy(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"comment": {
			mustHave: []string{"`--attachment <path>` to `chimii issue comment add`"},
			mustNot:  []string{"chimii attachment upload"},
		},
		"assignment": {
			mustHave: []string{"`--attachment <path>` to `chimii issue comment add`"},
			mustNot:  []string{"chimii attachment upload"},
		...
		"chat_direct": {
			mustHave: []string{"`chimii attachment upload <local-path>`"},
			mustNot:  []string{"text-only"},
		},
		// IM surfaces are text-only. The upload command must not appear: it binds
		// to a Chimii chat reply, which an IM reply is not, so suggesting it
		...
		"chat_slack": {
			...
			mustNot:  []string{"run `chimii attachment upload"},
		},
		"chat_feishu": {
		...
		"quickcreate": {
			mustHave: []string{"your stdout is text-only", "`chimii issue create` call itself via `--attachment <path>`"},
			mustNot:  []string{"chimii attachment upload"},
		},
	}
