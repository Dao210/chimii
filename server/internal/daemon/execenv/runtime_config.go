...
// runtimeMarkerBegin and runtimeMarkerEnd delimit the Chimii-managed brief
...
const (
	runtimeMarkerBegin = "<!-- BEGIN CHIMII-RUNTIME (auto-managed; do not edit) -->"
	runtimeMarkerEnd   = "<!-- END CHIMII-RUNTIME -->"
...
// writeRuntimeConfigFile writes the Chimii runtime brief to path without
...
// locateMarkerBlock finds the [start, end) byte range of the Chimii marker
...
//
//   - User content carries a stray `<!-- END CHIMII-RUNTIME -->` (e.g. a
...
// CleanupRuntimeConfig excises the Chimii marker block from the runtime
...
// of Chimii runs without ever touching a single non-managed byte.
//
...
// the user inside the same directory after a Chimii task would pick up
...
// agent about the Chimii runtime environment and available CLI tools.
//
