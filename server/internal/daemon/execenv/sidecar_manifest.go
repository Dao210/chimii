...
const sidecarManifestFile = ".chimii_sidecar_manifest.json"
...
//     so the agent still discovers the Chimii skill, just under a
//     different directory name.
//   - For Chimii-only namespaces (.agent_context/issue_context.md,
//     .chimii/project/resources.json) the caller swallows the error
...
// down a Chimii skill without colliding with a user-installed skill
...
// collision we append `-chimii`, then `-chimii-2`, `-chimii-3`,
...
func allocateCollisionFreeSkillDir(skillsParent, baseSlug string) (slug, dir string, err error) {
	...
	for i := 0; i < maxAttempts; i++ {
		...
		case i == 0:
			candidate = baseSlug
		case i == 1:
			candidate = baseSlug + "-chimii"
		default:
			candidate = fmt.Sprintf("%s-chimii-%d", baseSlug, i)
		}
...
// (.agent_context/, .chimii/, .claude/skills/, .github/skills/,
...
// issue-review-chimii. A managed skill directory is platform-owned — the
