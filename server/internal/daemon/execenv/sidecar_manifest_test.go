...
// nothing related to Chimii before a task ran must contain nothing
// related to Chimii after the task finishes — no .agent_context/,
// no .claude/skills/, no .chimii/, no stub directories. The test
...
// must still exist and the Chimii-written sibling must be gone.
...
// round-trip to the .chimii/project/resources.json branch — a separate
...
func TestCleanupSidecarsLeavesUserContentInTrackedDirIntact(t *testing.T) {
	...
	// Imagine Prepare wrote .chimii/sidecar.txt and created
	// .chimii/ + .chimii/project/, then exited. Between Prepare
	// and Cleanup the user dropped their own file under .chimii/.
	managedDir := filepath.Join(workDir, ".chimii")
	...
	// .chimii still holds user-notes.txt, so rmdir must have been
...
// collision-free sibling (which lives under .../issue-review-chimii/)
// is fully cleaned up.
//
...
// provider, seed a user skill at the exact slug Chimii would use
// (`.claude/skills/issue-review/SKILL.md` etc.), run the full
// Prepare → Inject → Cleanup cycle with a Chimii skill of the same
// name, and assert the workdir snapshot is byte-identical to the
// seed. The user's SKILL.md must not be touched, and the Chimii
// sibling (which lives at `<slug>-chimii`) must be fully removed by
// CleanupSidecars.
		...
		t.Run(tc.provider, func(t *testing.T) {
			...
			ctx := TaskContextForEnv{
				...
				AgentSkills: []SkillContextForEnv{
					{
						Name:        "Issue Review",
						Description: "Chimii's version",
						Content:     "---\nname: issue-review\n---\n\nChimii skill content.\n",
						Files: []SkillFileContextForEnv{
							{Path: "templates/checklist.md", Content: "- [ ] check"},
						},
					},
...
// — a Chimii-only namespace file. If the user already has a file at
...
// is the matching byte-exact matrix for `.chimii/project/
// resources.json` — the other Chimii-only namespace file. Same
...
func TestPrepareThenCleanupSidecarsProjectResourcesCollisionPerProvider(t *testing.T) {
	...
	for _, provider := range allFileBasedProviders {
		...
		t.Run(provider, func(t *testing.T) {
			...
			if err := os.MkdirAll(filepath.Join(workDir, ".chimii", "project"), 0o755); err != nil {
				t.Fatalf("seed dir: %v", err)
			}
			userBody := `{"user":"owns this file"}`
			userPath := filepath.Join(workDir, ".chimii", "project", "resources.json")
			if err := os.WriteFile(userPath, []byte(userBody), 0o644); err != nil {
				t.Fatalf("seed user file: %v", err)
			}
...
// first try the natural slug, then `-chimii`, then `-chimii-2`,
// `-chimii-3`, … The PR-review concern is "Chimii skill must still
...
func TestAllocateCollisionFreeSkillDir(t *testing.T) {
	...
	// 2) Pre-existing user dir at the base slug → bump to `-chimii`.
	...
	if slug != "issue-review-chimii" {
		t.Errorf("second allocation should bump to `-chimii`; got %q", slug)
	}
	if dir != filepath.Join(parent, "issue-review-chimii") {
		t.Errorf("second allocation path = %q, want under parent", dir)
	}

	// 3) Pre-existing collision at the bumped slug too → bump again.
	if err := os.MkdirAll(filepath.Join(parent, "issue-review-chimii"), 0o755); err != nil {
	...
	if slug != "issue-review-chimii-2" {
		t.Errorf("third allocation should be `-chimii-2`; got %q", slug)
	}
	if dir != filepath.Join(parent, "issue-review-chimii-2") {
...
// `Issue Review` skill, the Chimii sibling must land at a different
// slug (so the agent still sees it), AND Cleanup must remove the
// Chimii sibling entirely without touching the user's.
func TestPrepareThenCleanupSidecarsMultiSkillCollisionFreeAllocation(t *testing.T) {
	...
	// Run only the inject side first — verify the Chimii skill
	...
	if err := writeContextFiles(workDir, "claude", TaskContextForEnv{
		IssueID: "11111111-2222-3333-4444-555555555555",
		AgentSkills: []SkillContextForEnv{
			{Name: "Issue Review", Content: "Chimii's version\n"},
		},
	}, manifest); err != nil {
		t.Fatalf("writeContextFiles: %v", err)
	}

	chimiiDir := filepath.Join(workDir, ".claude", "skills", "issue-review-chimii")
	if _, err := os.Stat(filepath.Join(chimiiDir, "SKILL.md")); err != nil {
		t.Errorf("Chimii sibling skill should exist at %s: %v", chimiiDir, err)
	}
	got, err := os.ReadFile(userFile)
	if err != nil {
	...
	// Chimii sibling is gone; user's path survives.
	if err := writeSidecarManifest(envRoot, manifest); err != nil {
		t.Fatalf("write manifest: %v", err)
	}
	if err := CleanupSidecars(envRoot); err != nil {
	...
	if _, err := os.Stat(chimiiDir); !os.IsNotExist(err) {
		t.Errorf("Chimii sibling should be removed by Cleanup; stat err=%v", err)
	}
	got, err = os.ReadFile(userFile)
	if err != nil {
