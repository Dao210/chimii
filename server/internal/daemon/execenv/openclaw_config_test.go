...
// MUL-2219 DoD calls out: the directory Chimii writes skills into MUST be
...
func TestPrepareOpenclawSkillWriteMatchesScanPath(t *testing.T) {
	...
	for _, s := range skills {
		...
		if _, err := os.Stat(want); err != nil {
			t.Errorf("openclaw scan target %s missing — Chimii's write path and the openclaw scanner are out of sync: %v", want, err)
		}
	}
}
...
//
// When a chimii agent is configured for gateway-mode openclaw and the
