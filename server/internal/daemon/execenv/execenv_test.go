...
func TestPrepareWithProjectResources(t *testing.T) {
		...
		ProjectResources: []ProjectResourceForEnv{
			{
				...
				ResourceRef:  json.RawMessage(`{"url":"https://github.com/chimii-ai/chimii","ref":"release/v2","default_branch_hint":"main"}`),
			},
		},
	}
	env, err := Prepare(PrepareParams{
	...
	resourcesPath := filepath.Join(env.WorkDir, ".chimii", "project", "resources.json")
	...
	for _, want := range []string{
		...
		"GitHub repo",
		"https://github.com/chimii-ai/chimii",
		"checkout ref: `release/v2`",
		"default branch hint: `main`",
		".chimii/project/resources.json",
	} {
...
func TestWriteProjectResourcesSkippedWhenNone(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, ".chimii", "project", "resources.json")); !os.IsNotExist(err) {
...
func TestPrepareWithRepoContext(t *testing.T) {
	...
	for _, e := range entries {
		name := e.Name()
		if name != ".agent_context" && name != ".chimii" && name != "CLAUDE.md" && name != ".claude" {
...
func TestWriteContextFilesAutopilotRunOnly(t *testing.T) {
	...
	for _, want := range []string{
		...
		"chimii autopilot get autopilot-1 --output json",
		"no assigned issue",
	} {
		if !strings.Contains(s, want) {
	...
	if strings.Contains(s, "Run `chimii issue get") {
...
// collision-free duplicates (issue-review, issue-review-chimii,
// issue-review-chimii-2, …). Reuse rolls back the prior dispatch's writes
...
func TestReuseRefreshesSkillsWithoutDuplicating(t *testing.T) {
	...
	if len(names) != 1 || names[0] != "issue-review" {
		t.Fatalf("after re-dispatch the skills dir = %v, want exactly [issue-review] with no -chimii duplicates", names)
	}
...
// next refresh dodges to issue-review-chimii. Reuse must reclaim the
...
func TestReuseReclaimsManagedSkillDirWithStrayAgentFile(t *testing.T) {
	...
	if len(names) != 1 || names[0] != "issue-review" {
		t.Fatalf("after reuse with a stray agent file the skills dir = %v, want exactly [issue-review] with no -chimii duplicate", names)
	}
...
func TestInjectRuntimeConfigClaude(t *testing.T) {
	...
	s := string(content)
	for _, want := range []string{
		"Chimii Agent Runtime",
		"chimii issue get",
		"chimii issue comment list",
		"Go Conventions",
		"PR Review",
...
func TestInjectRuntimeConfigAvailableCommandsCoreOnly(t *testing.T) {
	...
	for _, want := range []string{
		...
		"chimii issue create --title",
		"chimii issue update <id>",
		"--description-file <path>",
		"--parent \"\"",
		"chimii repo checkout <url>",
		"chimii issue status <id> <status>",
		"chimii issue comment add <issue-id>",
		"chimii issue comment add --help",
		"chimii squad member set-role <squad-id>",
	} {
		if !strings.Contains(s, want) {
	...
	for _, banned := range []string{
		"chimii issue list [--status",
		"chimii issue label list",
		"chimii issue subscriber list",
		"chimii label list",
		"chimii workspace member list",
		"chimii agent list",
		"chimii squad list",
		"chimii issue runs",
		"chimii issue run-messages",
		"chimii attachment download",
		"chimii autopilot list",
		"chimii autopilot create",
		"chimii autopilot update",
		"chimii autopilot trigger",
		"chimii autopilot delete",
		"chimii project get",
		"chimii project resource list",
		"chimii issue assign",
		"chimii issue label add",
		"chimii issue label remove",
		"chimii issue subscriber add",
		"chimii issue subscriber remove",
		"chimii issue comment delete",
		"chimii label create",
	} {
		if strings.Contains(s, banned) {
...
func TestInjectRuntimeConfigCodex(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "Chimii Agent Runtime") {
...
func TestInjectRuntimeConfigNoSkills(t *testing.T) {
	...
	s := string(content)
	if !strings.Contains(s, "chimii issue get") {
		t.Error("should reference chimii CLI even without skills")
	}
...
func TestPrepareWithRepoContextOpencode(t *testing.T) {
	...
	for _, e := range entries {
		name := e.Name()
		if name != ".agent_context" && name != ".chimii" && name != "AGENTS.md" {
			t.Errorf("unexpected entry in workdir: %s", name)
		}
	}
...
func TestInjectRuntimeConfigRequiresExplicitCommentPost(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			// The workflow must contain an explicit `chimii issue comment add`
			// invocation for this issue — not just a prose mention of posting.
			mustContain := []string{
				"chimii issue comment add issue-1",
				"mandatory",
			}
			...
			for _, want := range []string{
				"Final results MUST be delivered via `chimii issue comment add`",
...
// heredoc on `chimii issue create`, the flags were silently swallowed into
...
// non-ASCII as `?` before reaching `chimii.exe` (#2198 / #2236 / #2376).
//
// Not parallel: mutates the package-level runtimeGOOS.
...
`
	...
	for _, marker := range []string{
		chimiiManagedBeginMarker,
		chimiiMultiAgentBeginMarker,
		chimiiMemoryFeatureBeginMarker,
		chimiiMemoryConfigBeginMarker,
	} {
...
func TestEnsureCodexSandboxConfigCreatesDefaultLinux(t *testing.T) {
	...
	s := string(data)
	if !strings.Contains(s, chimiiManagedBeginMarker) || !strings.Contains(s, chimiiManagedEndMarker) {
	...
	// inherit from any surrounding table scope. See upsertChimiiManagedBlock
	// for why.
...
// workspace-write and rejects mutation commands (e.g. `chimii issue create`)
...
func TestEnsureCodexSandboxConfigIsIdempotent(t *testing.T) {
	...
	if n := strings.Count(string(data), chimiiManagedBeginMarker); n != 1 {
...
func TestEnsureCodexSandboxConfigHoistsAboveUserTables(t *testing.T) {
	...
	// permissions.chimii.sandbox_mode and Codex would never see it — see
...
`
	...
	beginIdx := strings.Index(s, chimiiManagedBeginMarker)
	endIdx := strings.Index(s, chimiiManagedEndMarker)
	tableIdx := strings.Index(s, "[permissions.chimii]")
	...
	if !(beginIdx < endIdx && endIdx < tableIdx) {
		t.Errorf("managed block must be hoisted above [permissions.chimii]; got begin=%d end=%d table=%d:\n%s", beginIdx, endIdx, tableIdx, s)
	}
	...
	if !strings.Contains(s, `trust = "always"`) {
		t.Error("lost user permissions.chimii content")
	}
	...
	if n := strings.Count(string(data2), chimiiManagedBeginMarker); n != 1 {
...
`
	...
	if beginIdx < 0 || tableIdx < 0 || beginIdx > tableIdx {
		t.Errorf("expected managed block to be hoisted above [permissions.chimii], got:\n%s", s)
	}
	if strings.Count(s, chimiiManagedBeginMarker) != 1 {
...
func TestPrepareCodexHomeFailsClosedWhenSandboxWriteFails(t *testing.T) {
	...
	stale := chimiiManagedBeginMarker + "\nsandbox_mode = \"danger-full-access\"\n" + chimiiManagedEndMarker + "\n"
...
func TestReuseRestoresCodexHome(t *testing.T) {
	...
	if !strings.Contains(string(data), chimiiManagedBeginMarker) {
		t.Error("reused config.toml missing chimii-managed block")
	}
}
...
// inside a Chimii task, despite the daemon redirecting CODEX_HOME to a
// per-task directory.
...
func TestInjectRuntimeConfigBriefKeepsStaticCatchUpRead(t *testing.T) {
	...
	if !strings.Contains(s, "chimii issue comment list "+issueID+" --recent 10 --output json") {
...
func TestInjectRuntimeConfigBriefOmitsResumedThreadAnchor(t *testing.T) {
	...
	for _, want := range []string{
		...
		"chimii issue comment list " + issueID + " --thread thread-root-1 --tail 30 --output json",
	} {
...
func TestInjectRuntimeConfigAssignmentTriggerMentionsRecent(t *testing.T) {
	...
	for _, want := range []string{
		"chimii issue comment list issue-1 --recent 10 --output json",
	...
	for _, banned := range []string{
		"chimii issue comment list issue-1 --output json",
...
func TestInjectRuntimeConfigIssueMetadataSectionScope(t *testing.T) {
	...
	coreDiscoveryLines := []string{
		...
		"chimii issue metadata set <issue-id> --key <k> --value <v> [--type string|number|bool]",
		"chimii issue metadata delete <issue-id> --key <k>",
	}
	...
	withoutSection := wantSection{
		// We can't simply require `chimii issue metadata list` absent
	...
	cases := []struct {
		...
		},
		{
			...
			filename: "CLAUDE.md",
			workflowStepPresent: []string{
				"chimii issue metadata list issue-md-2 --output json",
				"See the `## Issue Metadata` section above",
				"chimii issue metadata set",
				"chimii issue metadata delete",
				"Before exiting",
...
func TestInjectRuntimeConfigIssueMetadataCodexFormattingUnchanged(t *testing.T) {
	...
	t.Run("linux_content_file", func(t *testing.T) {
		...
		if !strings.Contains(s, "chimii issue metadata list issue-md-codex --output json") {
