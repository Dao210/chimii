...
func prepareCodexHomeWithOpts(codexHome string, opts CodexHomeOptions, logger *slog.Logger) error {
	...
	// `missing field path` and bails out of `thread/start`. Chimii writes the
	...
	// so cross-task and cross-workspace context leaks (chimii#3130) cannot
...
// enumerated by a plain `codex` run, keeping Chimii task history out of the
// user's own thread list.
const codexSessionStoreRoot = "chimii-sessions"
...
// shared home's chimii-sessions root that have not been touched within
