...
func Run(ctx context.Context, cases []Case) Report {
	...
	for _, c := range cases {
		...
		if err := ctx.Err(); err != nil {
			...
			start := time.Now()
			result, err := build.CompileDesign(ctx, *c.Recipe, c.Inventory, build.CatalogVersion, build.StarterCatalog, time.Unix(0, 0))
			...
			if len(result.Plan.Placements) > 0 {
				...
				start = time.Now()
				checked := build.ValidateWithCatalog(x.Placements, c.Inventory, build.StarterCatalog)
			...
			start := time.Now()
			v := build.ValidateWithCatalog(c.Placements, c.Inventory, build.StarterCatalog)
...
func checkDesign(recipe build.AssemblyRecipe, plan build.BuildPlan) []string {
	...
	actual := map[build.DesignVector]int{}
	...
	for q, cell := range target {
		color, ok := actual[q]
		if !ok || (recipe.Constraints.ExactColors && color != cell.Color) {
