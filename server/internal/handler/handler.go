...
import (
	...
	composio "github.com/chimii-ai/chimii/server/internal/integrations/composio"
...
type Config struct {
	...
	// CHIMII_VCS_SECRET_KEY is set. It is the product boundary: the feature is
	// intended for self-hosted Chimii only (where Chimii and the Git instance
	...
	// CHIMII_TRUSTED_PROXIES env var (comma-separated CIDRs, e.g.
...
type Handler struct {
	...
	// CHIMII_LARK_SECRET_KEY is set; tests that need a no-op
	...
	// (Feishu when CHIMII_LARK_SECRET_KEY is set). The router does NOT
	...
	// + app tokens (MUL-3666). Nil unless CHIMII_SLACK_SECRET_KEY is set.
	...
	// configured (CHIMII_SLACK_SECRET_KEY set).
	SlackBindingTokens *slack.BindingTokenService
	// SlackHistory backs the agent-facing `chimii chat history` command: it
	...
	// CHIMII_VCS_SECRET_KEY is unset; the connect/webhook handlers return 503
