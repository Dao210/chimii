...
import (
	...
	"github.com/chimii-ai/chimii/server/internal/attribution"
...
type AgentTaskResponse struct {
	...
	ChatChannelType          string                 `json:"chat_channel_type,omitempty"`           // "slack" when the chat session is backed by an IM channel; empty for a web-only chat. Makes the agent channel-aware (read history from the channel, not Chimii)
	ChatInThread             bool                   `json:"chat_in_thread,omitempty"`              // true when the latest @mention was a thread reply; tells the agent to start with `chimii chat thread` vs `chimii chat history`
	ChatMessage              string                 `json:"chat_message,omitempty"`                // user message for chat tasks
	ChatMessageAttachments   []ChatAttachmentMeta   `json:"chat_message_attachments,omitempty"`    // attachments on the user message — agent calls `chimii attachment download <id>` per entry
	...
	// the owner. The agent's effective Chimii credentials stay owner-scoped —
	...
	// CHIMII_TOKEN in the agent process environment. The server binds it to
...
// `chimii attachment download <id>` rather than guessing from the
...
func (h *Handler) UpdateAgent(w http.ResponseWriter, r *http.Request) {
	...
	if _, ok := rawFields["custom_env"]; ok {
		writeError(w, http.StatusBadRequest, "custom_env is no longer accepted on this endpoint; use PUT /api/agents/{id}/env (or `chimii agent env set`)")
		return
	}
