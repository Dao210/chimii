...
func privateAgentTestFixture(t *testing.T) (agentID, ownerID, memberID string) {
	...
	if err := testPool.QueryRow(ctx, `
		...
		VALUES ('Private Agent Owner', 'private-agent-owner@chimii.test')
		RETURNING id
	...
	t.Cleanup(func() {
		testPool.Exec(context.Background(),
			`DELETE FROM "user" WHERE email = 'private-agent-owner@chimii.test'`)
	})
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO "user" (name, email)
		VALUES ('Plain Member', 'plain-member@chimii.test')
		RETURNING id
	...
	t.Cleanup(func() {
		testPool.Exec(context.Background(),
			`DELETE FROM "user" WHERE email = 'plain-member@chimii.test'`)
	})
...
func TestMentionAgent_RejectsCrossWorkspaceAgentUUID(t *testing.T) {
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO "user" (name, email)
		VALUES ('Foreign Owner', 'cross-ws-foreign@chimii.test')
		RETURNING id
	...
	t.Cleanup(func() {
		testPool.Exec(context.Background(),
			`DELETE FROM "user" WHERE email = 'cross-ws-foreign@chimii.test'`)
	})
	...
	// Chimii's mention format is markdown-linked: [@Name](mention://agent/<uuid>).
