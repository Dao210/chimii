...
// channel table-of-contents (`chimii chat history`), Thread reads one thread's
// messages (`chimii chat thread [id]`). Both are scoped server-side to the
...
// GetChatChannelHistory serves `chimii chat history` — the channel overview:
// recent top-level messages, each thread tagged with its id + reply count (no
// thread contents). The agent drills into a thread with `chimii chat thread`.
...
// GetChatThread serves `chimii chat thread [id]` — one thread's messages. With
