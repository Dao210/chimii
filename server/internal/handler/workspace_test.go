...
VALUES ($1, 123456789, 'chimii-ai', 'chimii', 3366, 987654321, 'abc123', 15368, 'completed', now())
`, wsID); err != nil {
...
VALUES ($1, 123456789, 'chimii-ai', 'chimii', 5265,
	'Workspace cleanup snapshot', 'open', 'https://github.com/chimii-ai/chimii/pull/5265',
	now(), now(), 'head-a')
RETURNING id
...
`, wsID, testUserID); err != nil {
	...
	t.Run("normalizes valid repos", func(t *testing.T) {
		...
		req := newRequest("PATCH", "/api/workspaces/"+wsID, map[string]any{
			"repos": []map[string]any{
				{
					"url":         "  https://github.com/chimii-ai/chimii.git  ",
				...
				{
					"url": "git@github.com:chimii-ai/chimii-cloud.git",
				},
			},
		})
		req = withURLParam(req, "id", wsID)
		...
		if repos[0].URL != "https://github.com/chimii-ai/chimii.git" || repos[0].Description != "main monorepo" {
			t.Fatalf("first repo not normalized: %+v", repos[0])
		}
		if repos[1].URL != "git@github.com:chimii-ai/chimii-cloud.git" {
...
`, wsID, testUserID); err != nil {
	...
	targetEmail := fmt.Sprintf("revocation-%s@chimii.ai", slug)
...
VALUES ($1, $2, 'Target Runtime', 'local', 'chimii_daemon', 'online', '', '{}'::jsonb, $3, now())
RETURNING id
...
`, fx.WorkspaceID, fx.AgentID, appID, testUserID).Scan(&installID); err != nil {
	...
	if _, err := testPool.Exec(ctx, `
INSERT INTO channel_user_binding (workspace_id, chimii_user_id, installation_id, channel_type, channel_user_id)
...
VALUES ($1, $2, 'Other Runtime', 'local', 'chimii_daemon', 'online', '', '{}'::jsonb, $3, now())
RETURNING id
...
`, wsID, testUserID); err != nil {
	...
	if err := testPool.QueryRow(ctx, `
INSERT INTO "user" (name, email) VALUES ($1, $2) RETURNING id
`, "Revocation No Runtimes Target", "revocation-no-runtimes@chimii.ai").Scan(&targetUserID); err != nil {
		t.Fatalf("create target user: %v", err)
	}
	t.Cleanup(func() {
		_, _ = testPool.Exec(context.Background(), `DELETE FROM workspace WHERE id = $1`, wsID)
