...
type AppConfig struct {
	...
	// self-hosted instances can show `chimii setup self-host` commands
	// with the operator's own domains instead of Chimii Cloud defaults.
	DaemonServerURL string `json:"daemon_server_url,omitempty"`
	DaemonAppURL    string `json:"daemon_app_url,omitempty"`

	// VCSIntegrationAvailable mirrors the CHIMII_VCS_INTEGRATION_ENABLED
	...
	// CHIMII_VCS_SECRET_KEY" hint a cloud user cannot resolve. Omitted when
...
func daemonSetupURLsFromEnv() (string, string) {
	serverURL := normalizePublicURL(os.Getenv("CHIMII_PUBLIC_URL"))
...
// (CHIMII_APP_URL, falling back to FRONTEND_ORIGIN), normalized. Shared by
// the daemon-setup URLs and the managed-cloud detection so both read the same
// signal.
func resolveFrontendAppURL() string {
	appURL := normalizePublicURL(os.Getenv("CHIMII_APP_URL"))
	if appURL == "" {
...
// Chimii Cloud, identified by its frontend host alone (chimii.ai). The
// daemon setup for the managed cloud is always
// `chimii setup` (which hardcodes api.chimii.ai), so the per-deployment URLs
// must be omitted from /api/config even when CHIMII_PUBLIC_URL is unset or
// misconfigured. Previously this also required serverURL==api.chimii.ai, so a
// cloud deployment that forgot CHIMII_PUBLIC_URL fell through and emitted a
// `setup self-host --server-url https://chimii.ai` command — pointing the
// daemon's backend at the frontend (no /health, no WebSocket proxy).
func isOfficialCloudDaemonConfig(appURL string) bool {
	return urlHostEquals(appURL, "chimii.ai")
}

// isOfficialCloudDeployment reports whether this server is the official Chimii
// Cloud, reusing the same frontend-host signal as the daemon setup (chimii.ai).
