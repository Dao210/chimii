...
func TestJoinCloudWaitlistRecordsEmailAndReason(t *testing.T) {
	userID := newWaitlistTestUser(t, "waitlist-ok@chimii.ai")
...
func TestJoinCloudWaitlistAllowsEmptyReason(t *testing.T) {
	userID := newWaitlistTestUser(t, "waitlist-noreason@chimii.ai")
...
func TestJoinCloudWaitlistMissingEmailReturns400(t *testing.T) {
	userID := newWaitlistTestUser(t, "waitlist-missing@chimii.ai")
...
func TestJoinCloudWaitlistRejectsOverlongReason(t *testing.T) {
	userID := newWaitlistTestUser(t, "waitlist-long@chimii.ai")
...
func TestJoinCloudWaitlistSecondCallOverwrites(t *testing.T) {
	userID := newWaitlistTestUser(t, "waitlist-overwrite@chimii.ai")
...
func TestBootstrapOnboardingRuntimeCreatesSingleGuideIssue(t *testing.T) {
	...
	if avatarURL == nil || *avatarURL != onboardingAssistantAvatarURL {
		t.Fatalf("agent avatar_url = %v, want seeded Chimii Helper avatar", avatarURL)
	}
...
func TestBootstrapOnboardingRuntime_WithStarterPrompt(t *testing.T) {
	...
	const wantPrompt = "Introduce Chimii to me, please."
	body := map[string]string{
		"workspace_id":   testWorkspaceID,
		"runtime_id":     testRuntimeID,
...
func TestPatchOnboardingSplitsQuestionnaireAndSourceEvents(t *testing.T) {
	userID := newWaitlistTestUser(t, "onboarding-event-split@chimii.ai")

	m := obsmetrics.NewBusinessMetrics()
	h := *testHandler
	h.Metrics = m

	const questionnaireCounter = "chimii_onboarding_questionnaire_submitted_total"
	const sourceCounter = "chimii_onboarding_source_submitted_total"
...
func TestPatchOnboardingAllSkippedResolvesBothCounters(t *testing.T) {
	userID := newWaitlistTestUser(t, "onboarding-source-skip@chimii.ai")
	...
	if got := counterValue(t, m, "chimii_onboarding_questionnaire_submitted_total"); got != 1 {
		t.Fatalf("expected questionnaire counter 1 for all-skip, got %v", got)
	}
	if got := counterValue(t, m, "chimii_onboarding_source_submitted_total"); got != 1 {
