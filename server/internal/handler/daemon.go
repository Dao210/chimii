...
type DaemonRegisterRequest struct {
	...
	CLIVersion      string   `json:"cli_version"` // chimii CLI version
...
func (h *Handler) buildClaimedTaskResponse(ctx context.Context, capabilities string, task *db.AgentTaskQueue, runtime db.AgentRuntime, runtimeID, runtimeWorkspaceID string) (resp AgentTaskResponse, deliveredCommentIDs []pgtype.UUID, agentSkillCount, builtinSkillCount int, failure *claimBuildFailure) {
			...
			if issue.ProjectID.Valid {
				...
				if rows := h.listProjectResourcesForProject(r.Context(), issue.ProjectID); len(rows) > 0 {
					...
					for _, row := range rows {
						...
						// so `chimii repo checkout` and the meta-skill render
	...
	if task.ChatSessionID.Valid {
		if cs, err := h.Queries.GetChatSession(r.Context(), task.ChatSessionID); err == nil {
			...
			// is operating inside an IM conversation and not the Chimii web app
			...
			// mis-flag made the brief inject `chimii attachment upload` guidance
			...
			// `chimii chat history` and `chimii chat thread`, and those two
			...
			// `chimii attachment download <id>` (the inline markdown URL is
	...
	if task.Context != nil && !task.IssueID.Valid && !task.ChatSessionID.Valid && !task.AutopilotRunID.Valid {
		...
		if json.Unmarshal(task.Context, &qc) == nil && qc.Type == service.QuickCreateContextType {
			...
			// the project, and `chimii repo checkout` sees the project's
	...
	// as the only authority for CHIMII_WORKSPACE_ID in the agent env. An
...
func (h *Handler) ClaimTaskByRuntime(w http.ResponseWriter, r *http.Request) {
	...
	// owner). The daemon will inject this as CHIMII_TOKEN into the agent
