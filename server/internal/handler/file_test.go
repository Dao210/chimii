...
func TestBuildMarkdownURL_PublicCdnAbsoluteURLReusedVerbatim(t *testing.T) {
	...
	testHandler.cfg.PublicURL = "https://api.chimii.test"
	testHandler.CFSigner = nil
	...
	id := seedAttachmentURL(t, "https://cdn.chimii.test/uploads/abc.png", "abc.png", "image/png", 1)
	att, err := testHandler.Queries.GetAttachment(context.Background(), db.GetAttachmentParams{
		ID:          parseUUID(id),
		WorkspaceID: parseUUID(testWorkspaceID),
	})
	...
	resp := testHandler.attachmentToResponse(att)
	if resp.MarkdownURL != "https://cdn.chimii.test/uploads/abc.png" {
...
func TestBuildMarkdownURL_PrivateBucketWithoutCdnDomainRoutesThroughAPIEndpoint(t *testing.T) {
	...
	resp := testHandler.attachmentToResponse(att)
	want := "https://api.chimii.test/api/attachments/" + id + "/download"
...
func TestIsDurablePublicURL(t *testing.T) {
	cases := []struct {
		...
		{"absolute https no signature", "https://cdn.chimii.test/foo.png", true},
		{"absolute http no signature", "http://cdn.chimii.test/foo.png", true},
