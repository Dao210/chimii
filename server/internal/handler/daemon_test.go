...
// `chimii issue comment add` call) left the user staring at a "Completed"
// badge with no reply.
...
func TestClaimTask_ChatPopulatesInitiator(t *testing.T) {
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO "user" (name, email) VALUES ('Installer User', 'installer-test@chimii.ai')
		RETURNING id
...
func createEphemeralUser(t *testing.T, label string) string {
	t.Helper()
	email := fmt.Sprintf("membership-cache-%s-%s@chimii.ai", label, uuid.NewString())
	var userID string
	if err := testPool.QueryRow(context.Background(), `
