...
type AttachmentResponse struct {
	...
	// CHIMII_LOCAL_UPLOAD_BASE_URL), and otherwise prefixing
	// CHIMII_PUBLIC_URL onto the stable per-attachment endpoint that the
...
//     proxies on every request, anchored on `CHIMII_PUBLIC_URL` so the
//     persisted URL keeps working for clients that don't share the
//     document origin (Desktop / mobile webview).
//
//  3. Last-resort fallback (no `CHIMII_PUBLIC_URL` configured): emit
