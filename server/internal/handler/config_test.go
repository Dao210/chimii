...
func TestGetConfigIncludesRuntimeAuthConfig(t *testing.T) {
	...
	t.Setenv("CHIMII_PUBLIC_URL", "https://api.example.com/")
	t.Setenv("CHIMII_APP_URL", "https://app.example.com/")
...
func TestGetConfigUsesAppURLForSameOriginDaemonSetup(t *testing.T) {
	t.Setenv("CHIMII_PUBLIC_URL", "")
	t.Setenv("CHIMII_APP_URL", "https://chimii.internal.example/")
	...
	if cfg.DaemonServerURL != "https://chimii.internal.example" {
		t.Fatalf("daemon_server_url: want same-origin URL, got %q", cfg.DaemonServerURL)
	}
	if cfg.DaemonAppURL != "https://chimii.internal.example" {
...
func TestGetConfigUsesFrontendOriginForSameOriginDaemonSetup(t *testing.T) {
	t.Setenv("CHIMII_PUBLIC_URL", "")
	t.Setenv("CHIMII_APP_URL", "")
	t.Setenv("FRONTEND_ORIGIN", "https://chimii.internal.example/")
...
func TestGetConfigOmitsOfficialCloudDaemonSetup(t *testing.T) {
	t.Setenv("CHIMII_PUBLIC_URL", "https://api.chimii.ai")
	t.Setenv("CHIMII_APP_URL", "")
	t.Setenv("FRONTEND_ORIGIN", "https://chimii.ai")
...
// frontend is chimii.ai, but the deployment does not set CHIMII_PUBLIC_URL to
// the api host. Previously this fell through to the same-origin branch and
// emitted daemon_server_url=https://chimii.ai, which the dialog turned into
// `chimii setup self-host --server-url https://chimii.ai` — pointing the
...
// setup URLs are omitted and the dialog falls back to `chimii setup`.
...
// cloud frontend when it is configured through CHIMII_APP_URL.
func TestGetConfigOmitsCloudDaemonSetupForConfiguredAppURL(t *testing.T) {
	t.Setenv("CHIMII_PUBLIC_URL", "")
	t.Setenv("CHIMII_APP_URL", "https://chimii.ai")
...
func TestURLHostEqualsCanonicalizesCommonHostForms(t *testing.T) {
	tests := []struct {
		...
		{name: "full URL", raw: "https://api.chimii.ai", want: true},
		{name: "bare host", raw: "api.chimii.ai", want: true},
		{name: "host port", raw: "api.chimii.ai:8080", want: true},
		{name: "trailing dot", raw: "https://api.chimii.ai.", want: true},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := urlHostEquals(tt.raw, "api.chimii.ai"); got != tt.want {
...
func TestGetConfigExposesServerVersion(t *testing.T) {
	...
	t.Setenv("CHIMII_APP_URL", "https://chimii.self-hosted.example")
	t.Setenv("FRONTEND_ORIGIN", "")
...
// suppressed on the managed cloud (frontend host chimii.ai) even when the
...
func TestGetConfigOmitsServerVersionOnOfficialCloud(t *testing.T) {
	...
	// Official cloud: frontend host chimii.ai -> version omitted.
	t.Setenv("CHIMII_APP_URL", "https://chimii.ai")
	t.Setenv("FRONTEND_ORIGIN", "")
	w := httptest.NewRecorder()
	testHandler.GetConfig(w, req)
