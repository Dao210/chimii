...
func TestLarkOutcomeReplierUsesAppURLForWebLinks(t *testing.T) {
	...
	if got := stub.bindingCalls[0].BindURL; got != "https://app.chimii.test/lark/bind?token=token+with+space" {
	...
	if !strings.Contains(stub.textOut[0].Text, "https://app.chimii.test/issues/MUL-42") {
...
func TestLarkOutcomeReplierIssueCreatedSendsConfirmation(t *testing.T) {
	...
	if !strings.Contains(got.Text, "https://chimii.test/issues/MUL-42") {
		t.Errorf("text should embed the deep link back to Chimii; got %q", got.Text)
	}
