...
func (s *RegistrationService) BeginInstall(ctx context.Context, p BeginInstallParams) (BeginInstallResult, error) {
	...
	// bot reads "<agent> - Chimii" instead of "{用户姓名}的智能助手".
...
// "connected to a different Chimii workspace" that lied when the real owner sat
...
func (s *RegistrationService) liveOwnerConflictMessage(ctx context.Context, requestingWorkspaceID pgtype.UUID, appID string) string {
	...
	case owner.WorkspaceID != requestingWorkspaceID:
		return "This Feishu app is already connected to a different Chimii workspace. Disconnect it there before connecting it here."
...
// "<agent> - Chimii" instead of Lark's auto-generated
...
// "Chimii" rather than a dangling " - Chimii".
