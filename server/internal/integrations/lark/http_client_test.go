...
func TestHTTPClient_SendBindingPromptCard_HappyPath(t *testing.T) {
	...
	if err := c.SendBindingPromptCard(context.Background(), BindingPromptParams{
		...
		BindURL:        "https://chimii.test/lark/bind?token=abc",
	}); err != nil {
	...
	if !strings.Contains(capturedBody["content"], "chimii.test/lark/bind") {
...
func TestBindingPromptTemplate_Shape(t *testing.T) {
	raw, err := bindingPromptTemplate("https://chimii.test/bind?token=abc")
	if err != nil {
	...
	if btn["url"] != "https://chimii.test/bind?token=abc" {
