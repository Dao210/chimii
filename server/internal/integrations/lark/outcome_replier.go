...
type LarkOutcomeReplier struct {
	...
	appURL       string // e.g. https://chimii.example, trailing slash trimmed
...
// OutcomeReplierConfig wires the production replier. AppURL is the Chimii web
// app host the user clicks into to redeem the binding token or open an issue
// (e.g. https://chimii.example). It comes from CHIMII_APP_URL and is
// intentionally separate from CHIMII_PUBLIC_URL, which is the backend/API
...
func NewLarkOutcomeReplier(cfg OutcomeReplierConfig) OutcomeReplier {
	...
	if cfg.AppURL == "" {
		log.Warn("lark outcome replier: CHIMII_APP_URL not set; binding prompt CTA will not work")
	}
...
// after MUL-2671's plain-text refactor. The link to Chimii is
...
// when empty (self-host operators who haven't configured CHIMII_APP_URL) the
