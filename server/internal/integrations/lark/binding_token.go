...
type InstallerBindParams struct {
	WorkspaceID    pgtype.UUID
	InstallationID pgtype.UUID
	ChimiiUserID   pgtype.UUID // the installer's Chimii account
...
// identity is the supplied chimiiUserID (taken from the session by
...
//     (installation, open_id), pointing at a DIFFERENT Chimii user.
...
func (s *BindingTokenService) RedeemAndBind(ctx context.Context, raw string, chimiiUserID pgtype.UUID) (RedeemedBindingToken, error) {
	if s.tx == nil {
	...
	isMember, err := qtx.IsWorkspaceMember(ctx, row.WorkspaceID, chimiiUserID)
	if err != nil {
	...
	_, err = qtx.CreateLarkUserBinding(ctx, CreateUserBindingParams{
		...
		ChimiiUserID:   chimiiUserID,
		InstallationID: row.InstallationID,
		ChannelUserID:  row.ChannelUserID,
	})
	if err != nil {
		// pgx.ErrNoRows here means the conflict row exists but its
		// chimii_user_id differs from ours, so the WHERE clause on
...
// (installation_id, lark_open_id) when chimii_user_id matches (the
...
func (s *BindingTokenService) BindInstallerTx(ctx context.Context, qtx *ChannelStore, p InstallerBindParams) error {
	...
	isMember, err := q.IsWorkspaceMember(ctx, p.WorkspaceID, p.ChimiiUserID)
	if err != nil {
...
// open_id) pair and points at a different Chimii user. Account
