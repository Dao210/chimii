...
func TestRegistrationClient_Begin_HappyPath(t *testing.T) {
	...
	res, err := c.Begin(context.Background(), "Ada - Chimii", "")
	if err != nil {
	...
	if !strings.HasPrefix(q.Get("source"), "go-sdk/chimii") {
		t.Errorf("qr source=%q want go-sdk/chimii", q.Get("source"))
	}
	// The name preset pre-fills the Lark PersonalAgent creation form so
	// the bot defaults to "<agent> - Chimii" rather than the
	// auto-generated "{用户姓名}的智能助手".
	if q.Get("name") != "Ada - Chimii" {
		t.Errorf("qr name=%q want %q", q.Get("name"), "Ada - Chimii")
	}
}
