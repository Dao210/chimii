// Package composio is the Stage 2 business-integration glue between Chimii and
// the standalone Composio SDK (server/pkg/composio). It owns Chimii semantics:
...
import (
	...
	sdk "github.com/chimii-ai/chimii/server/pkg/composio"
...
type Config struct {
	...
	// trailing slash (e.g. "https://chimii.ai"). The Composio callback
	...
	// browser redirect (e.g. "https://chimii.ai"). May be empty, in which
...
//
// The composio_user_id sent to Composio is the Chimii user id verbatim — the
...
func (s *Service) CompleteCallback(ctx context.Context, state, status, connectedAccountID string) (string, error) {
	...
	// composio_user_id == chimii user id invariant) and was created under the
	...
	if _, err := s.store.UpsertUserComposioConnection(ctx, db.UpsertUserComposioConnectionParams{
		...
		// Invariant: composio_user_id == Chimii user id.
