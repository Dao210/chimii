...
// The agent turns the prompt into a well-formed `chimii issue create` in the
...
// completion surfaces to the invoker as a Chimii inbox notification through the
...
const (
	...
	slashQueuedText          = "✅ On it — I'm turning that into an issue. You'll get a Chimii notification when it's ready."
	slashNotMemberText       = "You're not a member of this Chimii workspace, so I can't file an issue for you."
	slashLinkAccountFallback = "Link your Slack account to Chimii first, then try `/issue` again."
	slashInternalErrorText   = "⚠️ Something went wrong creating the issue. Please try again."
	slashDisabledText        = "This Slack app isn't connected to Chimii (or was disconnected). Ask a workspace admin to reconnect it."
)
...
// resolveUser maps the Slack user id to the bound Chimii user, re-checking
...
func (p *SlashCommandProcessor) resolveUser(ctx context.Context, inst engine.ResolvedInstallation, slackUserID string) (pgtype.UUID, error) {
	...
	return binding.ChimiiUserID, nil
}
...
func (p *SlashCommandProcessor) bindingText(ctx context.Context, inst engine.ResolvedInstallation, slackUserID string) string {
	...
	return "👋 To file issues, link your Slack account to Chimii: <" +
