...
// Slack user id is bound to their Chimii account. It mirrors
...
var (
	...
	// different Chimii user (account transfer must go through explicit unbind).
...
// chimiiUserID (taken from the session, never from the token). Returns
...
func (s *BindingTokenService) RedeemAndBind(ctx context.Context, raw string, chimiiUserID pgtype.UUID) (RedeemedBindingToken, error) {
	...
	if _, err := qtx.CreateChannelUserBinding(ctx, db.CreateChannelUserBindingParams{
		...
		// the ON CONFLICT DO UPDATE WHERE chimii_user_id=… gating rejected it.
		if errors.Is(err, pgx.ErrNoRows) {
