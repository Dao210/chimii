...
// failed) read so the unified `chimii chat history` / `chimii chat thread`
...
// `chimii chat history` (channel overview) and `chimii chat thread [id]`
...
// boundary for `chimii chat thread <id>` (the agent supplies only a
...
// drills into with `chimii chat thread <id>`. Backs `chimii chat history`.
...
// threads) reads its linear conversation. Backs `chimii chat thread [id]`.
...
// agent can drill in with `chimii chat thread <id>`.
