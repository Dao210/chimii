...
func PlanConversation(ctx context.Context, g TextGenerator, c Catalog, input any) (ConversationDecision, error) {
	...
	instructions := `Help a child discuss an electronic-block project within the kit capabilities below. Return exactly one JSON object. The ONLY allowed fields are outcome, project_id, composition, title, message, question; omit fields that do not apply.
Choose ONE response form: {"outcome":"ready","project_id":"listed project ID","title":"short title"}, {"outcome":"reply","project_id":"existing project ID","message":"answer"}, {"outcome":"clarify","question":"one essential question"}, or {"outcome":"unsupported","message":"specific missing capability"}. When permitted below, replace project_id with composition. reply is an outcome value, never a field name.
