...
func TestRedisClientName(t *testing.T) {
	tests := []struct {
		...
		{"empty_suffix_returns_existing", "chimii-api:store", "", "chimii-api:store"},
		{"empty_existing_uses_default_prefix", "", "store", "chimii-api:store"},
...
func TestNewNamedRedisClient_SetsClientName(t *testing.T) {
	...
	opts := client.Options()
	if opts.ClientName != "chimii-api:store" {
		t.Errorf("ClientName = %q, want %q", opts.ClientName, "chimii-api:store")
	}
}
...
func TestNewNamedRedisClient_DisableClientName_InvalidValue(t *testing.T) {
	...
	if opts.ClientName != "chimii-api:store" {
		t.Errorf("ClientName = %q, want %q (invalid env should fall back to naming enabled)", opts.ClientName, "chimii-api:store")
	}
}
...
func TestServerAddr(t *testing.T) {
	tests := []struct {
		...
		{name: "empty host preserves all interfaces", port: "8080", want: ":8080"},
		{name: "ipv4 loopback", host: "127.0.0.1", port: "18080", want: "127.0.0.1:18080"},
		{name: "ipv6 loopback", host: "::1", port: "18080", want: "[::1]:18080"},
		{name: "host whitespace is trimmed", host: " localhost ", port: "8080", want: "localhost:8080"},
	}
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := serverAddr(tt.host, tt.port); got != tt.want {
				t.Fatalf("serverAddr(%q, %q) = %q, want %q", tt.host, tt.port, got, tt.want)
			}
		})
	}
}
