...
import (
	...
	"github.com/chimii-ai/chimii/server/internal/scheduler"
...
func main() {
	...
	if os.Getenv("CHIMII_DEV_VERIFICATION_CODE") != "" {
		if strings.EqualFold(strings.TrimSpace(os.Getenv("APP_ENV")), "production") {
			slog.Warn("CHIMII_DEV_VERIFICATION_CODE is set but ignored because APP_ENV=production.")
		} else {
			slog.Warn("CHIMII_DEV_VERIFICATION_CODE is enabled. Use it only for local development or private test instances.")
		}
	}
	...
	shutdownHoldDuration := envNonNegativeDuration("CHIMII_SHUTDOWN_HOLD_DURATION", 0)

	// Feature flags: loaded once at startup from CHIMII_FEATURE_FLAGS_FILE
	...
	srv := &http.Server{
		Addr:    serverAddr(os.Getenv("CHIMII_BIND_HOST"), port),
		Handler: r,
	}
