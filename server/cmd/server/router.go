...
import (
	...
	"github.com/chimii-ai/chimii/server/internal/integrations/channel"
	...
	"github.com/chimii-ai/chimii/server/internal/util/secretbox"
	composiosdk "github.com/chimii-ai/chimii/server/pkg/composio"
	db "github.com/chimii-ai/chimii/server/pkg/db/generated"
	"github.com/chimii-ai/chimii/server/pkg/featureflag"
)
...
// CHIMII_APP_URL and falls back to FRONTEND_ORIGIN, matching how the backend
// resolves the app URL elsewhere (handler.daemonSetupURLsFromEnv) and the CLI
// login flow (cmd/chimii tryResolveAppURL). Empty when neither is set.
func appURLFromEnv() string {
	if v := strings.TrimRight(strings.TrimSpace(os.Getenv("CHIMII_APP_URL")), "/"); v != "" {
		return v
	}
...
// CHIMII_TRUSTED_PROXIES env var. Invalid entries are dropped with a single
...
func parseTrustedProxies(raw string) []netip.Prefix {
	...
	for _, part := range strings.Split(raw, ",") {
		...
		if err != nil {
			slog.Warn("CHIMII_TRUSTED_PROXIES: ignoring invalid CIDR",
				"value", s, "error", err)
			continue
		}
		out = append(out, p)
...
// a build that does not inject VERSION.
...
func NewRouterWithOptions(pool *pgxpool.Pool, hub *realtime.Hub, bus *events.Bus, analyticsClient analytics.Client, rdb *redis.Client, opts RouterOptions) (chi.Router, *handler.Handler) {
	...
	signupConfig := handler.Config{
		...
		CloudRuntimeFleetTimeout: envDuration("CHIMII_CLOUD_FLEET_TIMEOUT", 35*time.Second),
	...
	// Lark integration. Only wired when CHIMII_LARK_SECRET_KEY is set:
	...
	if larkKey, err := secretbox.LoadKey("CHIMII_LARK_SECRET_KEY"); err == nil {
		...
		if err != nil {
			...
			if err != nil {
				...
				// CHIMII_LARK_SECRET_KEY is the operator's opt-in for
				...
				// real Lark traffic can point CHIMII_LARK_HTTP_BASE_URL
				// at a mock server.
				//
				// CHIMII_LARK_HTTP_BASE_URL is an OPTIONAL deployment-wide
				...
				larkClient := lark.NewHTTPAPIClient(lark.HTTPClientConfig{
					BaseURL: strings.TrimSpace(os.Getenv("CHIMII_LARK_HTTP_BASE_URL")),
					Logger:  slog.Default(),
				})
				...
				// to initialize (bad CHIMII_LARK_CALLBACK_BASE_URL or
				...
				go lark.BackfillRegionFromLegacyOverride(context.Background(), cs,
					strings.TrimSpace(os.Getenv("CHIMII_LARK_HTTP_BASE_URL")),
					strings.TrimSpace(os.Getenv("CHIMII_LARK_CALLBACK_BASE_URL")),
					slog.Default())
				...
				// CHIMII_LARK_REGISTRATION_DOMAIN / _LARK_DOMAIN env
				// vars override the protocol hosts for staging / dev.
				regCfg := lark.RegistrationConfig{
					Domain:     strings.TrimSpace(os.Getenv("CHIMII_LARK_REGISTRATION_DOMAIN")),
					LarkDomain: strings.TrimSpace(os.Getenv("CHIMII_LARK_REGISTRATION_LARK_DOMAIN")),
				}
		...
		slog.Info("lark integration disabled (CHIMII_LARK_SECRET_KEY not set)")
	}

	// Slack integration. Multi-tenant B2 model (MUL-3666): Chimii hosts ONE
	...
	//   - CHIMII_SLACK_SECRET_KEY decrypts the per-installation bot token
	...
	//     Slack at all.
	//   - CHIMII_SLACK_APP_TOKEN is the app-level token (xapp-) authorizing the
	...
	if slackKey, err := secretbox.LoadKey("CHIMII_SLACK_SECRET_KEY"); err == nil {
		...
		if err != nil {
			...
			// the Slack user to their Chimii account.
			...
			slackReplier := slack.NewOutboundReplier(slack.OutboundReplierConfig{
				...
				// app URL (CHIMII_APP_URL ?? FRONTEND_ORIGIN), NOT CHIMII_PUBLIC_URL
			...
			// On-demand history reader behind the unified `chimii chat history`
			...
			// and a Chimii notification when the issue lands.
		...
		slog.Info("slack integration disabled (CHIMII_SLACK_SECRET_KEY not set)")
	}
	...
	// (COMPOSIO_CALLBACK_BASE_URL, falling back to CHIMII_PUBLIC_URL). The
	...
	if composioAPIKey := strings.TrimSpace(os.Getenv("COMPOSIO_API_KEY")); composioAPIKey != "" {
		if !featureflags.ComposioMCPAppsEnabled(context.Background(), opts.FeatureFlags) {
			...
			if err != nil {
				...
				case callbackBase == "":
					slog.Error("composio: no callback base url (set COMPOSIO_CALLBACK_BASE_URL or CHIMII_PUBLIC_URL); composio integration disabled")
				default:
	...
	if vcsKey, err := secretbox.LoadKey("CHIMII_VCS_SECRET_KEY"); err == nil {
		...
		slog.Info("vcs integration disabled (CHIMII_VCS_SECRET_KEY not set)")
	}
	...
	// Cloud PAT verifier: validates mcn_ tokens against Chimii Cloud
	...
	// Reuses CHIMII_CLOUD_FLEET_URL (the same URL the cloud-runtime
	...
	// Share the same trusted-proxy CIDRs (CHIMII_TRUSTED_PROXIES) so the
	...
	// GitHub App webhook (no Chimii auth — requests are authenticated via
	// HMAC-SHA256 signature in the handler) and post-install setup callback.
	r.Post("/api/webhooks/github", h.HandleGitHubWebhook)
	r.Get("/api/github/setup", h.GitHubSetupCallback)
	// Slack OAuth callback (no Chimii auth in the path — it is hit by Slack's
	...
	// VCS webhook for token-based providers (Forgejo / Gitea / GitLab). No Chimii
	...
	// Stripe webhook (no Chimii auth — Stripe signs the raw body
	// with a shared secret, the chimii-cloud upstream verifies. We
	...
	r.Group(func(r chi.Router) {
		...
		// cloud-runtime — chimii-cloud's Fleet and Billing share
		...
		r.Group(func(r chi.Router) {
			...
			r.Route("/api/build", func(r chi.Router) {
				...
				r.Get("/catalog/parts", h.ListBuildCatalogParts)
				...
				r.Get("/conversations/{conversationID}", h.GetBuildConversation)
				r.Post("/conversations/{conversationID}/messages", h.CreateBuildSession)
				r.Get("/inventions", h.ListInventions)
...
// CHIMII_LARK_CALLBACK_BASE_URL), we log and fall back to the
...
func buildLarkConnector(installSvc *lark.InstallationService, apiClient lark.APIClient) (lark.EventConnector, string) {
	endpointFetcher, err := lark.NewHTTPConnectionTokenFetcher(lark.HTTPConnectionTokenConfig{
		BaseURL: strings.TrimSpace(os.Getenv("CHIMII_LARK_CALLBACK_BASE_URL")),
		Logger:  slog.Default(),
	})
	if err != nil {
	...
	if proxyURL := strings.TrimSpace(os.Getenv("CHIMII_LARK_WS_PROXY_URL")); proxyURL != "" {
...
func cloudRuntimeFleetURLFromEnv() string {
	if url := strings.TrimSpace(os.Getenv("CHIMII_CLOUD_FLEET_URL")); url != "" {
		return url
	}
	return strings.TrimSpace(os.Getenv("CHIMII_FLEET_URL"))
}
...
// already-resolved CHIMII_PUBLIC_URL, then the app URL.
