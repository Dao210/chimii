...
func withAgentContext(t *testing.T) {
	t.Helper()
	t.Setenv("CHIMII_TASK_ID", "task-1")
}
...
func TestFindLocalPathLinksAllowsLegitimateContent(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			body: "the PR is at [#42](https://github.com/chimii-ai/chimii/pull/42)",
		},
		{
			name: "mention link",
...
func TestGuardLocalPathLinksOnlyFiresInAgentContext(t *testing.T) {
	...
	t.Run("human PAT context is never linted", func(t *testing.T) {
		// No CHIMII_AGENT_ID / CHIMII_TASK_ID: a person running the CLI. Their
...
func TestGuardLocalPathLinksHintIsPerCommand(t *testing.T) {
	...
	err := guardLocalPathLinks(
		"[screenshot]("+shot+")",
		"issue description",
		"`chimii issue update` cannot carry files — deliver the file with `chimii issue comment add <issue-id> --attachment <path>` instead, and drop the link.",
	)
	if err == nil {
		t.Fatal("expected a hard failure")
	}
	msg := err.Error()
	if !strings.Contains(msg, "`chimii issue update` cannot carry files") {
		t.Errorf("update hint missing, got: %v", msg)
	}
	if !strings.Contains(msg, "chimii issue comment add <issue-id> --attachment <path>") {
		t.Errorf("update hint must redirect to comment add, got: %v", msg)
	}
	if strings.Contains(msg, "chimii issue update --attachment") {
