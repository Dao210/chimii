...
var issueReorderCmd = &cobra.Command{
	...
	Long: "Reposition an issue inside its current status column by computing a new\n" +
		...
		"different column, change its status first with `chimii issue status`.",
	Args: exactArgs(1),
...
func init() {
	...
	issueCommentListCmd.Flags().String("before", "", "Cursor (RFC3339Nano timestamp). With --recent: thread cursor (last_activity_at). With --thread + --tail: reply cursor (reply created_at). Read from the X-Chimii-Next-Before response header; must be paired with --before-id.")
	issueCommentListCmd.Flags().String("before-id", "", "Cursor UUID. With --recent: thread root UUID. With --thread + --tail: oldest reply UUID. Read from the X-Chimii-Next-Before-Id response header; must be paired with --before.")
...
func quickCreateAttachmentIDsFromEnv() ([]string, error) {
	raw := strings.TrimSpace(os.Getenv("CHIMII_QUICK_CREATE_ATTACHMENT_IDS"))
	if raw == "" {
		return nil, nil
	}
	var ids []string
	if err := json.Unmarshal([]byte(raw), &ids); err != nil {
		return nil, fmt.Errorf("parse CHIMII_QUICK_CREATE_ATTACHMENT_IDS: %w", err)
	}
...
func runIssueCreate(cmd *cobra.Command, _ []string) error {
	...
	if hasDesc {
		if err := guardLocalPathLinks(desc, "issue description",
			"Deliver the file itself with `chimii issue create --attachment <path>` (repeatable) and drop the link."); err != nil {
			return err
		}
	...
	// Quick-create stamp: when the daemon sets CHIMII_QUICK_CREATE_TASK_ID
	// before invoking the agent, the agent's `chimii issue create` call
	...
	if taskID := os.Getenv("CHIMII_QUICK_CREATE_TASK_ID"); taskID != "" {
...
func runIssueUpdate(cmd *cobra.Command, args []string) error {
	...
	if cmd.Flags().Changed("description") || cmd.Flags().Changed("description-stdin") || cmd.Flags().Changed("description-file") {
		...
		if err := guardLocalPathLinks(desc, "issue description",
			"`chimii issue update` cannot carry files — deliver the file with `chimii issue comment add <issue-id> --attachment <path>` instead, and drop the link."); err != nil {
			return err
		}
		body["description"] = desc
	}
...
func reorderTargetNotInColumnError(ctx context.Context, client *cli.APIClient, otherRef, issueRef resolvedID, status string) error {
	if other, err := fetchIssue(ctx, client, otherRef.ID); err == nil {
		if otherStatus := strVal(other, "status"); otherStatus != "" && otherStatus != status {
			return fmt.Errorf("issue %s is in the %q column but %s is in %q; move one with `chimii issue status` first, or pick a target in the same column", otherRef.Display, otherStatus, issueRef.Display, status)
		}
	}
...
func runIssueCommentList(cmd *cobra.Command, args []string) error {
	...
	if nb := respHeaders.Get("X-Chimii-Next-Before"); nb != "" {
		if nbid := respHeaders.Get("X-Chimii-Next-Before-Id"); nbid != "" {
...
func runIssueCommentAdd(cmd *cobra.Command, args []string) error {
	...
	if err := guardLocalPathLinks(content, "comment body",
		"Deliver the file itself with `chimii issue comment add <issue-id> --attachment <path>` (repeatable) and drop the link."); err != nil {
		return err
	}
...
func resolveAssignee(ctx context.Context, client *cli.APIClient, name string, kinds assigneeKinds) (string, string, error) {
	if client.WorkspaceID == "" {
		return "", "", fmt.Errorf("workspace ID is required to resolve assignees; use --workspace-id or set CHIMII_WORKSPACE_ID")
	}
...
// from `chimii workspace member list --output json`) should use this instead of
