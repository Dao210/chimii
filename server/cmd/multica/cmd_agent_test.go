...
// impersonation escape: a sandbox fault strips every CHIMII_* env var from
// an agent subprocess, which then runs `chimii` from the *parent* directory
...
func TestNewAPIClient_WorkdirParentEscapeFailsClosed(t *testing.T) {
	...
	// Daemon-owned tree: {root}/.chimii marker + {root}/{ws}/{task}/workdir
	...
	t.Setenv("CHIMII_AGENT_ID", "")
	t.Setenv("CHIMII_TASK_ID", "")
	t.Setenv("CHIMII_DAEMON_PORT", "")
	t.Setenv("CHIMII_TOKEN", "")
	t.Setenv("CHIMII_SERVER_URL", "http://127.0.0.1:8080")
...
// agent task (CHIMII_AGENT_ID / CHIMII_TASK_ID set), the CLI must NOT
// silently read the user-global ~/.chimii/config.json to recover a missing
...
func TestResolveWorkspaceID_AgentContextSkipsConfig(t *testing.T) {
	...
	t.Run("agent context with explicit env uses env", func(t *testing.T) {
		t.Setenv("CHIMII_AGENT_ID", "agent-123")
		t.Setenv("CHIMII_TASK_ID", "task-456")
		t.Setenv("CHIMII_DAEMON_PORT", "")
		t.Setenv("CHIMII_WORKSPACE_ID", "env-ws")
	...
	t.Run("daemon port marker also skips config", func(t *testing.T) {
		t.Setenv("CHIMII_AGENT_ID", "")
		t.Setenv("CHIMII_TASK_ID", "")
		t.Setenv("CHIMII_DAEMON_PORT", "27182")
		t.Setenv("CHIMII_WORKSPACE_ID", "")
...
func TestResolveToken_AgentContextSkipsConfig(t *testing.T) {
	...
	t.Run("agent context without env never reads config", func(t *testing.T) {
		...
		if got := resolveToken(testCmd()); got != "" {
			t.Fatalf("resolveToken() = %q, want empty in agent context without CHIMII_TOKEN", got)
		}
	})
	...
	t.Run("daemon port marker without env never reads config", func(t *testing.T) {
		...
		if got := resolveToken(testCmd()); got != "" {
			t.Fatalf("resolveToken() = %q, want empty in daemon-managed context without CHIMII_TOKEN", got)
		}
	})
	...
	t.Run("agent context uses explicit task token env", func(t *testing.T) {
		...
		t.Setenv("CHIMII_TASK_ID", "task-456")
		t.Setenv("CHIMII_DAEMON_PORT", "")
		t.Setenv("CHIMII_TOKEN", "mat_task_token")

		if got := resolveToken(testCmd()); got != "mat_task_token" {
			t.Fatalf("resolveToken() = %q, want CHIMII_TOKEN", got)
		}
	})
	...
	t.Run("daemon port set without agent context avoids config fallback", func(t *testing.T) {
		...
		t.Setenv("CHIMII_TASK_ID", "")
		t.Setenv("CHIMII_TOKEN", "")
		t.Setenv("CHIMII_DAEMON_PORT", "19514")
	...
	t.Run("daemon port set with explicit task token uses task token", func(t *testing.T) {
		...
		if got := resolveToken(testCmd()); got != "mat_task_token" {
			t.Fatalf("resolveToken() = %q, want CHIMII_TOKEN (task token wins over daemon signal)", got)
		}
	})

	// CHIMII_SERVER_URL is a user-facing env var that may be set in a
	// normal shell. It is NOT a daemon identity signal — only
	// CHIMII_DAEMON_PORT is. The config fallback must still work when
	// SERVER_URL is set but no daemon signal is present.
	t.Run("CHIMII_SERVER_URL alone does not block config fallback", func(t *testing.T) {
		t.Setenv("CHIMII_AGENT_ID", "")
		t.Setenv("CHIMII_TASK_ID", "")
		t.Setenv("CHIMII_TOKEN", "")
		t.Setenv("CHIMII_DAEMON_PORT", "")
		t.Setenv("CHIMII_SERVER_URL", "https://api.chimii.ai")
...
func TestNewAPIClient_AgentContextRequiresTaskToken(t *testing.T) {
	t.Setenv("CHIMII_SERVER_URL", "http://127.0.0.1:8080")
	t.Setenv("CHIMII_WORKSPACE_ID", "workspace-123")
	t.Setenv("CHIMII_AGENT_ID", "agent-123")
	t.Setenv("CHIMII_TASK_ID", "task-456")
	...
	t.Run("member token fails closed", func(t *testing.T) {
		t.Setenv("CHIMII_TOKEN", "mul_member_token")
...
func TestAgentUpdateNoFieldsErrorPointsAtEnvCommand(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	t.Setenv("CHIMII_SERVER_URL", "http://127.0.0.1:0")
	t.Setenv("CHIMII_WORKSPACE_ID", "test-ws")
	t.Setenv("CHIMII_TOKEN", "test-token")
	t.Setenv("CHIMII_AGENT_ID", "")
	t.Setenv("CHIMII_TASK_ID", "")
	...
	msg := err.Error()
	if !strings.Contains(msg, "chimii agent env set") {
		t.Fatalf("no-fields error must direct users to `chimii agent env set`; got: %q", msg)
	}
}
...
func TestAgentUpdateDoesNotExposeCustomEnvFlags(t *testing.T) {
	for _, flag := range []string{"custom-env", "custom-env-stdin", "custom-env-file"} {
		if agentUpdateCmd.Flag(flag) != nil {
			t.Errorf("agent update must NOT expose --%s after MUL-2600; use `chimii agent env set` instead", flag)
		}
	}
}
...
func TestAgentSkillsAddCallsAdditiveEndpoint(t *testing.T) {
	...
	t.Setenv("CHIMII_SERVER_URL", srv.URL)
	t.Setenv("CHIMII_WORKSPACE_ID", "ws-1")
	t.Setenv("CHIMII_TOKEN", "test-token")
