...
var loginCmd = &cobra.Command{
	...
	Long:  "Log in to Chimii, then automatically discover and watch all your workspaces.",
...
// "prompt me interactively", preserving the legacy `chimii login --token`
...
func runLogin(cmd *cobra.Command, args []string) error {
	...
	if err := autoWatchWorkspaces(cmd); err != nil {
		...
		fmt.Fprintf(os.Stderr, "Run 'chimii workspace list' and 'chimii workspace watch <id>' to set up manually.\n")
		return nil
	}

	fmt.Fprintf(os.Stderr, "\n→ Run 'chimii daemon start' to start your local agent runtime.\n")
	return nil
}
...
func autoWatchWorkspaces(cmd *cobra.Command) error {
	...
	if len(workspaces) > 1 {
		fmt.Fprintln(os.Stderr, "\nUse 'chimii workspace switch <id|slug>' to change the default workspace.")
	}
...
func waitForWorkspaceCreation(cmd *cobra.Command, client *cli.APIClient) ([]struct {
	...
	if appURL == "" {
		...
		fmt.Fprintln(os.Stderr, "\nNo workspaces found.")
		fmt.Fprintln(os.Stderr, "Create a workspace in the web dashboard, then run 'chimii login' again.")
		return nil, nil
	}
	...
	// but it still honors CHIMII_HTTP_TIMEOUT via AtLeastAPITimeout so a user
