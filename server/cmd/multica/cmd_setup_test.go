...
// honor CHIMII_SERVER_URL when --server-url is not passed, instead of always
...
func TestResolveSelfHostServerURL(t *testing.T) {
	...
	t.Run("env var honored when flag absent", func(t *testing.T) {
		t.Setenv("CHIMII_SERVER_URL", "https://api.internal.co")
	...
	t.Run("flag wins over env", func(t *testing.T) {
		t.Setenv("CHIMII_SERVER_URL", "https://env.example")
		cmd := newCmd()
	...
	// CHIMII_SERVER_URL is documented as a ws:// daemon address; the probe and
	...
	t.Run("normalizes the documented ws:// daemon form", func(t *testing.T) {
		t.Setenv("CHIMII_SERVER_URL", "wss://api.internal.co/ws")
		serverURL, userProvided := resolveSelfHostServerURL(newCmd(), cli.CLIConfig{})
		if serverURL != "https://api.internal.co" {
			t.Fatalf("server_url: want normalized https base, got %q", serverURL)
		}
...
// CHIMII_APP_URL is honored when the flag is absent.
func TestSelfHostAppURLHonorsEnv(t *testing.T) {
	cmd := &cobra.Command{}
	cmd.Flags().String("app-url", "", "")

	t.Run("env honored when flag absent", func(t *testing.T) {
		t.Setenv("CHIMII_APP_URL", "https://app.internal.co")
		if got := cli.FlagOrEnv(cmd, "app-url", "CHIMII_APP_URL", ""); got != "https://app.internal.co" {
			t.Fatalf("app_url: want env value, got %q", got)
		}
	})

	t.Run("flag wins over env", func(t *testing.T) {
		t.Setenv("CHIMII_APP_URL", "https://env.example")
		if err := cmd.Flags().Set("app-url", "https://flag.example"); err != nil {
			t.Fatalf("set flag: %v", err)
		}
		if got := cli.FlagOrEnv(cmd, "app-url", "CHIMII_APP_URL", ""); got != "https://flag.example" {
