...
func TestResolveAppURL(t *testing.T) {
	...
	t.Run("prefers CHIMII_APP_URL", func(t *testing.T) {
		t.Setenv("CHIMII_APP_URL", "http://localhost:14000")
...
func TestBrowserLoginInstructionsSSHRemoteHint(t *testing.T) {
	const loginURL = "https://chimii.ai/login?cli_callback=http%3A%2F%2Flocalhost%3A43689%2Fcallback"
...
func TestLoginTokenFlagWiring(t *testing.T) {
	...
	if tokenFlag.NoOptDefVal != tokenPromptSentinel {
		t.Fatalf("loginCmd --token NoOptDefVal = %q, want %q (legacy `chimii login --token` prompt mode would break)", tokenFlag.NoOptDefVal, tokenPromptSentinel)
	}
}

// TestLoginTokenHelpOutputRendersCleanly renders loginCmd's flag help through
// the same pflag path `chimii login -h` uses (FlagUsagesWrapped) and locks the
...
// `chimii login --token`. The original implementation hardcoded `mul_`
// only, which rejected legitimate Chimii Cloud Node PATs (`mcn_`) at
