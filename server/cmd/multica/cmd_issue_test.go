...
func TestRunIssueCommentAddRejectsExternalAttachmentWithZeroUploads(t *testing.T) {
	...
	t.Setenv("CHIMII_TOKEN", "mat_test-token")
...
func TestRunIssueCreateSendsExistingAttachmentIDs(t *testing.T) {
	...
	t.Setenv("CHIMII_WORKSPACE_ID", "ws-1")
	t.Setenv("CHIMII_TOKEN", "test-token")
	t.Setenv("CHIMII_QUICK_CREATE_ATTACHMENT_IDS", `["att-env","att-shared"]`)
...
func TestRunIssuePullRequestsListsLinkedPRsAsJSON(t *testing.T) {
		...
		case "/api/issues/issue-uuid/pull-requests":
			json.NewEncoder(w).Encode(map[string]any{
				"pull_requests": []map[string]any{
					{
						"url":    "https://github.com/chimii-ai/chimii/pull/42",
						"number": float64(42),
						"state":  "open",
	...
	if pr["url"] != "https://github.com/chimii-ai/chimii/pull/42" || pr["number"] != float64(42) || pr["state"] != "open" || pr["title"] != "MUL-2818 add issue PR CLI" {
...
func TestRunIssuePullRequestsTableIncludesCoreFields(t *testing.T) {
	...
	out, _ := io.ReadAll(r)
	text := string(out)
	for _, want := range []string{"NUMBER", "STATE", "TITLE", "URL", "42", "open", "MUL-2818 add issue PR CLI", "https://github.com/chimii-ai/chimii/pull/42"} {
...
// chimii-ai/chimii#1620: when one name is a substring of another (e.g.
...
// chimii-ai/chimii#1620: passing a full UUID or its 8-char prefix must
...
// `chimii issue subscriber add --user "<SquadName>"` or
// `chimii project create --lead "<SquadName>"` would resolve to
...
func TestRunIssueCommentList_ThreadTailPassesThroughAndPrintsReplyCursor(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.Method == http.MethodGet && strings.HasSuffix(r.URL.Path, "/comments") {
			...
			w.Header().Set("X-Chimii-Next-Before", "2026-01-01T00:00:00.000000001Z")
			w.Header().Set("X-Chimii-Next-Before-Id", "00000000-0000-0000-0000-000000000999")
			w.Write([]byte("[]"))
			return
		}
		t.Errorf("unexpected request: %s %s", r.Method, r.URL.String())
...
func TestRunIssueCommentList_RecentStillLabelsCursorAsThread(t *testing.T) {
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		w.Header().Set("X-Chimii-Next-Before", "2026-01-01T00:00:00.000000001Z")
		w.Header().Set("X-Chimii-Next-Before-Id", "00000000-0000-0000-0000-000000000777")
		w.Write([]byte("[]"))
	}))
	defer srv.Close()
