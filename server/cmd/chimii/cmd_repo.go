...
func runRepoCheckout(cmd *cobra.Command, args []string) error {
	...
	daemonPort := os.Getenv("CHIMII_DAEMON_PORT")
	if daemonPort == "" {
		return fmt.Errorf("CHIMII_DAEMON_PORT not set (this command is intended to be run by an agent inside a daemon task)")
	}

	workspaceID := os.Getenv("CHIMII_WORKSPACE_ID")
	agentName := os.Getenv("CHIMII_AGENT_NAME")
	taskID := os.Getenv("CHIMII_TASK_ID")
	...
	reqBody := map[string]string{
		...
		"task_id":       taskID,
		"checkout_mode": strings.TrimSpace(os.Getenv("CHIMII_REPO_CHECKOUT_MODE")),
	}
