...
func TestRunWorkspaceCreatePostsWorkspaceAndDoesNotSwitchDefault(t *testing.T) {
	...
	t.Setenv("CHIMII_WORKSPACE_ID", "existing-workspace")
...
func TestRunWorkspaceSwitch(t *testing.T) {
	...
	// Isolate HOME so the test never touches the developer's ~/.chimii.
	t.Setenv("HOME", t.TempDir())
	t.Setenv("CHIMII_SERVER_URL", srv.URL)
	t.Setenv("CHIMII_TOKEN", "test-token")
	...
	t.Run("isolates by profile", func(t *testing.T) {
		...
		wantSuffix := filepath.Join(".chimii", "profiles", "staging", "config.json")
