...
// loginTokenPrefixes are the token prefixes `chimii login --token` accepts.
// The CLI used to hardcode `mul_` only, which made it impossible to log in
// with a Chimii Cloud Node PAT (`mcn_`) even though the server happily
...
var authCmd = &cobra.Command{
	Use:   "auth",
	Short: "Authenticate chimii with Chimii",
}
...
func resolveToken(cmd *cobra.Command) string {
	if v := strings.TrimSpace(os.Getenv("CHIMII_TOKEN")); v != "" {
		return v
	}
	...
	// inDaemonManagedExecutionContext already covers the CHIMII_DAEMON_PORT
	// signal for subprocesses that lost CHIMII_AGENT_ID / CHIMII_TASK_ID.
...
func resolveAppURL(cmd *cobra.Command) string {
	for _, key := range []string{"CHIMII_APP_URL", "FRONTEND_ORIGIN"} {
	...
	fmt.Fprintln(os.Stderr, "No app URL configured. Run 'chimii setup' first.")
	os.Exit(1)
	return "" // unreachable
}
...
func runAuthLoginBrowser(cmd *cobra.Command) error {
	...
	if err != nil {
		return cli.WithUserMessage("Sign-in did not complete: the server could not issue an access token for the CLI. Run `chimii login` again.", err)
	}
	...
	if err := patClient.GetJSON(ctx, "/api/me", &me); err != nil {
		return cli.WithUserMessage("Sign-in did not complete: the server did not accept the new credential. Run `chimii login` again.", err)
	}
...
func runAuthLoginToken(cmd *cobra.Command, providedToken string) error {
	...
	if err := client.GetJSON(ctx, "/api/me", &me); err != nil {
		return cli.WithUserMessage("Could not sign in with that token — make sure it is valid and not expired, then run `chimii login --token <token>` again.", err)
	}
...
func runAuthStatus(cmd *cobra.Command, _ []string) error {
	...
	if token == "" {
		fmt.Fprintln(os.Stderr, "Not authenticated. Run 'chimii login' to authenticate.")
		return nil
	}
...
<title>Chimii — Authenticated</title>
<style>
