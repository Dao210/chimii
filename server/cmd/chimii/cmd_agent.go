...
// custom_env values — `chimii agent list / get / update` no longer
...
func init() {
	...
	// `chimii agent env set <id>` — that path is owner/admin-only,
...
func newAPIClient(cmd *cobra.Command) (*cli.APIClient, error) {
	...
	if serverURL == "" {
		return nil, fmt.Errorf("server URL not set: use --server-url flag, CHIMII_SERVER_URL env, or 'chimii config set server_url <url>'")
	}
	if inDaemonManagedExecutionContext() && !strings.HasPrefix(token, "mat_") {
		// When the ONLY daemon signal is a workdir marker (no CHIMII_AGENT_ID /
		// CHIMII_TASK_ID / CHIMII_DAEMON_PORT), the likeliest cause outside a
		...
		if !inAgentExecutionContext() && os.Getenv("CHIMII_DAEMON_PORT") == "" {
			if markerPath := daemonTaskContextMarkerPath(); markerPath != "" {
				return nil, fmt.Errorf("agent execution context requires CHIMII_TOKEN to be a task-scoped mat_ token; detected a daemon task marker at %s — if you are not running inside an agent task this is likely a leftover, remove it and retry", markerPath)
			}
		}
		return nil, fmt.Errorf("agent execution context requires CHIMII_TOKEN to be a task-scoped mat_ token")
	}

	client := cli.NewAPIClient(serverURL, workspaceID, token)
	// When running inside a daemon task, attribute actions to the agent.
	if agentID := os.Getenv("CHIMII_AGENT_ID"); agentID != "" {
		client.AgentID = agentID
	}
	if taskID := os.Getenv("CHIMII_TASK_ID"); taskID != "" {
...
const (
	defaultCloudServerURL = "https://api.chimii.ai"
	defaultCloudAppURL    = "https://chimii.ai"
)

func tryResolveServerURL(cmd *cobra.Command) string {
	val := cli.FlagOrEnv(cmd, "server-url", "CHIMII_SERVER_URL", "")
	if val != "" {
...
func resolveServerURL(cmd *cobra.Command) string {
	...
	fmt.Fprintln(os.Stderr, "No server configured. Run 'chimii setup' first.")
	os.Exit(1)
	return "" // unreachable
}
...
func inAgentExecutionContext() bool {
	return os.Getenv("CHIMII_AGENT_ID") != "" || os.Getenv("CHIMII_TASK_ID") != ""
}

// inDaemonManagedExecutionContext reports whether the CLI is being invoked
// from inside a daemon-managed agent task. CHIMII_DAEMON_PORT is included as
// a defense-in-depth marker for subprocesses that lose CHIMII_AGENT_ID or
// CHIMII_TASK_ID but still run under the daemon environment. In this context
// workspace and token must come from daemon-provided env; falling back to
// user-global ~/.chimii/config.json can make agent writes land as a member.
func inDaemonManagedExecutionContext() bool {
	return inAgentExecutionContext() || os.Getenv("CHIMII_DAEMON_PORT") != "" || hasDaemonTaskContextMarker()
}
...
func resolveWorkspaceID(cmd *cobra.Command) string {
	val := cli.FlagOrEnv(cmd, "workspace-id", "CHIMII_WORKSPACE_ID", "")
	if val != "" {
		return val
	}
...
func requireWorkspaceID(cmd *cobra.Command) (string, error) {
	...
	if id == "" {
		if inDaemonManagedExecutionContext() {
			return "", fmt.Errorf("workspace_id is required: CHIMII_WORKSPACE_ID must be set by the daemon in agent execution context (no fallback to user config)")
		}
		return "", fmt.Errorf("workspace_id is required: use --workspace-id flag, set CHIMII_WORKSPACE_ID env, or run 'chimii config set workspace_id <id>'")
	}
	return id, nil
}
...
func runAgentUpdate(cmd *cobra.Command, args []string) error {
	...
	if len(body) == 0 {
		return fmt.Errorf("no fields to update; use --name, --description, --instructions, --runtime-id, --runtime-config, --model, --thinking-level, --service-tier, --custom-args, --mcp-config, --visibility, --status, or --max-concurrent-tasks (env vars now live behind `chimii agent env set <id>`)")
	}
