...
var rootCmd = &cobra.Command{
	Use:           "chimii",
	Short:         "Chimii CLI — local agent runtime and management tool",
	Long:          "Work seamlessly with Chimii from the command line.",
	SilenceUsage:  true,
	SilenceErrors: true,
}
...
func init() {
	...
	rootCmd.SetVersionTemplate("chimii {{.Version}}\n")
	...
	rootCmd.PersistentFlags().String("server-url", "", "Chimii server URL (env: CHIMII_SERVER_URL)")
	rootCmd.PersistentFlags().String("workspace-id", "", "Workspace ID (env: CHIMII_WORKSPACE_ID)")
	rootCmd.PersistentFlags().String("profile", "", "Configuration profile name (e.g. dev) — isolates config, daemon state, and workspaces")
	rootCmd.PersistentFlags().BoolVar(&debugFlag, "debug", false, "Print full error details on failure (env: CHIMII_DEBUG)")
