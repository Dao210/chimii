...
-- re-connecting it — even to represent a DIFFERENT agent in the SAME Chimii
...
-- the conflict update to the SAME Chimii workspace: a team already owned by a
...
-- old catch-all "connected to a different Chimii workspace". Meant to be read
...
-- to a Chimii user. The old composite member-FK is gone, so this no
...
-- before calling. ON CONFLICT DO UPDATE is still gated on chimii_user_id
...
INSERT INTO channel_user_binding (
    workspace_id, chimii_user_id, installation_id,
...
ON CONFLICT (installation_id, channel_user_id) DO UPDATE SET
    ...
    bound_at = now()
WHERE channel_user_binding.chimii_user_id = EXCLUDED.chimii_user_id
RETURNING *;

-- name: GetChannelUserBindingByUserID :one
-- The inbound identity lookup: does this platform user id map to a Chimii
...
-- bound to ANOTHER installation in the SAME Chimii workspace + SAME Slack team,
...
-- different Chimii workspaces, and a user may hold different Chimii accounts in
...
DELETE FROM channel_user_binding
WHERE workspace_id = $1 AND chimii_user_id = $2;
