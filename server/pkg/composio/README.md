...
not import any other Chimii package, so it can be reused by other services or
...
import (
    ...
    "github.com/chimii-ai/chimii/server/pkg/composio"
)
...
link, err := client.CreateLink(ctx, composio.CreateLinkRequest{
    ...
    UserID:       chimiiUserID.String(), // your own user id
    CallbackURL:  "https://chimii.ai/api/integrations/composio/callback",
})
...
- **Standalone.** Zero coupling to Chimii internals — depend on this package
