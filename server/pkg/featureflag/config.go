...
// CHIMII_*_CONFIG / CHIMII_*_FILE knobs documented in .env.example.
const EnvFlagFile = "CHIMII_FEATURE_FLAGS_FILE"
...
// NewServiceFromEnv constructs a Service wired with the standard chimii
// config sources, in order of decreasing precedence:
//
//  1. EnvProvider (FF_<KEY> overrides — Ops kill switches, fastest path).
//  2. StaticProvider loaded from the YAML file at CHIMII_FEATURE_FLAGS_FILE
//     (when the env var is set and the file exists).
//
// When CHIMII_FEATURE_FLAGS_FILE is unset, the Service still works — the
