...
func TestBuildPiArgsIgnoresSystemPrompt(t *testing.T) {
	args := buildPiArgs("hello world", "/tmp/s.jsonl", ExecOptions{
		SystemPrompt: "the entire chimii runtime brief",
	}, slog.Default())
	...
	for _, a := range args {
		...
		if a == "the entire chimii runtime brief" {
...
func TestStripPiToolCallMarkup(t *testing.T) {
	tests := map[string]string{
		...
		`before response:bash{command:<|"|>chimii issue comment list issue-id --all --output json<|"|>} after`: "before  after",
