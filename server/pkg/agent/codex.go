...
const (
	chimiiCodexMcpBeginMarker = "# BEGIN chimii-managed mcp_servers (do not edit; regenerated by daemon)"
	chimiiCodexMcpEndMarker   = "# END chimii-managed mcp_servers"
)

var codexMcpBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(chimiiCodexMcpBeginMarker) +
		`.*?^` + regexp.QuoteMeta(chimiiCodexMcpEndMarker) + `\n*`)
...
func ensureCodexMcpConfig(configPath string, mcpConfig json.RawMessage, logger *slog.Logger) error {
	...
	if managed {
		...
		if block == "" {
			block = chimiiCodexMcpBeginMarker + "\n" + chimiiCodexMcpEndMarker + "\n"
		}
		if stripped == "" {
			updated = block
		} else {
...
func renderCodexMcpServersBlock(raw json.RawMessage) (string, bool, error) {
	...
	sb.WriteString(chimiiCodexMcpBeginMarker)
	sb.WriteString("\n")
	...
	sb.WriteString(chimiiCodexMcpEndMarker)
	sb.WriteString("\n")
...
func isChimiiMcpSelectorKey(k string) bool {
	switch k {
	...
	go func() {
		...
		_, err := c.request(runCtx, "initialize", map[string]any{
			"clientInfo": map[string]any{
				"name":    "chimii-agent-sdk",
				"title":   "Chimii Agent SDK",
				"version": "0.2.0",
			},
