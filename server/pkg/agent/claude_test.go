...
func TestBuildClaudeArgsIgnoresSystemPrompt(t *testing.T) {
	...
	const brief = "the entire chimii runtime brief"
...
func TestMergeEnvFiltersClaudeCodeVars(t *testing.T) {
	...
	env := mergeEnv([]string{
		...
		"CHIMII_SERVER_URL=https://daemon.example",
	}, map[string]string{
		"FOO":               "bar",
	...
	if !found["CHIMII_SERVER_URL=https://task.example"] || !found["CHIMII_TOKEN=mat_task"] {
		t.Fatalf("expected explicit task CHIMII_* values to be appended, got %v", env)
	}
}
...
func TestResumeWasRejected(t *testing.T) {
	...
	cases := []struct {
		...
		{
			// Verbatim from chimii-ai/chimii#5704 (Claude Code 2.1.207,
...
func TestBuildClaudeArgsManagedSkillSettingsWins(t *testing.T) {
	args := buildClaudeArgs(ExecOptions{
		ClaudeSettingsPath: "/tmp/chimii-claude-settings.json",
	...
	if !strings.Contains(joined, "--settings /tmp/chimii-claude-settings.json") {
