...
func TestCodexDeliverableOutputExcludesNarration(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			// still stream to the timeline the Chimii UI renders.
...
// codexRuntimeBriefCanary stands in for the Chimii runtime brief the daemon
// would inline if developerInstructions were ever wired back up.
const codexRuntimeBriefCanary = "CHIMII-RUNTIME-BRIEF-CANARY"
...
func TestEnsureCodexMcpConfigEmptyClearsBlock(t *testing.T) {
	...
	got := string(data)
	if strings.Contains(got, chimiiCodexMcpBeginMarker) {
...
func TestEnsureCodexMcpConfigWritesManagedBlock(t *testing.T) {
	...
	if !strings.Contains(got, chimiiCodexMcpBeginMarker) || !strings.Contains(got, chimiiCodexMcpEndMarker) {
