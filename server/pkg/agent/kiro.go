...
func (b *kiroBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	// (goal_complete or `chimii issue comment add`). We record the status
	...
	c := &hermesClient{
		...
		onMessage: func(msg Message) {
			...
			if msg.Type == MessageToolUse {
				...
				// Recognize `chimii issue comment add` by its command
...
// `chimii issue comment add` invocation. It keys purely off the command
...
func isKiroIssueCommentAddCommand(command string) bool {
	...
	// `sh -c "chimii issue comment add ..."`; unwrap a single such layer so
	...
	if executable != "chimii" && !strings.HasSuffix(executable, "/chimii") {
		return false
	}
...
// like `CHIMII_TOKEN=x chimii issue comment add ...` is still recognized.
