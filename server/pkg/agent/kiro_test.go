...
while IFS= read -r line; do
  ...
  case "$line" in
    ...
    *'"method":"session/new"'*)
      ...
      ;;
    *'"method":"session/prompt"'*)
      printf '{"jsonrpc":"2.0","method":"session/notification","params":{"sessionId":"ses_comment_done","update":{"type":"ToolCall","toolCallId":"tc-comment","name":"Shell","status":"pending","parameters":{"command":"chimii issue comment add issue-1 --content-file ./reply.md"}}}}\n'
...
func TestKiroIssueCommentAddCommand(t *testing.T) {
	...
	tests := []struct {
		...
		{"chimii issue comment add issue-1 --content-file ./reply.md", true},
		{"./chimii issue comment add issue-1 --content-file ./reply.md", true},
		{"/usr/local/bin/chimii issue comment add issue-1 --content-file ./reply.md", true},
		{"CHIMII_TOKEN=x chimii issue comment add issue-1 --content-file ./reply.md", true},
		{"FOO=1 BAR=2 ./chimii issue comment add issue-1", true},
		{`sh -c "chimii issue comment add issue-1 --content-file ./reply.md"`, true},
		{`bash -c 'chimii issue comment add issue-1'`, true},
		{`/bin/sh -c "chimii issue comment add issue-1"`, true},
		{"chimii issue get issue-1", false},
		{"echo chimii issue comment add issue-1", false},
		{`sh -c "echo chimii issue comment add issue-1"`, false},
		{"FOO=bar", false},
		{"", false},
	}
	for _, tt := range tests {
...
// `chimii issue comment add` must be recognized from its command payload
...
func TestKiroIssueCommentAddToolIgnoresToolTitle(t *testing.T) {
	...
	for _, tt := range tests {
		msg := Message{
			Type:  MessageToolUse,
			Tool:  tt.tool,
			Input: map[string]any{"command": "chimii issue comment add issue-1 --content-file ./reply.md"},
		}
...
func TestKiroBackendDoesNotCompleteRunningCommentAddWithoutResult(t *testing.T) {
	...
	result := runKiroCloseErrorScript(t, fakeKiroACPRunningToolCloseErrorScript(
		"execute_bash",
		"chimii issue comment add issue-1 --content-file ./reply.md",
	))
	if result.Status != "failed" {
...
// #5509 regression against the real captured wire shape: a `chimii issue
...
while IFS= read -r line; do
  ...
  case "$line" in
    ...
    *'"method":"session/new"'*)
      ...
      ;;
    *'"method":"session/prompt"'*)
      printf '{"jsonrpc":"2.0","method":"session/notification","params":{"sessionId":"ses_two","update":{"type":"ToolCall","toolCallId":"tc-1","name":"terminal","status":"pending","parameters":{"command":"chimii issue comment add issue-1 --content-file ./progress.md"}}}}\n'
      ...
      printf '{"jsonrpc":"2.0","method":"session/notification","params":{"sessionId":"ses_two","update":{"type":"ToolCall","toolCallId":"tc-2","name":"terminal","status":"pending","parameters":{"command":"chimii issue comment add issue-1 --content-file ./final.md"}}}}\n'
...
while IFS= read -r line; do
  ...
  case "$line" in
    ...
    *'"method":"session/new"'*)
      ...
      ;;
    *'"method":"session/prompt"'*)
      printf '{"jsonrpc":"2.0","method":"session/notification","params":{"sessionId":"ses_failed","update":{"type":"ToolCall","toolCallId":"tc-fail","name":"execute_bash","status":"pending","parameters":{"command":"chimii issue comment add issue-1 --content-file ./reply.md"}}}}\n'
...
func TestKiroBackendInvokesACPWithTrustAllTools(t *testing.T) {
	...
	session, err := backend.Execute(ctx, "prompt-ignored", ExecOptions{
		Model:      "bogus-model",
		Timeout:    5 * time.Second,
		CustomArgs: []string{"acp", "--trust-tools", "shell", "-a", "--agent", "chimii"},
	})
	if err != nil {
		t.Fatalf("execute: %v", err)
	}
	...
	if strings.Join(lines, "\n") != strings.Join([]string{"acp", "--trust-all-tools", "--agent", "chimii"}, "\n") {
