...
// the chimii CLI version reported by the daemon at registration time. The
...
// binding, no-retry semantics on `chimii issue create` failure — see PR
...
// MinHandoffCLIVersion is the lowest chimii CLI version whose daemon renders
...
var (
	ErrCLIVersionMissing = errors.New("chimii CLI version not reported by daemon")
	ErrCLIVersionTooOld  = errors.New("chimii CLI version is below required minimum")
)
