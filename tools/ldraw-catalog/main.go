...
// Vertices contain two endpoints, followed by two controls for conditional lines.
// These are render-only data and must not contribute to physical bounds.
type renderLineGroup struct {
	Color       string    `json:"color"`
	Conditional bool      `json:"conditional"`
	Vertices    []float64 `json:"vertices"`
}

type renderLineData struct {
	Version int               `json:"version"`
	Groups  []renderLineGroup `json:"groups"`
}
...
func parseSource(raw []byte) ([]parsedLine, error) {
	...
	for rowIndex, row := range rows {
		...
		kind := fields[0]
		need := map[string]int{"1": 15, "2": 8, "3": 11, "4": 14, "5": 14}[kind]
		if need == 0 {
			continue
		}
		if len(fields) < need {
...
func (l *archiveLibrary) flatten(path string, matrix transform, material materialContext, stack map[string]bool, deps map[string]bool, out *[]triangle, lines *[]renderLine) error {
	if stack[path] {
	...
	for _, line := range source.Lines {
		...
		case '1':
			...
			if err := l.flatten(childPath, compose(matrix, line.Transform), childMaterial(line.Color, material), stack, deps, out, lines); err != nil {
				return err
			}
		case '2', '5':
			points := make([]vec3, len(line.Points))
			for i, point := range line.Points {
				points[i] = matrix.point(point)
			}
			*lines = append(*lines, renderLine{Points: points, Material: geometryMaterial(line.Color, material), Conditional: line.Kind == '5'})
		case '3':
...
func (l *archiveLibrary) compilePart(partID string, colors map[int]colorValue) (catalogAsset, error) {
	...
	triangles := []triangle{}
	lines := []renderLine{}
	if err := l.flatten(root, identity(), materialContext{Surface: "current", Edge: "edge-current"}, map[string]bool{}, deps, &triangles, &lines); err != nil {
		return catalogAsset{}, err
	}
	if len(triangles) == 0 {
		return catalogAsset{}, errors.New("part contains no triangles")
	}
	glb, bounds, err := buildGLB(partID, triangles, lines, colors)
	if err != nil {
		return catalogAsset{}, err
	}
...
func renderTypeScript(lock lockFile, assets []catalogAsset) ([]byte, error) {
	...
	fmt.Fprintf(&b, "export const LDRAW_CATALOG_VERSION = %q;\n", "ldraw-official-"+lock.Release+"-"+lock.ArchiveSHA256[:12]+"-r2")
...
func buildGLB(partID string, triangles []triangle, lines []renderLine, colors map[int]colorValue) ([]byte, [6]float64, error) {
	...
	doc := gltfDocument{
		...
		Scenes: []map[string]any{{"nodes": []int{0}}}, Nodes: []map[string]any{{"mesh": 0, "name": partID, "extras": map[string]any{"ldrawLines": groupRenderLines(lines, colors)}}},
	}
...
func groupRenderLines(lines []renderLine, colors map[int]colorValue) renderLineData {
	groups := map[string]*renderLineGroup{}
	for _, line := range lines {
		color := line.Material
		if color != "current" && color != "edge-current" {
			color = materialHexColor(color, colors)
		}
		key := fmt.Sprintf("%s/%t", color, line.Conditional)
		group := groups[key]
		if group == nil {
			group = &renderLineGroup{Color: color, Conditional: line.Conditional, Vertices: []float64{}}
			groups[key] = group
		}
		for _, point := range line.Points {
			group.Vertices = append(group.Vertices, point.X, point.Y, point.Z)
		}
	}
	...
	sort.Strings(keys)
	result := renderLineData{Version: 1, Groups: []renderLineGroup{}}
...
func materialHexColor(name string, colors map[int]colorValue) string {
	switch {
	case strings.HasPrefix(name, "edge-fixed-"):
		code, _ := strconv.Atoi(strings.TrimPrefix(name, "edge-fixed-"))
		if edge := colors[code].Edge; strings.HasPrefix(edge, "#") {
			return edge
		}
		return "#333333"
	case strings.HasPrefix(name, "direct-"):
		return "#" + strings.TrimPrefix(name, "direct-")
...
func materialColor(name string, colors map[int]colorValue) []float64 {
	r, g, b := parseHexColor(materialHexColor(name, colors))
	// LDraw VALUE is sRGB, while glTF baseColorFactor is linear.
	return []float64{srgbToLinear(r), srgbToLinear(g), srgbToLinear(b), 1}
}
