...
This guide documents the local development workflow for contributors working on the Chimii codebase.
...
- the main checkout usually uses `.env` and `POSTGRES_DB=chimii`
...
POSTGRES_DB=chimii
POSTGRES_PORT=5432
DATABASE_URL=postgres://chimii:chimii@localhost:5432/chimii?sslmode=disable
PORT=8080
FRONTEND_PORT=3000
```
...
```bash
POSTGRES_DB=chimii_my_feature_702
POSTGRES_PORT=5432
PORT=18782
FRONTEND_PORT=13702
DATABASE_URL=postgres://chimii:chimii@localhost:5432/chimii_my_feature_702?sslmode=disable
```
...
```bash
git worktree add ../chimii-feature -b feat/my-change main
cd ../chimii-feature
make dev
```
...
- main checkout
  ...
  - frontend: `3000`
- worktree checkout
  - database: `chimii_my_feature_702`
...
The daemon authenticates using the CLI's stored token (`chimii login`).
...
whatever server is configured in `~/.chimii/config.json`. That's fine for
...
Example: worktree at `../chimii-feat-auth` produces profile
`dev-chimii_feat_auth-347`, matching that worktree's port and database
allocation.
...
For deterministic local automation, set `CHIMII_DEV_VERIFICATION_CODE=888888`
...
CONFIG_DIR="$HOME/.chimii/profiles/$PROFILE"
mkdir -p "$CONFIG_DIR"
...
local backend. Agent-executed `chimii` commands automatically use the same
...
rm -rf "$HOME/.chimii/profiles/$PROFILE"
```
...
1. Compiles the `chimii` CLI from `server/cmd/chimii` into
   `apps/desktop/resources/bin/chimii`
...
backend logs. If you set `CHIMII_DEV_VERIFICATION_CODE=888888` before starting
...
- `DESKTOP_APP_SUFFIX` = `<folder>-<offset>` — its own single-instance lock /
  `userData`, and an app named `Chimii Canary <folder>-<offset>` so it is
  distinguishable in Cmd+Tab. The offset keeps it unique across worktrees that
  share a folder name at different paths.

The primary checkout is left untouched (`5173`, `Chimii Canary`). Set either
...
Nothing in this flow touches the system-installed `chimii` or the default
`~/.chimii/config.json`:

| Resource | System / Production | Local Dev (per-worktree) |
|---|---|---|
| Config | `~/.chimii/config.json` | `~/.chimii/profiles/dev-<slug>-<hash>/config.json` |
| Daemon PID | `~/.chimii/daemon.pid` | `~/.chimii/profiles/dev-<slug>-<hash>/daemon.pid` |
| Health port | `19514` | `19514 + 1 + (name_hash % 1000)` |
| Workspaces dir | `~/chimii_workspaces/` | `~/chimii_workspaces_dev-<slug>-<hash>/` |
| Database | remote / production | local Docker: `chimii_<slug>_<hash>` |
| Desktop profile | `desktop-api.chimii.ai` | `desktop-localhost-<port>` |
...
```bash
docker compose exec -T postgres psql -U chimii -d postgres -At -c "select datname from pg_database order by datname;"
```
