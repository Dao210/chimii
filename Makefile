.PHONY: help makehelp dev server daemon cli chimii build test migrate-up migrate-down sqlc seed clean setup start stop check worktree-env setup-main start-main stop-main check-main setup-worktree start-worktree stop-worktree check-worktree db-up db-down db-reset
...
POSTGRES_DB ?= chimii
POSTGRES_USER ?= chimii
POSTGRES_PASSWORD ?= chimii
POSTGRES_PORT ?= 5432
PORT := $(or $(BACKEND_PORT),$(API_PORT),$(SERVER_PORT),$(PORT),8080)
FRONTEND_PORT ?= 3000
FRONTEND_ORIGIN ?= http://localhost:$(FRONTEND_PORT)
CHIMII_APP_URL ?= $(FRONTEND_ORIGIN)
DATABASE_URL ?= postgres://$(POSTGRES_USER):$(POSTGRES_PASSWORD)@localhost:$(POSTGRES_PORT)/$(POSTGRES_DB)?sslmode=disable
NEXT_PUBLIC_API_URL ?= http://localhost:$(PORT)
NEXT_PUBLIC_WS_URL ?= ws://localhost:$(PORT)/ws
GOOGLE_REDIRECT_URI ?= $(FRONTEND_ORIGIN)/auth/callback
CHIMII_SERVER_URL ?= ws://localhost:$(PORT)/ws
...
.PHONY: build-eval build-eval-test build-brickgpt-check
BUILD_EVAL_ARGS ?=
...
build-brickgpt-check: ## Verify local server/BrickGPT source without loading models or Gurobi
	python3 scripts/brickgpt-eval/bridge.py verify-source
...
daemon: ## Restart the local agent daemon using the CLI's stored auth/session
	@$(MAKE) chimii CHIMII_ARGS="daemon restart --profile local"

cli: ## Run the chimii CLI with ARGS or CHIMII_ARGS from source
	@$(MAKE) chimii CHIMII_ARGS="$(CHIMII_ARGS)"

chimii: ## Run the chimii CLI entrypoint directly from the Go source tree
	cd server && go run -ldflags "-X main.version=$(VERSION) -X main.commit=$(COMMIT) -X main.date=$(DATE)" ./cmd/chimii $(CHIMII_ARGS)
...
build: ## Build the server, CLI, and migrate binaries into server/bin
	...
	cd server && go build -ldflags "-X main.version=$(VERSION) -X main.commit=$(COMMIT) -X main.date=$(DATE)" -o bin/chimii ./cmd/chimii
