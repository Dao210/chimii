...
const DATABASE_URL = process.env.DATABASE_URL ?? "postgres://chimii:chimii@localhost:5432/chimii?sslmode=disable";
...
export class TestApiClient {
  ...
  async login(email: string, name: string) {
    ...
    try {
      ...
      const configuredDevCode = process.env.CHIMII_DEV_VERIFICATION_CODE?.trim();
