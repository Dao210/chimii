...
GUARD_DIR=$(mktemp -d "${TMPDIR:-/tmp}/chimii-agent-cli-guard.XXXXXX")
...
printf '%s\n' "$line" >>"$CHIMII_AGENT_CLI_GUARD_MARKER"
exit 126
EOF
  chmod 755 "$sentinel"
done <"$NAMES_FILE"
...
set +e
PATH="$BIN_DIR:$PATH" CHIMII_AGENT_CLI_GUARD_MARKER="$MARKER_FILE" "$@"
command_status=$?
set -e
