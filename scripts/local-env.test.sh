...
local_env="$(
  env -i PATH="$PATH" bash -c '
    ...
    printf "%s\n" \
      ...
      "CHIMII_APP_URL=${CHIMII_APP_URL}" \
      "GOOGLE_REDIRECT_URI=${GOOGLE_REDIRECT_URI}" \
      "CHIMII_SERVER_URL=${CHIMII_SERVER_URL}" \
...
require_env "$local_env" 'CHIMII_APP_URL=http://localhost:3100'
require_env "$local_env" 'GOOGLE_REDIRECT_URI=http://localhost:3100/auth/callback'
require_env "$local_env" 'CHIMII_SERVER_URL=ws://localhost:9100/ws'
