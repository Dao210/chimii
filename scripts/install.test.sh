...
# release-binary path will download.
_setup_sandbox() {
  local tmp="$1"
  ...
  cat >"$payload_dir/chimii" <<'STUB'
#!/usr/bin/env bash
echo "chimii v0.3.2 (commit: test)"
STUB
  chmod +x "$payload_dir/chimii"
  tar -czf "$tmp/chimii.tar.gz" -C "$payload_dir" chimii

  cat >"$stub_bin/curl" <<'STUB'
#!/usr/bin/env bash
if [[ "$*" == *"-sI"* ]]; then
  printf 'HTTP/2 302\r\nlocation: https://github.com/Dao210/chimii/releases/tag/v0.3.2\r\n'
  exit 0
fi
...
_run_installer() {
  ...
  if ! PATH="$tmp/stub-bin:$tmp/install-bin:/usr/bin:/bin" \
    CHIMII_BIN_DIR="$tmp/install-bin" \
    CHIMII_TEST_ARCHIVE="$tmp/chimii.tar.gz" \
  ...
  if [[ ! -x "$tmp/install-bin/chimii" ]]; then
    echo "expected fallback binary at $tmp/install-bin/chimii" >&2
    cat "$out" >&2 || true
    cat "$err" >&2 || true
    return 1
  fi
...
test_install_uses_release_binary() {
  local tmp
  tmp="$(mktemp -d)"
  ...
  _setup_sandbox "$tmp"
  _run_installer "$tmp"
  if grep -q "Homebrew" "$tmp/install.out" "$tmp/install.err"; then
    echo "did not expect the installer to depend on an unpublished Homebrew tap" >&2
    return 1
  fi
}
...
test_remote_ssh_install_prints_token_login_hint() {
  ...
  if ! grep -q "https://chimii.ai/settings?tab=tokens" "$tmp/install.out"; then
  ...
  if ! grep -q "chimii login --token <YOUR_TOKEN>" "$tmp/install.out"; then
    echo "expected token login command in installer output" >&2
    cat "$tmp/install.out" >&2 || true
    return 1
  fi
  if grep -q "chimii config set server_url" "$tmp/install.out"; then
    echo "did not expect default cloud server config command in installer output" >&2
    cat "$tmp/install.out" >&2 || true
    return 1
  fi
  if grep -q "chimii config set app_url" "$tmp/install.out"; then
