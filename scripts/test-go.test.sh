...
TEST_DIR=$(mktemp -d "${TMPDIR:-/tmp}/chimii-test-go.XXXXXX")
...
mkdir -p "$BIN_DIR"
export CHIMII_TEST_GO_CALLS="$CALLS_FILE"
...
case "${1:-}" in
  list)
    ...
    printf '%s\n' \
      github.com/chimii-ai/chimii/server \
      github.com/chimii-ai/chimii/server/internal/daemon \
      github.com/chimii-ai/chimii/server/pkg/agent \
      github.com/chimii-ai/chimii/server/pkg/agent/internal/testutil
    ;;
  test)
    printf '%s\n' "$*" >>"$CHIMII_TEST_GO_CALLS"
    ;;
  *)
...
expected_calls='test -race github.com/chimii-ai/chimii/server github.com/chimii-ai/chimii/server/internal/daemon
