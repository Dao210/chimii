...
Chimii's product analytics live in the **operational database** and in
**Prometheus / Grafana**. This document is the catalogue of instrumentation and
its history.

See [MUL-1122](https://github.com/chimii-ai/chimii) for the original design
context and [MUL-4127](https://github.com/chimii-ai/chimii) for the PostHog
retirement below.
...
>   those.
> - The `chimii_signup_source` attribution cookie (`captureSignupSource`,
...
Self-hosters should **never inherit a Chimii-issued `POSTHOG_API_KEY`** —
that would route their users' behavior to our analytics project. The
defaults guarantee this:

- `.env.example` ships `POSTHOG_API_KEY=` empty, and native deployment
  configuration does not set a default.
...
- **Person properties (`$set`)** — use for mutable cohort signals
  ...
  `setPersonProperties()` in `@chimii/core/analytics`. Use
...
Grafana via `chimii_*` business counters (see `server/internal/metrics`).
...
| `signup_source` | string | Opaque attribution bundle from the frontend cookie `chimii_signup_source` (UTM + referrer). Empty when the cookie is absent. |
...
| `signup_source` | string | Attribution bundle. Today only its bucketed form survives, as the `chimii_signup_total{signup_source}` Prometheus label (see `NormalizeSignupSource`); it is no longer set as a person property for segmentation. |
...
| `cli_version` | string | Version of the `chimii` CLI that registered it. |
...
| Metric | Type | Labels |
|---|---|---|
| `chimii_agent_task_enqueued_total` | counter | `source`, `runtime_mode` |
| `chimii_agent_task_dispatched_total` | counter | `source`, `runtime_mode` |
| `chimii_agent_task_started_total` | counter | `source`, `runtime_mode`, `provider` |
| `chimii_agent_task_terminal_total` | counter | `source`, `runtime_mode`, `terminal_status` |
| `chimii_agent_task_failed_total` | counter | `source`, `runtime_mode`, `failure_reason` |
| `chimii_agent_task_queue_wait_seconds` | histogram | `source`, `runtime_mode` |
| `chimii_agent_task_run_seconds` | histogram | `source`, `runtime_mode`, `terminal_status` |
| `chimii_agent_task_total_seconds` | histogram | `source`, `runtime_mode`, `terminal_status` |
...
- `terminal_status` is the task's final `agent_task_queue.status` —
  ...
  `chimii_agent_task_terminal_total{terminal_status=…}`. Failures
  additionally increment `chimii_agent_task_failed_total` carrying the coarse
...
`chimii_issue_executed_total{source}` (not PostHog) and backed in the DB by
`issue.first_executed_at`. Per-task completion counts live in Grafana via
`BusinessMetrics.RecordTaskTerminal`; use `chimii_issue_executed_total` for the
...
- Attribution is NOT a separate event; UTM + referrer origin are written
  to the `chimii_signup_source` cookie on the first anonymous pageview
...
`BusinessMetrics.RecordTaskTerminal` counter (exported as a `chimii_*` task
...
`chimii_issue_executed_total` against `issue.first_executed_at` rather than a
PostHog event.
