...
[mul-963]: https://chimii-api.copilothub.ai/issues/28c34ad2-102a-4f46-91ac-336ed78c5859
...
If you see `no such host` *inside a Codex session on macOS* but `curl https://chimii-api.copilothub.ai` from a plain shell on the same machine works, you are hitting the Seatbelt bug below.
...
The daemon writes a *chimii-managed* block into each task's
`$CODEX_HOME/config.toml`, delimited by `# BEGIN chimii-managed` /
`# END chimii-managed` markers. Anything outside the markers is left
...
`[permissions.chimii]`, a bare `sandbox_mode = "..."` line would be parsed
as `permissions.chimii.sandbox_mode` and Codex would silently ignore it.
...
```bash
# Is the Chimii API reachable at all?
curl -sSf https://chimii-api.copilothub.ai/healthz
```

From inside a Codex session (after the daemon writes its config):

```bash
chimii issue list --limit 1 --output json >/dev/null && echo OK
```
...
- **A. Domain-scoped `permissions` profile** (tight): when the upstream
  `network_access` fix is available, prefer writing a `permissions.chimii`
  profile that allows only `chimii-api.copilothub.ai` and
  `chimii-static.copilothub.ai`. Keeps filesystem sandbox intact.
...
sed -n '/# BEGIN chimii-managed/,/# END chimii-managed/p' \
  ~/chimii_workspaces/$WORKSPACE_ID/$TASK_SHORT/codex-home/config.toml
```
