...
import { pickLocale, type SupportedLocale } from "@chimii/core/i18n";
import { useAuthStore } from "@chimii/core/auth";
import { useWelcomeStore } from "@chimii/core/onboarding";
import { workspaceKeys, workspaceListOptions } from "@chimii/core/workspace/queries";
import { api } from "@chimii/core/api";
import { useHasOnboarded } from "@chimii/core/paths";
import { setCurrentWorkspace } from "@chimii/core/platform";
import { ThemeProvider } from "@chimii/ui/components/common/theme-provider";
import { ChimiiIcon } from "@chimii/ui/components/common/chimii-icon";
import { Toaster } from "@chimii/ui/components/ui/sonner";
...
import { captureEvent } from "@chimii/core/analytics";
import { RESOURCES } from "@chimii/views/locales";
  ...
  if (isLoading) {
    return (
      <div className="flex h-screen items-center justify-center">
        <ChimiiIcon className="size-6 animate-pulse" />
      </div>
    );
  }
...
function AppContent() {
  ...
  // can pick the matching CLI profile (server_url from ~/.chimii config).
  useEffect(() => {
    if (!runtimeConfig) return;
    window.daemonAPI.setTargetApiUrl(runtimeConfig.apiUrl);
  }, [runtimeConfig]);

  // Listen for invite IDs delivered via deep link (chimii://invite/<id>).
  ...
  // Listen for auth token delivered via deep link (chimii://auth/callback?token=...).
    ...
    <div className="flex h-screen items-center justify-center bg-background p-8 text-foreground">
      <div className="max-w-xl rounded-lg border bg-card p-6 shadow-sm">
        ...
        <p className="mt-3 text-sm text-muted-foreground">
          Chimii Desktop could not load <code>~/.chimii/desktop.json</code>. Fix or remove the file and restart the app.
        </p>
