...
import { createPersistStorage, defaultStorage } from "@chimii/core/platform";
import { createSafeId } from "@chimii/core/utils";
import { isReservedSlug } from "@chimii/core/paths";
...
// render time via `routeIconForPath` (@chimii/views/layout), which shares the
// route → icon map in `@chimii/core/paths` with the sidebar nav — so the two
