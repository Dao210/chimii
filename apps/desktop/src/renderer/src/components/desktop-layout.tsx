...
import { ModalRegistry } from "@chimii/views/modals/registry";
import { AppSidebar, GlobalShortcuts } from "@chimii/views/layout";
import { SearchCommand, SearchTrigger } from "@chimii/views/search";
import { FloatingChat } from "@chimii/views/chat";
import { WorkspaceSlugProvider, paths, useCurrentWorkspace } from "@chimii/core/paths";
import { useNavigation } from "@chimii/views/navigation";
import { getCurrentSlug, subscribeToCurrentSlug } from "@chimii/core/platform";
import { useDesktopUnreadBadge } from "@chimii/views/platform";
import {
...
function useInternalLinkHandler() {
  useEffect(() => {
    ...
    window.addEventListener("chimii:navigate", handler);
    return () => window.removeEventListener("chimii:navigate", handler);
  }, []);
}
 ...
 * `chimii:navigate` event, whose handler `openTab`s into the ACTIVE
