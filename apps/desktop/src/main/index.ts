...
// chimii CLI can find agent binaries like claude/codex/opencode. Must run
...
function handleDeepLink(url: string): void {
  try {
    ...
    // chimii://auth/callback?token=<jwt>
...
function installLocaleRefresh(window: BrowserWindow): void {
  // Electron has no dedicated OS-language event. Check whenever any Chimii
...
// single-worktree case still lands at "Chimii Canary".
const DEV_APP_NAME = process.env.DESKTOP_APP_SUFFIX
  ? `Chimii Canary ${process.env.DESKTOP_APP_SUFFIX}`
  : "Chimii Canary";
...
if (is.dev) {
  ...
  // to "Chimii", but anchoring it here makes WM_CLASS ↔ StartupWMClass
...
if (!gotTheLock) {
  ...
  app.whenReady().then(async () => {
    ...
    electronApp.setAppUserModelId(
      is.dev ? "ai.chimii.desktop.dev" : "ai.chimii.desktop",
    );
