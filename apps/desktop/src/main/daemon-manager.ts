...
const PREFS_PATH = join(homedir(), ".chimii", "desktop_prefs.json");
...
// `chimii daemon start` blocks until the daemon reports ready, polling /health
// for up to its own startup timeout (45s in server/cmd/chimii/cmd_daemon.go) to
...
// Keep the Go impl in sync: server/cmd/chimii/cmd_daemon.go healthPortForProfile.
...
function profileDir(profile: string): string {
  return profile
    ? join(homedir(), ".chimii", "profiles", profile)
    : join(homedir(), ".chimii");
}
...
// Sidecar file that records which Chimii user the cached PAT in config.json
 ...
 * a stuck start is an auth problem. Hits the same endpoint `chimii auth status`
...
function findCliOnPath(): string | null {
  const candidates = process.platform === "win32" ? ["chimii.exe"] : ["chimii"];
 ...
 *   to `apps/desktop/resources/bin/chimii`. `bundle-cli.mjs` populates this
 *   before dev starts, so iterating on Go changes is "make build → restart".
 * - Packaged: `app.getAppPath()` → `<Chimii.app>/Contents/Resources/app.asar`.
...
function bundledCliPath(): string {
  const binName = process.platform === "win32" ? "chimii.exe" : "chimii";
...
/**
 * Returns a usable `chimii` binary path. Priority:
 ...
 *   5. `chimii` on PATH (dev convenience / user-installed via brew).
...
async function mintPat(jwt: string): Promise<string> {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({ name: "Chimii Desktop" }),
  });
  if (!res.ok) {
...
function desktopSpawnEnv(): NodeJS.ProcessEnv {
  return { ...process.env, CHIMII_LAUNCHED_BY: "desktop" };
}

async function startDaemon(): Promise<{ success: boolean; error?: string }> {
  const bin = await resolveCliBinary();
  if (!bin) return { success: false, error: "chimii CLI is not installed" };
