const RELEASE_ARCHIVE_PREFIX = "chimii-cli-";
...
export function selectPlatformReleaseAssetName(
  ...
  // Prefer the versioned `chimii-cli-<v>-<os>-<arch>.<ext>` name; fall
  // back to the legacy `chimii_<os>_<arch>.<ext>` so older releases that
  ...
  const legacyName = `chimii_${os}_${mappedArch}.${ext}`;
