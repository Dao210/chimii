...
// Desktop prefers the bundled `chimii` CLI shipped inside the app for
...
async function installFresh(): Promise<string> {
  ...
  const workDir = join(tmpdir(), `chimii-cli-${Date.now()}`);
...
/**
 * Returns the path to a usable `chimii` binary. If one is already present at
