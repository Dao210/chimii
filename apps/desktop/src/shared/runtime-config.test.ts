...
describe("runtime config", () => {
  it("uses cloud defaults without a desktop.json file", () => {
    expect(DEFAULT_RUNTIME_CONFIG).toEqual({
      schemaVersion: 1,
      apiUrl: "https://api.chimii.ai",
      wsUrl: "wss://api.chimii.ai/ws",
  ...
  it("strips the leading api. label when deriving appUrl", () => {
    expect(
      parseRuntimeConfig(
        JSON.stringify({ schemaVersion: 1, apiUrl: "https://api.chimii.ai" }),
      ),
    ).toEqual({
      schemaVersion: 1,
      apiUrl: "https://api.chimii.ai",
  ...
  it("rejects non-http api schemes", () => {
    expect(() =>
      parseRuntimeConfig(JSON.stringify({ schemaVersion: 1, apiUrl: "file:///tmp/chimii" })),
  ...
  it("derives dev appUrl by stripping the leading api. label", () => {
    ...
    // public web host, not the api host. Chimii's convention exposes the
    // api at `api.<web-host>`, so stripping the leading label gives the
    // right web origin without a separate VITE_APP_URL.
    expect(
      runtimeConfigFromDevEnv({ apiUrl: "https://api.test.chimii.ai" }),
    ).toEqual({
      schemaVersion: 1,
      apiUrl: "https://api.test.chimii.ai",
      wsUrl: "wss://api.test.chimii.ai/ws",
      appUrl: "https://test.chimii.ai",
    });
  });
  ...
  it("dev VITE_APP_URL still wins over apiUrl-derived value", () => {
    expect(
      runtimeConfigFromDevEnv({
        apiUrl: "https://api.test.chimii.ai",
        appUrl: "https://staging.chimii.ai",
      }),
    ).toEqual({
      schemaVersion: 1,
      apiUrl: "https://api.test.chimii.ai",
