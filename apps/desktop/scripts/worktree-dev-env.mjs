...
// (keyed by the app name "Chimii Canary"). The env hooks to override both
...
// The dev app then shows e.g. "Chimii Canary mul-3724-194" in Cmd+Tab and gets
...
// a basename at different paths (e.g. /a/chimii vs /b/chimii) or whose names
...
// the primary checkout keeps the unchanged 5173 / "Chimii Canary" defaults.
  ...
  if (log) {
    console.log(
      `[dev:desktop] worktree isolation → renderer port ${env.DESKTOP_RENDERER_PORT}, ` +
        `app "Chimii Canary ${env.DESKTOP_APP_SUFFIX}"`,
    );
  }
  return env;
}
