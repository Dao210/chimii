...
describe("worktree-dev-env", () => {
  ...
  it("disambiguates worktrees that share a folder name at different paths", () => {
    // Same basename "chimii", different parent dirs → different offsets/suffixes,
    // so each gets its own single-instance lock.
    expect(offsetForPath("/tmp/a/chimii")).not.toBe(offsetForPath("/tmp/b/chimii"));
    expect(appSuffixForPath("/tmp/a/chimii")).not.toBe(
      appSuffixForPath("/tmp/b/chimii"),
    );
  });
