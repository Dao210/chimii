...
import enCommon from "@chimii/views/locales/en/common.json";
import enAuth from "@chimii/views/locales/en/auth.json";
import enSettings from "@chimii/views/locales/en/settings.json";
...
describe("LoginPage", () => {
  ...
  // the chimii:// deep link, not silently redirect to the workspace page.
  it("mints a token and deep-links to Desktop when already logged in with platform=desktop", async () => {
    ...
    authStateRef.state.user = { id: "u1", email: "test@chimii.ai" };
    ...
    try {
      ...
      await waitFor(() => {
        expect(hrefSetter).toHaveBeenCalledWith(
          "chimii://auth/callback?token=handoff-jwt",
        );
      });
      expect(
        await screen.findByRole("button", { name: "Open Chimii Desktop" }),
      ).toBeInTheDocument();
    } finally {
      Object.defineProperty(window, "location", {
        configurable: true,
  ...
  describe("post-login redirect ownership (#5009)", () => {
    ...
    it("fetches the workspace list before redirecting a visitor who arrived authenticated", async () => {
      ...
      await waitFor(() => {
        expect(mockReplace).toHaveBeenCalledWith("/acme/creations");
      });
