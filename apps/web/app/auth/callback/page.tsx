...
import { paths, resolvePostAuthDestination } from "@chimii/core/paths";
import { api } from "@chimii/core/api";
import { validateCliCallback, redirectToCliCallback } from "@chimii/views/auth";
import {
  Card,
  CardHeader,
  CardTitle,
...
function CallbackContent() {
  ...
  useEffect(() => {
    ...
    // headless/WSL2 `chimii login` can receive the JWT after browser-based
    ...
    if (cliCallback) {
      ...
      loginWithGoogle(code, redirectUri)
        .then(async (loggedInUser) => {
          ...
          //    login on chimii.ai instead of clicking the email link,
      ...
      <div className="flex min-h-screen items-center justify-center">
        <Card className="w-full max-w-sm">
          <CardHeader className="text-center">
            <CardTitle className="text-2xl">Opening Chimii</CardTitle>
            <CardDescription>
              You should see a prompt to open the Chimii desktop app. If
