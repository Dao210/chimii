/**
 * MUL-5208 — the web half of the `chimii:navigate` bridge.
 *
...
import { useNavigation, type NavigationAdapter } from "@chimii/views/navigation";

function navigate(path: string) {
  window.dispatchEvent(
    new CustomEvent("chimii:navigate", { detail: { path } }),
  );
}
...
describe("WebNavigationProvider internal link bridge", () => {
  ...
  it("ignores an event without a path", () => {
    ...
    window.dispatchEvent(new CustomEvent("chimii:navigate", { detail: {} }));
 ...
 * or replaces with a fallback. A wrong `true` walks the user out of Chimii,
