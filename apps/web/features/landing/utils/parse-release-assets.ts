 ...
 *   chimii-desktop-{version}-mac-{arch}.{dmg|zip}
 *   chimii-desktop-{version}-windows-{arch}.exe
 *   chimii-desktop-{version}-linux-{arch}.{AppImage|deb|rpm}
 *
...
const DESKTOP_ARTIFACT_RE =
  /^chimii-desktop-[^-]+-(mac|windows|linux)-([a-z0-9_]+)\.(dmg|zip|exe|AppImage|deb|rpm)$/i;
