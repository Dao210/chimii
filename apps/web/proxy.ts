...
import { LOCALE_COOKIE } from "@chimii/core/i18n";
import {
  CHIMII_LOCALE_HEADER,
...
// Forward the resolved locale to RSC layouts via the `x-chimii-locale`
...
function nextWithLocale(req: NextRequest): NextResponse {
  ...
  headers.set(CHIMII_LOCALE_HEADER, resolveLocale(req));
...
export function proxy(req: NextRequest) {
  ...
  const hasSession = req.cookies.has("chimii_logged_in");
  ...
  // https://chimii.ai/ must remain a public-site navigation even when a local
