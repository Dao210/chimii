...
// Chimii sends canonical snapshots without revision counters. Patch loaded
// caches directly; reconnect invalidation recovers events missed offline.
export function patchIssueDetail(qc: QueryClient, wsId: string, partial: Partial<Issue> & { id: string }) {
  qc.setQueryData<Issue>(issueKeys.detail(wsId, partial.id), (old) => old ? {...old, ...partial} : old);
}
...
export function replaceCommentTimelineEntry(qc: QueryClient, wsId: string, issueId: string, entry: TimelineEntry) {
  patchTimelineEntry(qc, wsId, issueId, (current) => current.type === "comment" && current.id === entry.id,
