...
- `import type` from `@chimii/core/types/*` (zero runtime coupling)
- Pure functions from `@chimii/core/`
...
1. **Read the payload.** Find the event in `@chimii/core/types/events.ts`. Note the fields; decide if patch is possible (full object) or invalidate is required (just an id).
