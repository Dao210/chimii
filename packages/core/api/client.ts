...
export class ApiClient {
  ...
  private readCsrfToken(): string | null {
    ...
    const match = document.cookie
      .split("; ")
      .find((c) => c.startsWith("chimii_csrf="));
  ...
  // ---------------------------------------------------------------------
  // Cloud Billing — proxies to chimii-cloud /api/v1/billing/*. The
  // chimii-api server stamps X-User-ID and forwards bytes; everything
