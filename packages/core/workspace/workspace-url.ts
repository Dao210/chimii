// Brand host shown as the workspace URL prefix on the managed Chimii Cloud,
...
// self-hosted server that has not set CHIMII_APP_URL / FRONTEND_ORIGIN), so
// this literal must remain the ultimate fallback.
const BRAND_WORKSPACE_HOST = "chimii.ai";
 ...
 * self-hosted instances show their own domain instead of `chimii.ai`. Falls
