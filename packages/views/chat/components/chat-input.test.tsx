...
import type { UploadResult } from "@chimii/core/hooks/use-file-upload";
import type { DraftUpload } from "@chimii/core/drafts";
...
function renderInput(props: Partial<React.ComponentProps<typeof ChatInput>> = {}) {
  ...
  render(
    <I18nProvider locale="en" resources={TEST_RESOURCES}>
      <ChatInput onSend={onSend} uploadEnabled agentName="Chimii" {...props} />
    </I18nProvider>,
  );
  return { onSend };
}
...
function element(props: Partial<React.ComponentProps<typeof ChatInput>>) {
  return (
    <I18nProvider locale="en" resources={TEST_RESOURCES}>
      <ChatInput onSend={vi.fn()} uploadEnabled agentName="Chimii" {...props} />
    </I18nProvider>
  );
}
...
describe("ChatInput focusRequest", () => {
  it("focuses the editor when focusRequest becomes a non-zero value (new chat)", () => {
    const { rerender } = render(
      <I18nProvider locale="en" resources={TEST_RESOURCES}>
        <ChatInput onSend={vi.fn()} agentName="Chimii" focusRequest={0} />
      </I18nProvider>,
    );
    ...
    rerender(
      <I18nProvider locale="en" resources={TEST_RESOURCES}>
        <ChatInput onSend={vi.fn()} agentName="Chimii" focusRequest={1} />
      </I18nProvider>,
    );
    ...
    rerender(
      <I18nProvider locale="en" resources={TEST_RESOURCES}>
        <ChatInput onSend={vi.fn()} agentName="Chimii" focusRequest={2} />
      </I18nProvider>,
    );
