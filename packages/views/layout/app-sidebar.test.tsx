...
vi.mock("@chimii/ui/components/ui/sidebar", () => ({
...
vi.mock("@chimii/ui/components/ui/dropdown-menu", () => ({
...
vi.mock("@chimii/core/paths", async (importOriginal) => ({
...
vi.mock("@chimii/core/api", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@chimii/core/api")>();
  return {
    ...actual,
    api: {
      ...actual.api,
...
vi.mock("@chimii/core/inbox/queries", () => ({
...
vi.mock("@chimii/core/issues/queries", () => ({ issueDetailOptions: () => ({ queryKey: ["issue"] }) }));
vi.mock("@chimii/core/issues/stores/create-mode-store", () => ({
  useCreateModeStore: { getState: () => ({ lastMode: "agent" }) },
  openCreateIssueWithPreference: vi.fn(),
}));
vi.mock("@chimii/core/issues/stores/draft-store", () => ({ useIssueDraftStore: () => false }));
vi.mock("@chimii/core/modals", () => ({ useModalStore: { getState: () => ({ modal: null, open: vi.fn() }) } }));
vi.mock("@chimii/core/pins/mutations", () => ({ useDeletePin: () => ({ mutate: deletePin }), useReorderPins: () => ({ mutate: vi.fn() }) }));
vi.mock("@chimii/core/pins/queries", () => ({ pinListOptions: () => ({ queryKey: ["pins"] }) }));
vi.mock("@chimii/core/projects/queries", () => ({ projectDetailOptions: () => ({ queryKey: ["project"] }) }));
vi.mock("@chimii/core/workspace/queries", () => ({
...
vi.mock("@tanstack/react-query", async (importOriginal) => ({
  ...
  useQuery: ({ queryKey }: { queryKey: readonly unknown[] }) => {
    if (queryKey[0] === "child-mode") return { data: { mode: "parent" } };
...
describe("PinRow", () => {
  ...
  it("highlights the active pin without exposing the retired Issues navigation", async () => {
...
describe("personal navigation", () => {
  beforeEach(() => {
    inboxItems.current = [];
    navigation.current = { pathname: "/acme/creations" };
  });

  it("keeps the persistent Inbox counter static", () => {
  ...
  it("does not expose the retired Chat navigation even with unread sessions", () => {
    chatSessions.current = [{ id: "chat-1", unread_count: 2 }];
    const { container } = render(<AppSidebar />);
    expect(container.querySelector('button[data-href="/acme/chat"]')).toBeNull();
