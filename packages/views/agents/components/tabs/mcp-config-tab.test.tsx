...
vi.mock("@chimii/core/runtimes", async () => {
  const actual =
    await vi.importActual<typeof import("@chimii/core/runtimes")>(
      "@chimii/core/runtimes",
    );
  return {
    ...actual,
...
describe("McpConfigTab", () => {
  ...
  it("projects historical aggregate config into individually managed rows", () => {
    ...
    expect(screen.getByRole("heading", { name: /managed by chimii/i })).toBeInTheDocument();
