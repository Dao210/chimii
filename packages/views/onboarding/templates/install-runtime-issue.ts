...
const ko = `Chimii에 오신 것을 환영합니다.

agent가 작업을 실행하려면 먼저 runtime이 필요합니다. runtime을 설치하는 동안에도 Chimii를 가벼운 프로젝트 관리 워크스페이스로 먼저 사용할 수 있습니다.
...
전체 가이드: https://chimii.ai/docs/install-agent-runtime
...
runtime이 연결되면 Chimii Helper를 만들어 안내를 받으며 첫 실행을 시작할 수 있습니다.`;

const ja = `Chimii へようこそ。

agent が作業を実行するには、まず runtime が必要です。runtime をインストールしている間も、Chimii を軽量なプロジェクト管理ワークスペースとして先に使うことができます。
...
詳しいガイド: https://chimii.ai/docs/install-agent-runtime
...
runtime が接続されたら、Chimii Helper を作成して、案内付きの最初の実行を始められます。`;
