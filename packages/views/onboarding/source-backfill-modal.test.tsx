...
vi.mock("@chimii/core/auth", async () => {
  const actual =
    await vi.importActual<typeof import("@chimii/core/auth")>(
      "@chimii/core/auth",
    );
...
vi.mock("@chimii/core/onboarding", async () => {
  const actual =
    await vi.importActual<typeof import("@chimii/core/onboarding")>(
      "@chimii/core/onboarding",
    );
...
vi.mock("@chimii/core/paths", async () => {
  const actual =
    await vi.importActual<typeof import("@chimii/core/paths")>(
      "@chimii/core/paths",
    );
...
vi.mock("@chimii/core/api", async () => {
  const actual =
    await vi.importActual<typeof import("@chimii/core/api")>(
      "@chimii/core/api",
    );
  return {
    ...actual,
...
function wipeDismissCounters() {
  for (let i = window.localStorage.length - 1; i >= 0; i--) {
    ...
    if (k && k.startsWith("chimii.source_backfill.dismiss.")) {
...
describe("SourceBackfillModal", () => {
  it("does not render when there is no user", () => {
    renderModal();
    expect(
      screen.queryByText(/How did you hear about Chimii/i),
  ...
  it("opens for an onboarded user with empty source once agents completed enough issues", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/How did you hear about Chimii/i),
      ).toBeInTheDocument();
    });
  });
  ...
  it("does not open once the per-user dismiss cap is reached on this browser", () => {
    window.localStorage.setItem("chimii.source_backfill.dismiss.u1", "3");
    setUser({
      id: "u1",
      onboarded_at: "2026-01-01T00:00:00Z",
      onboarding_questionnaire: { source: [] },
