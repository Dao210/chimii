...
import { runtimeKeys } from "@chimii/core/runtimes/queries";
...
function daemonCommands(serverUrl: string | undefined, appUrl: string | undefined) {
  ...
  if (normalizedServerUrl && normalizedAppUrl) {
    return {
      setupCmd: `chimii setup self-host --server-url ${normalizedServerUrl} --app-url ${normalizedAppUrl}`,
      tokenCmd: `chimii config set server_url ${normalizedServerUrl}
chimii config set app_url ${normalizedAppUrl}
chimii login --token <YOUR_TOKEN>
chimii daemon start`,
    };
  }

  return {
    setupCmd: "chimii setup",
    tokenCmd: `chimii config set server_url ${CLOUD_SERVER_URL}
chimii config set app_url ${CLOUD_APP_URL}
chimii login --token <YOUR_TOKEN>
chimii daemon start`,
  };
}
...
export function ConnectRemoteDialog({ onClose }: { onClose: () => void }) {
  ...
  // `chimii setup` is one blocking command that handles config + login
