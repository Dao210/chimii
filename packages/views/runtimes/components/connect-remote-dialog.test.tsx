...
import { configStore } from "@chimii/core/config";
...
describe("ConnectRemoteDialog", () => {
  ...
  it("uses cloud setup commands by default", () => {
    ...
    expect(baseElement).toHaveTextContent("chimii setup");
    expect(baseElement).not.toHaveTextContent("chimii setup self-host");
    expect(baseElement).toHaveTextContent(
      "chimii config set server_url https://api.chimii.ai",
    );
    expect(baseElement).toHaveTextContent(
      "chimii config set app_url https://chimii.ai",
    );
  });
  ...
  it("disables font ligatures in setup command code", () => {
    ...
    const setupCode = Array.from(baseElement.querySelectorAll("code")).find((node) =>
      node.textContent?.includes("chimii setup"),
    );
  ...
  it("disables font ligatures in fallback token command code", () => {
    ...
    const tokenCode = Array.from(baseElement.querySelectorAll("code")).find((node) =>
      node.textContent?.includes("chimii login --token <YOUR_TOKEN>"),
    );
  ...
  it("transitions from setup instructions to the connected state", async () => {
    ...
    expect(baseElement).not.toHaveTextContent("chimii setup");
  });
});
