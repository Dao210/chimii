...
import { larkInstallationsOptions, larkKeys } from "@chimii/core/lark";
import { api, ApiError } from "@chimii/core/api";
import type { LarkInstallation, LarkInstallStatusResponse } from "@chimii/core/types";
...
// install path is per-agent (each Chimii Agent gets exactly one Bot —
...
function InstallationRow({
  ...
  const { t } = useT("settings");
  // The bot is bound 1:1 to a Chimii Agent (per the (workspace_id,
  // agent_id) UNIQUE in lark_installation). Render the Chimii agent's
