...
import type { Attachment as AttachmentRecord } from "@chimii/core/types";
...
vi.mock("@chimii/core/paths", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@chimii/core/paths")>();
  return {
    ...actual,
...
describe("Attachment — image dispatch", () => {
  ...
  it("renders the configured CDN URL when description markdown stores the stable API URL", () => {
    ...
    const markdownUrl = `https://chimii-api.copilothub.ai/api/attachments/${id}/download`;
    const att = makeRecord({
      id,
  ...
  it("re-signs the inline media URL through getAttachment on token-mode clients (MUL-3254)", async () => {
    ...
    getBaseUrlMock.mockReturnValue("https://chimii-api.copilothub.ai");
    configStore.setState({ cdnDomain: "cdn.example.test", cdnSigned: true });
    const id = "11111111-2222-3333-4444-555555555555";
    const markdownUrl = `https://chimii-api.copilothub.ai/api/attachments/${id}/download`;
    const signed =
  ...
  it("stable /api/attachments/<id>/download download_url falls through to the durable record.markdown_url for native <img> loadability (MUL-3192)", () => {
    ...
    const att = makeRecord({
      ...
      markdown_url: "https://api.chimii.test/api/attachments/att-1/download",
    ...
    const img = document.querySelector("img");
    expect(img?.getAttribute("src")).toBe(
      "https://api.chimii.test/api/attachments/att-1/download",
    );
