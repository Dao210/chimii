...
vi.mock("@chimii/core/issues/queries", () => ({
...
vi.mock("@chimii/core/issues/stores/draft-store", () => ({
...
vi.mock("@chimii/core/issues/stores/quick-create-store", () => ({
  useQuickCreateStore: (selector?: (state: typeof mockQuickCreateStore) => unknown) =>
    (selector ? selector(mockQuickCreateStore) : mockQuickCreateStore),
}));

vi.mock("@chimii/core/issues/stores/issue-create-settings-store", () => ({
...
vi.mock("@chimii/core/issues/mutations", () => ({
  useCreateIssue: () => ({ mutateAsync: mockCreateIssue }),
  useUpdateIssue: () => ({ mutate: vi.fn() }),
}));

vi.mock("@chimii/core/labels", () => ({
  useAttachLabelToIssue: () => ({ mutateAsync: mockAttachLabel }),
}));

vi.mock("@chimii/core/properties", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@chimii/core/properties")>();
  return {
    ...actual,
    useSetIssueProperty: () => ({
...
vi.mock("@chimii/core/api", async () => {
  ...
  const { parseWithFallback } = await vi.importActual<typeof import("@chimii/core/api/schema")>(
    "@chimii/core/api/schema",
  );
  const { DuplicateIssueErrorBodySchema } = await vi.importActual<
    typeof import("@chimii/core/api/schemas")
  >("@chimii/core/api/schemas");
  return {
    api: {
...
vi.mock("@chimii/ui/components/ui/dialog", () => ({
...
vi.mock("@chimii/ui/components/ui/button", () => ({
  Button: ({
    children,
    disabled,
    onClick,
...
vi.mock("@chimii/ui/components/ui/switch", () => ({
  Switch: ({
    checked,
    onCheckedChange,
  }: {
...
vi.mock("@chimii/ui/components/common/file-upload-button", () => ({
...
describe("CreateIssueModal", () => {
  beforeEach(() => {
    ...
    mockApiUploadFile.mockResolvedValue({
      ...
      markdown_url: "https://chimii-api.copilothub.ai/api/attachments/11111111-2222-3333-4444-555555555555/download",
  ...
  it("prunes draft attachments the reopened description no longer references", async () => {
    ...
    const deleted = {
      ...
      markdown_url: "https://chimii-api.copilothub.ai/api/attachments/99999999-8888-7777-6666-555555555555/download",
    };
