...
export function CreationDetailPage({ creationId }: { creationId: string }) {
  ...
  const { data, isLoading, isError, refetch } = useQuery(
    buildCreationOptions(workspaceId, creationId),
  );
  return (
    <main className="h-full overflow-y-auto bg-[#f4ead5] text-[#1d241f]">
      <div className="mx-auto max-w-[1320px] px-5 py-7 md:px-8">
        <AppLink
          href={workspacePaths.creations()}
          className="mb-5 inline-flex items-center gap-2 text-sm font-black text-[#39715a] hover:underline"
        >
        ...
        {isLoading ? (
          ...
          <div className="flex min-h-[560px] flex-col items-center justify-center rounded-[2rem] border-2 border-dashed border-[#9c9079] bg-[#fff8e9] text-center">
            <h1 className="text-2xl font-black">
              {t(($) => $.detail_error_title)}
            </h1>
            <p className="mt-2 text-[#687068]">
          ...
          <>
            <AppLink
              ...
              href={`${workspacePaths.build()}?${new URLSearchParams({ source_kind: "brick", source_id: data.id, source_hash: data.build_plan.content_hash })}`}
            >
