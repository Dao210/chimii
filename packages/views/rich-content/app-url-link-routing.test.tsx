...
beforeEach(() => {
  navigatedPaths = [];
  window.addEventListener("chimii:navigate", captureNavigate);
  openSpy = vi.spyOn(window, "open").mockImplementation(() => null);
});

afterEach(() => {
  window.removeEventListener("chimii:navigate", captureNavigate);
  vi.restoreAllMocks();
});
...
describe("RichContent link routing", () => {
  ...
  it("still hands a genuinely external link to the browser", () => {
    const external = "https://github.com/chimii-ai/chimii/pull/1";
