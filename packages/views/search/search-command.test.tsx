...
const {
  ...
  mockGetShareableUrl: vi.fn((p: string) => `https://app.chimii/${p}`),
  mockMembers: {
    current: [] as Array<{
      id: string;
      workspace_id: string;
...
vi.mock("@chimii/core/issues/stores", () => {
...
vi.mock("@chimii/ui/components/common/theme-provider", () => ({
...
describe("SearchCommand", () => {
  beforeEach(() => {
    ...
    mockGetShareableUrl.mockReset().mockImplementation((p: string) => `https://app.chimii/${p}`);
    mockMembers.current = [];
  ...
  it("copies issue link and identifier when on an issue detail route", async () => {
    ...
    expect(mockClipboardWrite).toHaveBeenCalledWith("https://app.chimii//ws-test/issues/issue-1");
